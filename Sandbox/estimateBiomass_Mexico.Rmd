...
if(exists(here("Figs/fig_habitat_map.png"))) {
...
# mapview(nav.paths.jcv.sf) + mapview(transects.jcf, zcol = "Type")
...
if (get.nav.sd) {
  ...
  if (get.nav) {
    if (nav.lag.sd >= 24) {
      ...
      nav.temp.sd <- data.frame(read.csv(saildroneURL, header = FALSE, 
...
if (resize.map) {
  ...
  if (survey.name %in% c("2107RL")) {
    ...
    # # Use combined nav data
    # map.bounds <- nav.all.sf %>%
    #   st_transform(crs = crs.proj) %>%
    #   st_bbox()
    # 
    # # Use manually defined boundaries, based on Lasker and Carranza data
    # map.bounds <- data.frame(lat = survey.lat, 
    #                          long = rev(survey.long)) %>% 
    #   st_as_sf(coords = c("long","lat"), crs = crs.geog) %>%
    #   st_transform(crs = crs.proj) %>%
    #   st_bbox()
  } else {
...
if (trawl.source == "Access") {
  ...
  haul.all <- haul.all %>% 
    ...
    rename(totalNum = TotalNum,
           totalWeight = TotalWtKg)
...
if (save.figs) {
  ...
  source(here("Code/plot_haul_proportion_wt_mex.R"))
  source(here("Code/plot_cluster_proportion_wt_mex.R"))
  ...
  ggsave(nasc.trawl.cluster.wt,
         filename = here("Figs/fig_nasc_trawl_cluster_wt_mex.png"),
         width = map.width*2, height = map.height)
  
  save(trawl.pie.cluster.wt, trawl.pie.cluster.wt, nasc.trawl.cluster.wt,
       file = here("Output/trawl_species_proportion_map_mex.Rdata"))
  
} else {
  load(here("Output/trawl_species_proportion_map_mex.Rdata"))
}

include_graphics(here("Figs/fig_nasc_trawl_cluster_wt_mex.png"))
```  
...
if (save.figs) {
  ...
  ggsave(nasc.cufes.trawl.plot.long, 
         filename = here("Figs/fig_nasc_cufes_haul_wt_long_mx.png"),
         width = map.width, height = map.height*3)
  
  save(nasc.cufes.trawl.plot, nasc.cufes.trawl.plot.long, 
       file = here("Output/nasc_cufes_trawl_map_mx.Rdata"))
}

include_graphics(here("Figs/fig_nasc_cufes_haul_wt_long_mx.png"))
```
...
source(here("Code/calculate_haul_length_frequency_mex.R"))
```
...
if (save.figs) {
  ...
  ggsave(acoustic.prop.comparison,
         filename = here("Figs/fig_acoustic_proportion_comparison_mx.png"),
         width = map.width*2, height = map.height)
...
include_graphics(here("Figs/fig_acoustic_proportion_comparison_mx.png"))
```
...
if (save.figs) {
  ...
  ggsave(nasc.cufes.prop.plot, 
         filename = here("Figs/fig_nasc_cufes_acoustic_cluster_mx.png"),
         width = map.width*3, height = map.height)
  
  ggsave(nasc.cufes.prop.plot.long, 
         filename = here("Figs/fig_nasc_cufes_acoustic_cluster_long_mx.png"),
         width = map.width, height = map.height*3)  
  
  ggsave(nasc.trawl.acoustic.prop, 
         filename = here("Figs/fig_nasc_acoustic_cluster_mx.png"),
         width = map.width*2, height = map.height)
  
  save(nasc.cufes.prop.plot, nasc.cufes.prop.plot.long, nasc.trawl.acoustic.prop,
       file = here("Output/nasc_cufes_prop_map_mex.Rdata"))
}

include_graphics(here("Figs/fig_nasc_cufes_acoustic_cluster_long_mx.png"))
```
...
stock.breaks <- data.frame(
  name = c("Sardinops sagax", "Engraulis mordax"),
...
if (nrow(strata.summ.rm) > 0) {
  ...
  biomass.dens.plot.lat <- ggplot(strata.summ) + 
    ...
    geom_text(data = stock.breaks,
              ...
              fontface = "italic", alpha = 0.5, colour = "red", size = 3) +
    geom_path(data = nasc.density.summ,  
              aes(start.lat, log(density + 1), group = scientificName)) +
    geom_point(data = nasc.density.summ, 
               aes(start.lat, log(density + 1), group = scientificName,
...
ctd.nav <- select(nav, datetime = time, long, lat) %>% 
