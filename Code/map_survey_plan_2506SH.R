# # Map "Saildrone" and Long Beach Carnage and Lisa Marie transects for 
# Summer 2025 Integrated West Coast Pelagics Survey (2506SH)
...
# Set padding around data
map.bounds <- transects %>%
  filter(Type == "Compulsory", Transect %in% c(19, 67)) %>%
  st_transform(crs = crs.proj) %>%
  st_bbox()
