    ...
    for (i in rsk.files.kite) {
      ...
      tbl.kite.tmp <- as_tibble(rsk.kite@data) %>% 
        mutate(filename = path_file(rsk.kite@metadata$filename),
               cruise = str_extract(filename, tdr.pattern.cruise),
...
if(nrow(tbl.all.new) > 0) {
  for (k in unique(tbl.all.new$haul)) {
    ...
    tdr.plot <- filter(tbl.all.new, haul == k) %>%
      # Correct time for kite and footrope, if necessary
      mutate(time = case_when(
        loc == "Kite" ~ time + hours(tdr.offset.k[k]),
        loc == "Footrope"~ time + hours(tdr.offset.f[k])))
    
    # tdr.plot %>% group_by(loc) %>% summarize(start = min(time), end = max(time))
    ...
    if ("Kite" %in% tdr.plot$loc && "Footrope" %in% tdr.plot$loc) {
      ...
      # tdr.plot.wider |>
      #   dplyr::summarise(n = dplyr::n(), .by = c(time, loc)) |>
      #   dplyr::filter(n > 1L)
    }
    ...
    if(exists("tv80.meas")) {
      ...
      trawl.tv80 <- tv80.meas %>% 
        mutate(datetime = with_tz(datetime, tzone = tdr.tz) + hours(tv80.offset))  %>%
