...
if (nav.source == "ERDDAP") {
  ...
  nav <- read_GPX(here("Data/Nav", seine.gpx.name))$tracks
  
  # Extract trackline points from GPX file
  nav.points <- read_GPX(here("Data/Nav", seine.gpx.name))$track_points %>% 
    project_sf(crs = 4326) %>% 
...
if (exists("nav"))
  i.map <- i.map %>% 
  addPolylines(data = nav, color = "blue", stroke = TRUE, weight = 2,
