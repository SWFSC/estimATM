...
# Define leg ends
leg.ends <- c(ymd("2023-12-11"), ymd("2023-12-15"))
...
nav.vessels.ns <- NA_character_

# Survey information ------------------------------------------------------
# Full survey name; only used in report title
survey.name.long       <- "Multifunction Trawl (MFT) Sea Trials"
...
survey.name            <- "2312RL"        # SWFSC/AST survey name
survey.start           <- "11 December"       # Survey start date
survey.end             <- "15 December"    # Survey end date
survey.year            <- "2023"          # Survey year, for report
survey.season          <- "Summer"        # Survey season, for report
survey.das             <- 5              # Days at sea allocated
survey.landmark.n      <- "San Diego, CA" # Landmark - N extent of survey
...
leg.breaks <- as.numeric(lubridate::ymd(c("2023-12-11", "2023-12-15")))
