...
pacman::p_load(tidyverse,plotly,lubridate,mapview,kableExtra,leaflet,gdtools,
               here,bookdown,odbc,DBI,knitr,DT,fs,sf,leafem,naniar,ggiraph,
               cowplot,xts,tmaptools,htmltools,rnaturalearth,scatterpie)
...
get.nav    <- T # Download nearshore vessel nav data
resize.map <- T # Adjust map boundaries to data
...
if (nav.source.ns == "ERDDAP") {
  ...
  nav.ns <- read_GPX(here("Data/Nav", seine.gpx.name))$tracks
  
  # Extract trackline points from GPX file
  nav.ns.points <- read_GPX(here("Data/Nav", seine.gpx.name))$track_points %>% 
    project_sf(crs = 4326) %>% 
...
# Convert sets to sf to set map bounds
sets.sf <- sets %>% 
...
# Create base map
base.map <- get_basemap(nav.ns, states, countries, locations, bathy, map.bounds, crs = crs.proj) 

# Save the basemap
ggsave(base.map, file = here("Figs/fig_basemap_checkSeines.png"), 
       height = map.height, width = map.width)

save(base.map, file = here("Data/Map/basemap_checkSeines.Rdata"))
```
...
# Compute mean individual weights for each seine species
seine.wt.indiv <- set.catch %>% 
  group_by(scientificName) %>% 
  summarise(totalWeight = sum(totalWeightkg, na.rm = TRUE),
            totalNum    = sum(totalNum, na.rm = TRUE)) %>% 
  mutate(indivWeight = round(totalWeight/totalNum, 3))

# Add mean individual weight and compute missing weights
set.catch <- set.catch %>% 
  left_join(select(seine.wt.indiv, scientificName, indivWeight)) %>% 
  mutate(totalWeight = case_when(
    is.na(totalWeightkg) ~ totalNum * indivWeight,
    TRUE ~ totalWeightkg)) %>% 
  mutate(totalWeight = round(totalWeight, 3))
...
if (scale.pies) {
  set.catch.pie$r    <- pie.radius*log(set.catch.pie$bin.level+1)
} else {
  set.catch.pie$r    <- pie.radius
}

set.pos <- filter(set.catch.pie, AllCPS > 0) %>% 
  arrange(desc(X))
...
set.zero <- filter(set.pos, AllCPS == 0)
...
pie.cols.ns <- set.pos %>% 
  select(key:RndHerring, -set_type) %>% 
  pivot_longer(cols = -key, names_to = "spp", values_to = "totalWeight") %>% 
  group_by(spp) %>% 
...
if (nrow(set.pos) > 0) {
  ...
  set.pie.wt <- base.map +
    ...
    # Plot trawl pies
    geom_scatterpie(data = set.pos, aes(X, Y, group = key, r = pie.radius, colour = ship),
                    cols = pie.cols[names(pie.cols) %in% pie.spp.ns],
                    alpha = 0.8, sorted_by_radius = TRUE) +
    # Configure pie outline colors
    scale_colour_manual(name = "Ship",
                        labels = c("LBC", "LM"),
                        values = c("LBC" = "black", "LM" = "gray")) +
...
ggsave(set.pie.wt,
       filename = here("Figs/fig_seine_proportion_set_wt.png"),
...
set.lengths.all <- set.lengths.all %>% 
  ...
  mutate(sex = case_when(
    !sex %in% c("male","female","unknown") ~ "missing",
    TRUE ~ sex)) %>% 
...
```{r missing-lengths}
# Select specimens with missing lengths
set.lengths %>% 
...
```{r missing-weights}
# Select specimens with missing weights
set.lengths %>% 
...
lw.plot.leg <- ggplot() +
  ...
  geom_line_interactive(data = lw.df.sub, aes(length_mm, weightg, 
                        ...
                        alpha = 0.75, colour = "gray20", linetype = 'dashed') +
  # Plot L/W data for current survey
  geom_point_interactive(data = set.lengths,
  ...
  # Plot individuals with missing lengths and weights
  geom_point_interactive(data = filter(set.lengths, missing.length == TRUE),
                         aes(totalLength_mm, weightg,
                             data_id = label, tooltip = label),
                         shape = 21, fill = 'red',  size = 2.5) +
  geom_point_interactive(data = filter(set.lengths, missing.weight == TRUE),
                         aes(totalLength_mm, weightg,
                             data_id = label, tooltip = label),
...
```{r map-hauls,out.height="10in",out.width="100%"}
# Set padding around data
imap.bounds <- map_bounds(nav.ns.points$lat, nav.ns.points$long, 0.1)
...
if (exists("set.catch.sf")) {
  i.map <- leaflet() %>% 
    ...
    addPolygons(data = wc_mpas, color = "#000414", weight = 2, fillColor =  ~mpaPalWC(Design),
                fillOpacity = 0.3, label = ~htmlEscape(Site_Nm), 
                group = "State MPAs") %>%
    ...
    addLayersControl(
      ...
      overlayGroups = c("U.S. MPAs","State MPAs", "State Waters", 
  ...
  if (exists("nav.ns"))
    i.map <- i.map %>% 
      addPolylines(data = nav.ns, color = "#000414", stroke = TRUE, weight = 1,
  ...
  if (exists("set.catch.sf") & nrow(filter(set.catch.sf, AllCPS == 0)) > 0)
    i.map <- i.map %>% 
      addCircleMarkers(data = filter(set.catch.sf, AllCPS == 0), 
                       radius = 5, color = "#000000", stroke = TRUE, weight = 2,
...
Map showing the portion of each species in purse seine catches, by weight.

```{r catch-proportions}
include_graphics(here("Figs/fig_seine_proportion_set_wt.png"))
```
