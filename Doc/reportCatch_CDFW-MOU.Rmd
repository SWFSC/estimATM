...
# Extract catch containing protected species
  ...
  left_join(spp.codes) %>% 
  left_join(select(haul.all, haul, netInWaterTime, 
...
lengths.psit <- lengths.all %>% 
  left_join(spp.codes) %>% 
  left_join(select(haul.all, haul, netInWaterTime, equilibriumTime, 
                   lat = startLatDecimal, long = startLongDecimal)) %>% 
  filter(species %in% protected.spp$species) %>% 
  select(cruise,ship,haul,collection,species,scientificName,commonName,specimenNumber,
         netInWaterTime,equilibriumTime,lat,long,individual_ID,
         forkLength_mm,weightg,notes,isAlive,
