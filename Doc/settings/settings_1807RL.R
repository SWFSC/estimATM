...
leg.breaks <- as.numeric(date(c("2018-06-24", "2018-07-20", 
                                      "2018-08-13", "2018-09-05",
                                      "2018-09-24")))

# Define ERDDAP data variables
erddap.vessel        <- "WTEG"    # Lasker == WTEG; Shimada == WTED; add "nrt" if during survey
erddap.survey.start  <- "2018-06-26" # Start of survey for ERDDAP vessel data query
erddap.survey.end    <- "2018-09-23" # End of survey for ERDDAP vessel data query
...
# Survey plan info --------------------------------------------------------
wpt.filename         <- "waypoints_1807RL.csv"
...
survey.start.sd  <- "2018-06-24" # Start of Saildrone survey
survey.end.sd    <- "2018-10-15" # End of Saildrone survey

# Set date range
erddap.url.sd <- "https://ferret.pmel.noaa.gov/pmel/erddap/tabledap/saildrone_west_coast_survey_2018"
erddap.survey.start.sd <- "2018-06-24T00%3A00%3A00Z"
erddap.survey.end.sd   <- "2018-10-15T19%3A59%3A00Z"
...
if (Sys.info()['nodename'] == "SWC-KSTIERHOF-D") {
  survey.dir           <- c(RL  = "//swc-storage3-s/AST3/SURVEYS/20180626_LASKER_SummerCPS",
                            SD  = "//swc-storage3-s/AST3/SURVEYS/20180626_LASKER_SummerCPS")   
} else {
  survey.dir           <- c(RL  = "//swc-storage3-s/AST3/SURVEYS/20180626_LASKER_SummerCPS",
                            SD  = "//swc-storage3-s/AST3/SURVEYS/20180626_LASKER_SummerCPS")
}
...
nasc.pattern.nearshore <- c(RL  = "Nearshore",
                            SD  = "\\d{3}N")
# Regex pattern for identifying offshore transects
nasc.pattern.offshore  <- c(RL  = "\\d{3}M",
                            SD  = "\\d{3}O")
# Regex pattern for identifying offshore transects
...
data.cps.nasc          <- c(RL  = here("Data/Backscatter/nasc_cps_RL_1807RL.csv"),
                            SD  = NA,
...
tx.rm                  <- list(RL = c("090", "595", "900", "EndLeg2", "Leg 3 transit to"),
                               SD  = NA)
