...
pacman::p_load(tidyverse,lubridate,here,sf,knitr,fs,leaflet,leaflet.extras,
               bookdown,leafem,mapview,htmltools,rnaturalearth,stplanr,ggrepel,
               kableExtra,tmaptools,ggiraph,gdtools)
...
# Define nav source
if(vessel.name %in% c("LBC","LM")) nav.source <- "GPX"
```

Specified files are processed and plotted at any time, but ideally prior to being processed using plotSurvey, estimateBiomass, etc. Below are specifics about the vessel and settings used:

Vessel: **`r vessel.name`**  
Process CSV: **`r process.csv`**  
Get nav: **`r get.nav`**  
Nav source: **`r nav.source`**  

```{r list-files}
if (process.csv) {
  # List full file paths
  csv.files <- c(dir_ls(file.path(survey.dir[vessel.name], nasc.dir[vessel.name]),
                        regexp = nasc.pattern.cps[vessel.name]))  
}
```
...
if (process.csv) {
  ...
  for (i in 1:length(csv.files)) {
    # Process i-th file
    nasc <- bind_rows(nasc, extract_csv(csv.files[i]), Sv.max = Sv.max)
...
nasc.sf <- nasc %>%
  select(filename, transect, transect.orig, int, dist_m, datetime, lat, long, cps.nasc) %>% 
  group_by(filename, transect, transect.orig, int) %>% 
  summarise(
    lat   = lat[1],
    long  = long[1],
    NASC  = mean(cps.nasc),
...
big.nasc <- nasc %>%
  ...
  mutate(NASC  = NASC/19,
         rank  = seq(n()),
         label = paste0('Transect: ', transect.orig, 
                        ' - Distance: ', round(dist_m), " m"),
         popup = paste0('<b>Transect: </b>', transect.orig, '<br/>', 
                        '<b>Time: </b>', min(datetime), "-", max(datetime), ' UTC<br/>',
...
} else if (nav.source == "GPX") {
  # Extract tracklines from GPX file
  nav.paths.sf <- read_GPX(here("Data/Nav", seine.gpx.name))$tracks
  
  # Extract trackline points from GPX file
  nav.points.sf <- read_GPX(here("Data/Nav", seine.gpx.name))$track_points %>% 
    project_sf(crs = 4326) %>% 
...
if (exists("nav.paths.sf"))
  i.map <- i.map %>% 
  addPolylines(data = nav.paths.sf, color = "magenta", stroke = TRUE, weight = 2,
               opacity = 0.5, fillOpacity = 0.5,
               group =  "Vessel Paths")

# Add backscatter data
i.map <- i.map %>% 
  addPolylines(data = nasc.paths, color = "#000414", weight = 8, 
