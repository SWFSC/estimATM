...
get.nav.sd         <- T # Download nav data from ERDDAP
copy.files         <- F # Copy data files from data to plotBio directory
overwrite.files    <- T # Overwrite existing files when copying (not CSV, see below)copy.files
...
if (sampling.trawl) {
  ...
  haul.pie <- haul.summ.wt %>% 
    select(haul, long, lat, Anchovy, JackMack, Jacksmelt, Other,
           PacHerring, PacMack, RndHerring, Sardine, AllCPS) %>%
  ...
  cluster.pie <- cluster.summ.wt %>% 
    select(cluster, long, lat, Anchovy, JackMack, Jacksmelt, Other,
           PacHerring, PacMack, RndHerring, Sardine, AllCPS) %>% 
    # Create bins for defining point size in NASC plots
    mutate(bin       = cut(AllCPS, trawl.breaks, include.lowest = TRUE),
           bin.level =  as.numeric(bin)) %>% 
  ...
  # Add missing sample.type variable
  if(is.null(haul.pie$sample.type)) haul.pie$sample.type <- "Trawl"
  if(is.null(cluster.pie$sample.type)) cluster.pie$sample.type <- "Trawl"
...
lm.transects <- transects.sf %>% 
  filter(Type %in% c("Compulsory", "Adaptive"), 
         Transect %in% c(seq(114,128,2), seq(131,153,2), seq(155,178,2)))

lm.transects <- transects.sf %>% 
  filter(Type == "Compulsory", Transect > 130) %>% 
  bind_rows(filter(transects.sf, Type == "Saildrone", between(Transect, 114, 128))) %>% 
  bind_rows(filter(transects.sf, Type == "Adaptive",  between(Transect, 114, 128)))

lm.map <- base.map + 
  geom_sf(data = lm.transects, colour = "gray50", linetype = "dotted") +
  geom_sf(data = filter(lm.transects, Transect < 136), colour = lm.colour) +
  geom_sf(data = lm.path, colour = lm.colour) +
  geom_sf(data = lm.sets, shape = 24, fill = "white") +
...
# Save vessel map
ggsave(lm.map,
       filename = here("Figs/fig_survey_update_2207RL-LM.png"),
       width = map.width, height = map.height) 
```

```{r add-lbc-data}
lbc.transects <- transects.sf %>% 
  filter(Type == "Nearshore", between(Transect, 138, 227) | Transect > 352)

# Import LM sets
lbc.sets <- read_csv(here("Data/Seine/lbc_sets_20220805.csv")) %>% 
  st_as_sf(coords = c("long","lat"), crs = 4326)

lbc.path <- lbc.transects %>% 
  filter(between(Transect, 186,227))

lbc.map <- base.map + 
  geom_sf(data = lbc.transects, colour = "gray50") +
  geom_sf(data = lbc.path, colour = lbc.colour) +
  geom_sf(data = lbc.sets, shape = 23, fill = "white") +
  # Format axes and titles
  coord_sf(crs = crs.proj, # CA Albers Equal Area Projection
           xlim = unname(c(map.bounds["xmin"], map.bounds["xmax"])), 
           ylim = unname(c(map.bounds["ymin"], map.bounds["ymax"])))
...
# Save vessel map
ggsave(lbc.map,
       filename = here("Figs/fig_survey_update_2207RL-LBC.png"),
       width = map.width, height = map.height) 
```

```{r add-sd-data}
sd.transects <- transects.sf %>% 
  filter(Type == "Saildrone")

sd.map <- base.map + 
  geom_sf(data = sd.transects, colour = "gray50", linetype = "dotted") +
  geom_sf(data = nav.sd.paths.sf, aes(colour = factor(saildrone)), show.legend = FALSE) +
  scale_colour_manual(values = c("1076" = sd.1076.colour, "1077" = sd.1077.colour)) +
  # Format axes and titles
  coord_sf(crs = crs.proj, # CA Albers Equal Area Projection
           xlim = unname(c(map.bounds["xmin"], map.bounds["xmax"])), 
           ylim = unname(c(map.bounds["ymin"], map.bounds["ymax"])))
...
# Save vessel map
ggsave(sd.map,
       filename = here("Figs/fig_survey_update_2207RL-SD.png"),
       width = map.width, height = map.height) 
```
...
```{r update-map-composite}
composite.map <- base.map + 
  # Add planned transects
  geom_sf(data = transects.sf, colour = "gray50", linetype = "dotted") +
  # Add Saildrone data
  geom_sf(data = nav.sd.paths.sf, aes(colour = factor(saildrone)), show.legend = FALSE) +
  scale_colour_manual(values = c("1076" = sd.1076.colour, "1077" = sd.1077.colour)) +
  # Add RL data
  geom_sf(data = nav.paths.sf, colour = rl.colour) +
  # Add LM data
  geom_sf(data = filter(lm.transects, Transect < 136), colour = lm.colour) +
  geom_sf(data = lm.path, colour = lm.colour) +
  # Add LBC data
  ...
  geom_sf(data = lbc.sets, shape = 23, fill = "white") +
  # Add sets and hauls
  geom_sf(data = haul.locs.sf, shape = 21, fill = "white") +
  geom_sf(data = lm.sets, shape = 24, fill = "white") +
  # Format axes and titles
  coord_sf(crs = crs.proj, # CA Albers Equal Area Projection
           xlim = unname(c(map.bounds["xmin"], map.bounds["xmax"])), 
...
# Save composite map
ggsave(composite.map,
       filename = here("Figs/fig_survey_update_2207RL.png"),
       width = map.width, height = map.height) 
```
