---
title: "plotSurvey: Survey Data Visualizer"
output:
...
pacman::p_load(tidyverse,cowplot,DBI,odbc,RSQLite,scatterpie,ggspatial,vroom,
               readxl,lubridate,rnaturalearth,sf,here,naniar,mapview,xts,terra,
               shadowtext,knitr,ggrepel,fs,leaflet,leaflet.extras,bookdown,
               htmltools,leafem,DT,rworldmap,rworldxtra,lwgeom,htmlwidgets,httr,
               ncdf4,rerddap,ggiraph,gdtools,xml2,kableExtra,gdtools)
...
get.db             <- ifelse(Sys.info()['nodename'] %in% c("SWC-KSTIERHOF-L"), F, T) 
...
process.csv.krill  <- F # Process krill backscatter files
...
# Get environmental layers from ERDDAP (e.g., sardine potential habitat, Chl a, etc.)
get.env            <- F

# Control script behavior (often F)
scale.pies         <- T # Scale catch pies to by total weight
...
if (copy.files) {
  ...
  dir_create(here("Data", c("Backscatter","CUFES","SCS","Trawl")))
...
nav.gps <- nav %>% 
  ...
  dplyr::select(GPS_date, GPS_time, latitude = lat, longitude = long)
...
```{r process-midwater-trawls}
# Format midwater trawl locations
if (survey.name %in% c("2506SH")) {
  haul.midwater <- read_csv(here("Data/Trawl/haul_info_midwater.csv")) %>% 
    mutate(time.start = mdy_hms(eq_date_time),
           time.end = mdy_hms(hb_date_time),
           perf.code = case_when(
             performance >= 0 ~ "Good",
             TRUE ~ "Aborted"))
  
  # Add lat/long data if missing
  if (!"lat" %in% names(haul.midwater)) {
    # Subset nav columns
    nav.match <- select(nav, time, long, lat)
    
    # Data frame for saving matched data
    nav.match.haul.midwater <- data.frame()
    
    # Match haul times to nav data and extract positions  
    for (i in unique(haul.midwater$haul)) {
      min.diff.eq       <- which.min(abs(difftime(haul.midwater$time.start[i],
                                                  nav$time))) 
      nav.match.df <- nav.match[min.diff.eq, ] %>% 
        mutate(haul = i)
      
      nav.match.haul.midwater <- bind_rows(nav.match.haul.midwater, nav.match.df)
    }
    
    # Add lat/long info to haul.midwater
    haul.midwater <- left_join(haul.midwater, nav.match.haul.midwater)
  }
  
  haul.midwater.sf <- haul.midwater %>% 
    filter(!is.na(lat) & !is.na(long)) %>% 
    st_as_sf(coords = c("long","lat"), crs = crs.geog) %>% 
    mutate(
      label = paste("Midwater trawl:", haul),
      popup = paste('<b>Midwater trawl:', haul, '</b><br/>',
                    'Performance:', performance, '<br/>',
                    'Status:', perf.code, '<br/>',
                    'Equlibrium:', eq_date_time, '<br/>',
                    'Haul back:', hb_date_time, '<br/>'))
  
  # ggplot(haul.midwater, aes(long, lat, fill = perf.code)) + geom_point(shape = 21) + coord_map()
  
}
```


```{r process-ctd-data, message=FALSE, include=FALSE}
...
if (sampling.trawl) {
  ...
  if (nrow(catch) > 0) {
    ...
    haul.summ.wt <- haul.summ.wt %>%  
      ...
      rename("Jacksmelt"  = "Atherinopsis californiensis",
             ...
             "JapSardine" = "Sardinops melanosticta",
  ...
  save(haul.pie, cluster.pie, haul.pos, cluster.pos, haul.zero, cluster.zero,
       file = here("Output/trawl_pie_plotSurvey.Rdata"))  
}
```
  ...
  if (process.csv) {
    ...
    if (length(csv.files.cps) > 0) {
      ...
      for (i in 1:length(csv.files.cps)) {
        # Process i-th file
        nasc.cps <- bind_rows(nasc.cps, 
                              extract_csv(csv.files.cps[i]), Sv.max = Sv.max)
  ...
  if (process.csv.krill) {
    if (process.csv) {
      ...
      if (length(csv.files.krill) > 0) {
        ...
        for (i in 1:length(csv.files.krill)) {
          # Process i-th file
          nasc.krill <- bind_rows(nasc.krill, 
                                  extract_csv(csv.files.krill[i], Sv.max = Sv.max))
    ...
    nasc.krill.sf <- nasc.krill %>%
      ...
      summarise(
        ...
        popup = paste0('<b>Transect: </b>', transect[1], '<br/>',
                       '<b>Time: </b>', min(datetime), " - ", max(datetime), ' UTC<br/>',
                       '<b>Latitude: </b>', round(lat,4), ", ", 
                       '<b>Longitude: </b>', round(long,4), '<br/>',
                       '<b>Distance: </b>', round(min(dist_m)), "-", round(max(dist_m)), ' m<br/>',
                       '<b>NASC: </b>', round(mean(NASC)), ' m<sup>2</sup> nmi<sup>-2</sup>')) %>%
      # Create bins for defining point size in NASC plots%>% 
      mutate(bin       = cut(NASC, nasc.breaks, include.lowest = TRUE),
...
if (exists("nasc.all")) {
  ...
  write_csv(big.nasc, file = here("Output/nasc_outliers.csv"))
}
```
...
if (save.figs) {
  ...
  if (sampling.acoustics & sampling.trawl) {
    nasc.haul.wt <- plot_grid(nasc.map.cps, trawl.pie.haul.wt, 
                              nrow = 1, labels = c("a)", "b)")) 
    
    nasc.cluster.wt <- plot_grid(nasc.map.cps, trawl.pie.cluster.wt, 
                                 nrow = 1, labels = c("a)", "b)")) 
    ...
    ggsave(nasc.cluster.wt,
           filename = here("Figs/fig_nasc_cluster_wt.png"),
           width = map.width*2, height = map.height) 
  }
} 
```
...
if (sampling.cufes) {
  ...
  cufes.hake.sf <- cufes %>% 
    filter(Density > 0, Species == "HakeEggs") %>%
    mutate(
      spp.label = case_when(
        Species == "HakeEggs" ~ "Hake",
        TRUE ~ Species),
      label = paste(spp.label, " eggs: ", round(Density, 1), " /cubic m", sep = ""),
      popup = paste('<b>Species:', spp.label, '</b><br/>', round(Density, 1),"eggs m<sup>3</sup>")) %>% 
    st_as_sf(coords = c("long","lat"), crs = crs.geog)
...
```{r get-raster-erddap,include=F}
if (get.env) {
  # Get and process environmental data from ERDDAP
  source(here("Code/get_env_erddap.R"))
} else {
  # Look for existing data and load if present
  if (file.exists(here("Data/Raster/hab_data_erddap.Rdata"))) {
    load(here("Data/Raster/hab_data_erddap.Rdata"))
  }
  
  if (file.exists(here("Data/Raster/chl_data_erddap.Rdata"))) {
    load(here("Data/Raster/chl_data_erddap.Rdata"))
  }
}
```
...
# Create color palette for sardine habitat
sarHabPal <- colorFactor(sar.hab.colors, 
                         domain = c("3", "5", "7"),
...
i.map <- leaflet() %>% 
  ...
  # Add layer controls
  addLayersControl(
    baseGroups = c("Esri.OceanBasemap","Esri.NatGeoWorldMap", 
    ...
    overlayGroups = c("Canada MPAs","Mexico MPAs","U.S. MPAs","State MPAs",
                      ...
                      "Sardine habitat-Probability", "Sardine habitat-Classification",
                      "Chlorophyll",
                      ...
                      "Backscatter (Large)",
                      "CTD Casts",
                      "CUFES Egg Density", "CUFES Egg Density-Hake",
                      "CUFES Egg Density-Squid", "CUFES Egg Density-Other",
                      "CUFES (Negative)",
...
if (sampling.cufes) {
  ...
  if (nrow(cufes.neg.sf) > 0)
    i.map <- i.map %>% 
      addCircleMarkers(data = cufes.neg.sf, radius = ~bin.level*2, color = "#000414", 
  ...
  if (nrow(cufes.hake.sf) > 0)
    i.map <- i.map %>% 
      addCircleMarkers(data = cufes.hake.sf,
                       radius = ~bin.level*2, color = "#000414", stroke = TRUE, weight = 1,
                       fillOpacity = 0.75, fillColor =  "#FFFFFF", label = ~label,
                       popup = ~popup, group = "CUFES Egg Density-Hake")
...
if (sampling.trawl) {
  ...
  if (exists("haul.midwater"))
    i.map <- i.map %>% 
      # Add trawl catch
      addCircleMarkers(data = haul.midwater.sf, radius = 5, color = "#000000", stroke = TRUE, weight = 2,
                       opacity = 0.8, fillOpacity = 1, fillColor =  "blue",
                       popup = ~popup, label = ~label,
                       group =  "Trawls")  
}
...
if (exists("uctd.sf"))
  i.map <- i.map %>% 
  # Add trawl catch
  addCircleMarkers(data = uctd.sf, radius = 5, color = "#000000", stroke = TRUE, weight = 2,
                   opacity = 0.8, fillOpacity = 1, fillColor =  "blue",
...
if (exists("ctd.sf"))
  i.map <- i.map %>% 
  # Add trawl catch
  addCircleMarkers(data = ctd.sf, radius = 5, color = "#000000", stroke = TRUE, weight = 2,
                   opacity = 0.8, fillOpacity = 1, fillColor =  "green",
                   popup = ~label, label = ~label,
                   group =  "CTD Casts")  

# Add raster habitat layers
# ## Potential sardine habitat
# if (exists("sar.hab"))
#   i.map <- i.map %>% addTiles() %>%
#   addRasterImage(sar.hab, colors = sar.pal, opacity = 0.5,
#                  group = "Sardine habitat-Probability") %>%
#   leaflet::addLegend(pal = sar.pal, values = values(sar.hab),
#             title = "Sardine habitat-Probability",
#             group = "Sardine habitat-Probability")
# 
# if (exists("sar.hab.class"))
#   i.map <- i.map %>% addTiles() %>%
#   addRasterImage(sar.hab.class, colors = sarHabPal, opacity = 0.5,
#                  group = "Sardine habitat-Classification") %>%
#   leaflet::addLegend(position = "bottomleft", 
#             pal = sarHabPal, 
#             values = values(sar.hab.class),
#             title = "Sardine habitat-Classification",
#             group = "Sardine habitat-Classification")

## Chlorophyll a
if (exists("chl.a.log"))
  i.map <- i.map %>% addTiles() %>%
  addRasterImage(chl.a.log, colors = chl.pal.log, opacity = 0.75,
                 group = "Chlorophyll") %>%
  leaflet::addLegend(
    position = "bottomleft",
    pal = chl.pal.log,
    values = values(chl.a.log),
...
if (exists("nav.sd.paths.sf"))
  i.map <- i.map %>% 
  addPolylines(data = nav.sd.paths.sf, color = c("#000414"), weight = 2,
...
if (exists("nav.now.sh"))
  i.map <- i.map %>% 
  addMarkers(data = nav.now.sh, label = ~label, popup = ~popup, 
             group = "Vessel Positions")
...
i.map <- i.map %>%  
  hideGroup(c("Backscatter (Large)", "Backscatter-Krill", "Backscatter-Krill (Small)", 
              ...
              "CUFES Egg Density-Squid", "CUFES Egg Density-Hake", "CUFES (Negative)",
              "Sardine habitat-Probability", "Sardine habitat-Classification", 
              "Chlorophyll", 
              "CTD Casts","Graticule",
              "Saildrone Tracks", "Saildrone Positions",
              "State Waters", "Exclusive Economic Zones",
              "Canada MPAs","Mexico MPAs"))
...
if (save.imap) {
  ...
  # Save i.map  
  mapshot(i.map, url = here("Output/imap/plotSurvey_explorer.HTML"))
}
```
...
(ref:nasc-outliers) An interactive plot of the top 100 backscatter ($s_A$) intervals (i.e., potential outliers/errors), labeled with the transect name and interval to facilitate further scrutiny of processing steps in Echoview.
...
## Provide acoustic transect breaks

Provide transect start and end times, which are helpful for populating Echoview file sets, etc. You may download the CSV or Excel file using the buttons below. 

```{r get-transect-breaks}
# Create directory for storing output
fs::dir_create(hakeML.dir)

# Summarise transect segments by start and end times
nasc.tx.summ <- nasc.cps %>%
  rename(transectPart = transect) %>% 
  mutate(transectNum = str_extract(transectPart, ("\\d{3}"))) %>% 
  group_by(transectNum, transectPart) %>% 
  summarise(transectStart = min(datetime),
            transectEnd = max(datetime)) %>% 
  arrange(desc(transectStart))

# Write to CSV
write_csv(nasc.tx.summ, here("Output/ek80_raw_summary.csv"))

# Save for hakeML
write_csv(nasc.tx.summ, 
          file = file.path(hakeML.dir, "transect_start_end_times.csv"))

# Create a searchable HTML table with a CSV download button
datatable(
  nasc.tx.summ,
  rownames = FALSE,
  extensions = 'Buttons', 
...
xfun::embed_file(here("Output/ek80_raw_summary.csv"), text = "Download the original CSV data")
```
...
if (exists("haul.bad")) {
  haul.bad %>% 
    ...
    select(haul, trawlDate, trawlPerformance) %>% 
    datatable(rownames = FALSE)
} else {
  print("No trawl data yet.")
}
```
...
(ref:backscatter-map-cps) The distribution of 38-kHz integrated backscattering coefficients (s_A, m^2^ nmi^-2^;  averaged over 2000-m distance intervals) ascribed to CPS.
...
(ref:backscatter-map-krill) The distribution of 200-kHz integrated backscattering coefficients (s_A, m^2^ nmi^-2^;  averaged over 2000-m distance intervals) ascribed to krill.
...
if (file.exists(here("Figs/fig_trawl_proportion_haul_wt.png"))) {
  ...
  print("No trawl haul proportion data yet.")
}
```

## By cluster

(ref:trawl-proportion-cluster-wt) Proportions of CPS species in trawl clusters, by weight (black points indicate trawl clusters with no CPS). 

```{r plot-trawl-proportion-cluster-wt,fig.cap='(ref:trawl-proportion-cluster-wt)',out.width='100%'}
if (file.exists(here("Figs/fig_trawl_proportion_cluster_wt.png"))) {
  include_graphics(here("Figs/fig_trawl_proportion_cluster_wt.png"))
} else {
  print("No trawl cluster proportion data yet.")
}
```
...
(ref:cufes-egg-density) Map of CUFES egg density (eggs m^-3^) for Northern Anchovy, Pacific Mackerel, and Jack Mackerel.
...
# Backscatter and trawl species proportions  

(ref:nasc-trawl-haul) A map of the a) distribution of 38-kHz integrated backscattering coefficients (s_A, m^2^ nmi^-2^;  averaged over 2000-m distance intervals and from 5 to 70-m deep) ascribed to CPS, and b) proportions of CPS species (by weight) in trawl hauls (black points indicate trawl hauls with no CPS).

```{r plot-nasc-trawl-proportion-haul-wt,fig.cap='(ref:nasc-trawl-haul)'}
if (file.exists(here("Figs/fig_nasc_haul_wt.png"))) {
  include_graphics(here("Figs/fig_nasc_haul_wt.png"))
} else {
  print("No trawl haul proportion data yet.")
}
```

(ref:nasc-trawl-cluster) A map of the a) distribution of 38-kHz integrated backscattering coefficients (s_A, m^2^ nmi^-2^;  averaged over 2000-m distance intervals and from 5 to 70-m deep) ascribed to CPS, and b) proportions of CPS species (by weight) in trawl clusters (black points indicate trawl clusters with no CPS).

```{r plot-nasc-trawl-proportion-cluster-wt,fig.cap='(ref:nasc-trawl-cluster)',out.width='100%'}
if (file.exists(here("Figs/fig_nasc_cluster_wt.png"))) {
  include_graphics(here("Figs/fig_nasc_cluster_wt.png"))
} else {
  print("No trawl cluster proportion data yet.")
}
```
...
(ref:nasc-cufes-trawl) A map of the a) distribution of 38-kHz integrated backscattering coefficients ($s_A$, m^2^ nmi^-2^;  averaged over 2000-m distance intervals) ascribed to CPS; b) CUFES egg density (eggs m^-3^) for Northern Anchovy, Pacific Sardine, and Jack Mackerel; and c) proportions of CPS species (by weight) in trawl hauls (black points indicate trawl hauls with no CPS).
...
if (file.exists(here("Figs/fig_nasc_cufes_haul_wt.png"))) {
  ...
  print("No backscatter/CUFES/haul proportion data yet.")
}
```

# Map station sampling data

A map showing the vessel track (gray lines), acoustic transects (black lines), and the locations of nighttime surface trawls (red), daytime midwater trawls (white), underway CTD (UCTD) casts (blue), and CTD casts (green, if present).  

```{r map-station-data}
if (exists("uctd.summ")|exists("ctd.summ")) {
  if (save.figs) {
    # Create Saildrone map
    sample.map <- base.map +
      geom_sf(data = nav.paths.sf, colour = "gray70") +
      coord_sf(crs = crs.proj, # CA Albers Equal Area Projection
               xlim = c(map.bounds["xmin"], map.bounds["xmax"]), 
               ylim = c(map.bounds["ymin"], map.bounds["ymax"]))
    
    if (exists("nasc.plot.cps")) {
      # Add UCTD casts to map
      sample.map <- sample.map +
        geom_point(data = nasc.plot.cps, aes(X, Y), colour = "black", size = 0.01) +
        coord_sf(crs = crs.proj, # CA Albers Equal Area Projection
                 xlim = c(map.bounds["xmin"], map.bounds["xmax"]), 
                 ylim = c(map.bounds["ymin"], map.bounds["ymax"]))
    }
    ...
    if (exists("uctd.summ")) {
      # Add UCTD casts to map
      sample.map <- sample.map +
        geom_point(data = uctd.summ, aes(X, Y), shape = 21, fill = "blue") +
        coord_sf(crs = crs.proj, # CA Albers Equal Area Projection
                 xlim = c(map.bounds["xmin"], map.bounds["xmax"]), 
                 ylim = c(map.bounds["ymin"], map.bounds["ymax"]))
    }
    ...
    if (exists("ctd.summ")) {
      # Add CTD casts to map
      sample.map <- sample.map +
        geom_point(data = ctd.summ, aes(X, Y), shape = 21, fill = "green") +
        coord_sf(crs = crs.proj, # CA Albers Equal Area Projection
                 xlim = c(map.bounds["xmin"], map.bounds["xmax"]), 
                 ylim = c(map.bounds["ymin"], map.bounds["ymax"]))
    }
    ...
    if (exists("haul.mid")) {
      ...
      # Add surface trawls to map
      sample.map <- sample.map +
        geom_point(data = haul.plot, aes(X, Y), shape = 21, fill = "red") +
        coord_sf(crs = crs.proj, # CA Albers Equal Area Projection
                 xlim = c(map.bounds["xmin"], map.bounds["xmax"]), 
                 ylim = c(map.bounds["ymin"], map.bounds["ymax"]))
    }
    ...
    if (exists("haul.midwater")) {
      haul.mw.plot <- ungroup(haul.midwater) %>% 
        filter(!is.na(lat) & !is.na(long)) %>% 
        project_df(to = crs.proj)
      
      # Add surface trawls to map
      sample.map <- sample.map +
        geom_point(data = haul.mw.plot, aes(X, Y), shape = 24, fill = "white") +
        coord_sf(crs = crs.proj, # CA Albers Equal Area Projection
                 xlim = c(map.bounds["xmin"], map.bounds["xmax"]), 
                 ylim = c(map.bounds["ymin"], map.bounds["ymax"]))
    }
    ...
    # Save map
    ggsave(sample.map, file = here("Figs/fig_station_sample_map.png"),
           height = map.height, width = map.width)
  }
}

if (file.exists(here("Figs/fig_station_sample_map.png"))) {
  include_graphics(here("Figs/fig_station_sample_map.png"))
} else {
  print("No station sample data yet.")
}
```
