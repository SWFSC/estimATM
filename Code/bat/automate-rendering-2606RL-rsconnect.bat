echo --------------------------------------------------
  echo Sync data from Google Cloud Storage 
echo --------------------------------------------------
...
echo --------------------------------------------------
  echo Publishing plotSurvey Rmarkdown document to Connect 
echo --------------------------------------------------
  :: Name of plotSurvey Rmd
SET RMD_FILE="plotSurvey.Rmd"
%R_PATH% publish connect "%RMD_FILE%" --id "%plotSurvey_id%" --no-prompt --no-browser



%R_PATH% -e "rmarkdown::render('%RMD_FILE%', output_format='html_document')"
:: Publish document to Rstudio Connect

:: Open resulting file
start "" "%~dp0..\..\Doc\plotSurvey.html"

echo --------------------------------------------------
  echo Publishing checkTrawls Rmarkdown document to Connect 
echo --------------------------------------------------
...
echo --------------------------------------------------
  echo Publishing plotTrawls Rmarkdown document to Connect 
echo --------------------------------------------------
