...
# Install and load pacman and pak (library management packages)
if (!require("pacman")) install.packages("pacman")
if (!require("pak")) install.packages("pak")

# Install and load required packages from CRAN ---------------------------------
pacman::p_load(tidyverse,cowplot,knitr,shadowtext,sf,fs,vroom,leaflet,xml2,
               DBI,stringr,gdata,forecast,lubridate,here,rnaturalearth,viridis,
               DT,mapview,bookdown,ggspatial,leafem,leaflet.extras,xts,ggiraph,gdtools)
...
uctd.hdr <- dir_ls(here("Data/UCTD"), regexp = uctd.hdr.pattern, recurse = TRUE) %>% 
  path_filter(regexp = "_processed", invert = TRUE)

# Remove bad casts
if(exists("uctd.cast.rm")) {
  # Filter paths by matching against the file names
  uctd.hdr <- uctd.hdr[!basename(uctd.hdr) %in% uctd.cast.rm]  
}

# Process UCTD cast headers
if (length(uctd.hdr) == 0) {
  ...
  if (process.files) {
    ...
    if(exists("uctd.cast.rm")) {
      # Filter paths by matching against the file names
      uctd.proc <- uctd.proc[!basename(uctd.proc) %in% uctd.cast.rm]  
    }
  ...
  uctd.summ <- all.uctd.casts %>% 
    ...
    left_join(select(all.uctd.hdr, cast, cast.date, lag, lat, long, leg, SOG)) %>%
    arrange(cast.date) %>%
  ...
  all.uctd.casts <- all.uctd.casts %>% 
    ...
    left_join(select(all.uctd.hdr, cast, cast.date, lat, long))
  
  uctd.summ <- uctd.summ %>% 
    left_join(select(uctd.class, cast, class)) %>% 
    select(cast.num, cast, cast.date, lag, lat, long, SOG, 
  ...
  # Exclude surface soak times
  if (exists("uctd.surface.rm"))
    all.uctd.casts <- all.uctd.casts %>% 
        filter(s > uctd.surface.rm)
...
ctd.hdr <- dir_ls(here("Data/CTD"), regexp = ctd.hdr.pattern, recurse = TRUE) %>% 
  path_filter(regexp = "_processed", invert = TRUE)
...
if (length(ctd.hdr) == 0) {
  ...
  if (nrow(all.ctd.casts) > 0) {
    ...
    all.ctd.casts <- all.ctd.casts %>% 
      ...
      left_join(select(all.ctd.hdr, cast, cast.date, lat, long))
...
if (length(uctd.summ) > 0) {
  ...
  # rename columns
  names(uctd.table) <- c("Cast","Filename","Date/time","Lag (s)","Lat","Lon",
...
if (exists("uctd.summ")) {
  ...
  uctd.sf <- project_df(uctd.summ, 4326, 3310) %>% 
    st_as_sf(coords = c("long","lat"), crs = crs.geog) 
}

if (exists("uctd.missing")) {
  if (nrow(uctd.missing) > 0) {
    uctd.missing.sf <- project_df(uctd.missing, 4326, 3310) %>% 
      st_as_sf(coords = c("long","lat"), crs = crs.geog) 
  }
}
if (exists("all.ctd.hdr")) {
  ctd.sf  <- project_df(all.ctd.hdr, 4326, 3310) %>% 
    st_as_sf(coords = c("long","lat"), crs = crs.geog) 
}

if (exists("ctd.missing")) {
  if (nrow(ctd.missing) > 0) {
    ctd.missing.sf <- project_df(ctd.missing, 4326, 3310) %>% 
      st_as_sf(coords = c("long","lat"), crs = crs.geog) 
  }
}
```
    ...
    if (length(all.uctd.casts) > 0) {
      if (length(uctd.table) > 0) {
        ...
        map.uctd <- base.map + 
          ...
          geom_point(data = uctd.sf, aes(X, Y, colour = leg)) +
...
```{r i-map-uctd, out.width='100%'}
# Create color palette for planned transects
txPal  <- colorFactor(wpt.colors, wpt.types)
# Create color palette for transect legs
leg.colors <- topo.colors(n_distinct(uctd.sf$leg))
legPal <- colorFactor(leg.colors, unique(uctd.sf$leg))
...
i.map.uctd <- i.map %>%
  ...
  addLayersControl(
    ...
    overlayGroups = c("UCTD Casts-Unprocessed", "UCTD Casts", 
                      "Transects","Vessel Track"),
    options = layersControlOptions(collapsed = FALSE))

# Add UCTD cast data
if (exists("uctd.missing.sf")) {
  i.map.uctd <- i.map.uctd %>% 
    addCircleMarkers(data = uctd.missing.sf,
                     radius = 5, color = "red", stroke = TRUE, opacity = 0.5,
                     fillOpacity = 0.5, fillColor =  "red", 
                     label = ~cast,
                     popup = ~cast,
                     group = "UCTD Casts-Unprocessed")
}
...
if (exists("uctd.sf")) {
  i.map.uctd <- i.map.uctd %>% 
    addCircleMarkers(data = uctd.sf,
                     color = ~legPal(leg),
                     radius = 5, stroke = FALSE, opacity = 0.75,
                     fillOpacity = 0.75,
                     label = ~cast,
                     popup = ~cast,
                     group = "UCTD Casts") %>% 
    # Add legends
    leaflet::addLegend("bottomleft", colors = leg.colors, 
                       values = sort(unique(uctd.sf$leg)),
                       labels = sort(unique(uctd.sf$leg)), 
                       title = "Leg", 
                       opacity = 1,
                       group = "UCTD Casts")
}
...
```{r uctd-dZt-plot,fig.height=10,fig.width=10}
if (exists("all.uctd.casts")) {
  if (length(all.uctd.casts) > 0) {
    if (save.figs) {
      # plot descent rate v depth
      uctd.dZt <- ggplot(data = all.uctd.casts, 
                         aes(dZt, Z, group = cast, colour = SOG)) + 
        geom_path_interactive(alpha = 0.75, aes(tooltip = cast)) +
    ...
    if (do.ggiraph) {
      ...
      uctd.dZt.interactive <- ggiraph::girafe(
        ggobj = uctd.dZt,
        dependencies = list(liberationsansHtmlDependency())
      )
      
      # Print interactive plot
      uctd.dZt.interactive
    } else {
      uctd.dZt
    }
  }  
...
```{r uctd-depth-time-plot,fig.height=10,fig.width=10}
# ,out.height="8in"
if (exists("all.uctd.casts")) {
  if (length(all.uctd.casts) > 0) {
    if (save.figs) {
    ...
    if (do.ggiraph) {
      # Create interactive plot
      uctd.Z.interactive <- ggiraph::girafe(
        ggobj = uctd.Z,
        dependencies = list(liberationsansHtmlDependency())
      )
...
```{r uctd-temp-depth-plot,fig.height=10,fig.width=10}
if (exists("all.uctd.casts")) {
  if (length(all.uctd.casts) > 0) {
    if (save.figs) {
    ...
    if (do.ggiraph) {
      # Create interactive plot
      uctd.T.interactive <- ggiraph::girafe(
        ggobj = uctd.T,
        dependencies = list(liberationsansHtmlDependency())
      )
...
```{r uctd-sal-depth-plot,fig.height=10,fig.width=10}
if (exists("all.uctd.casts")) {
  if (length(all.uctd.casts) > 0) {
    if (save.figs) {
    ...
    if (do.ggiraph) {
      # Create interactive plot
      uctd.S.interactive <- ggiraph::girafe(
        ggobj = uctd.S,
        dependencies = list(liberationsansHtmlDependency())
      )
...
```{r uctd-sv-depth-plot,fig.height=10,fig.width=10}
if (exists("all.uctd.casts")) {
  if (length(all.uctd.casts) > 0) {
    if (save.figs) {
    ...
    if (do.ggiraph) {
      # Create interactive plot
      uctd.Sv.interactive <- ggiraph::girafe(
        ggobj = uctd.Sv,
        dependencies = list(liberationsansHtmlDependency())
      )
...
## Chlorophyll-a v. Depth plot

```{r uctd-chl-depth-plot,fig.height=10,fig.width=10}
if (exists("all.uctd.casts")) {
  if (length(all.uctd.casts) > 0) {
    if (has_name(all.uctd.casts, "ChlA")) {
      if (save.figs) {
        # plot sound speed v depth
        uctd.chl <- ggplot(data = all.uctd.casts, aes(ChlA, Z, group = cast, colour = factor(cast))) + 
          geom_path_interactive(alpha = 0.75, aes(tooltip = cast)) +
          xlab("\nChlorophyll-a (ug/L)") + ylab("Depth (m)\n") +
          # scale_x_continuous(limits = c(round(min(all.uctd.casts$ChlA), 0) - 1, 
          #                               round(max(all.uctd.casts$ChlA), 0) + 1),
          #                    breaks = seq(round(min(all.uctd.casts$ChlA),0) - 1, 
          #                                 round(max(all.uctd.casts$ChlA),0) + 1, 10),
          #                    expand = c(0,0)) +  
          scale_y_continuous(limits = c(round(min(all.uctd.casts$Z), digits = -1), 0),
                             breaks = seq(round(min(all.uctd.casts$Z), digits = -1), 0, 25),
                             expand = c(0,0)) + 
          facet_wrap(~leg, ncol = 2, scales = "free_x") +
          theme_bw() +
          theme(panel.grid.major = element_line(linewidth = 0.75), 
                legend.position  = "none",
                strip.background.x = element_blank(),
        ...
        # save plot
        ggsave(uctd.chl, file = here("Figs/fig_uctd_ChlA_v_Z.png"))
      }
      
      if (do.ggiraph) {
        # Create interactive plot
        uctd.chl.interactive <- ggiraph::girafe(
          ggobj = uctd.chl,
          dependencies = list(liberationsansHtmlDependency())
        )
        
        # Print interactive plot
        uctd.chl.interactive
      } else {
        uctd.chl
      }
    }  
...
```{r ctd-map-all, out.width='100%'}
if (save.figs) {
  if (exists("ctd.hdr")) {
    if (length(ctd.hdr) > 0) {
      map.ctd <- base.map + 
        geom_sf(data = transects.sf) + 
        geom_point(data = ctd.sf, aes(X, Y, colour = leg)) +
...
i.map.ctd <- i.map %>% 
  ...
  addLayersControl(
    baseGroups = c("Esri.NatGeoWorldMap", "Esri.OceanBasemap",
                   "Esri.WorldImagery","Esri.WorldStreetMap","OpenStreetMap.Mapnik"),
    overlayGroups = c("CTD Casts-Unprocessed", "CTD Casts", 
                      "Transects", "Vessel Track"),
    options = layersControlOptions(collapsed = FALSE))
...
if (exists("ctd.missing.sf")) {
  i.map.ctd <- i.map.ctd %>% 
    addCircleMarkers(data = ctd.missing.sf,
                     radius = 5, color = "red", stroke = TRUE, opacity = 0.5,
                     fillOpacity = 0.5, fillColor =  "red", 
                     label = ~cast,
                     popup = ~cast,
                     group = "CTD Casts-Unprocessed")
}
...
if (exists("ctd.sf")) {
  i.map.ctd <- i.map.ctd %>% 
    addCircleMarkers(data = ctd.sf,
                     color = ~legPal(leg),
                     radius = 5, stroke = FALSE, opacity = 0.5,
                     fillOpacity = 0.5, #fillColor =  "blue", color = "#000414", 
                     label = ~cast,
                     popup = ~cast,
                     group = "CTD Casts") %>% 
    # Add legends
    leaflet::addLegend("bottomleft", colors = leg.colors, 
                       values = sort(unique(ctd.sf$leg)),
                       labels = sort(unique(ctd.sf$leg)), 
                       title = "Leg", 
                       opacity = 1,
                       group = "CTD Casts")
}
...
```{r ctd-temp-depth-plot,fig.height=10,fig.width=10}
if (exists("all.ctd.casts")) {
  if (length(all.ctd.casts) > 0) {
    if (save.figs) {
      # plot temperature v depth
      ctd.T <- ggplot(data = all.ctd.casts, aes(T, Z, group = cast, colour = factor(cast))) + 
        geom_path_interactive(alpha = 0.75, aes(tooltip = cast.date)) +
        xlab("\nTemperature (C)") + ylab("Depth (m)\n") + 
    ...
    if (do.ggiraph) {
      # Create interactive plot
      ctd.T.interactive <- ggiraph::girafe(
        ggobj = ctd.T,
        dependencies = list(liberationsansHtmlDependency())
      )
...
```{r ctd-sal-depth-plot,fig.height=10,fig.width=10}
if (exists("all.ctd.casts")) {
  if (length(all.ctd.casts) > 0) {
    if (save.figs) {
      # plot salinity v depth
    ...
    if (do.ggiraph) {
      # Create interactive plot
      ctd.S.interactive <- ggiraph::girafe(
        ggobj = ctd.S,
        dependencies = list(liberationsansHtmlDependency())
      )
...
```{r ctd-sv-depth-plot,fig.height=10,fig.width=10}
if (exists("all.ctd.casts")) {
  if (length(all.ctd.casts) > 0) {
    if (save.figs) {
      # plot sound speed v depth
    ...
    if (do.ggiraph) {
      # Create interactive plot
      ctd.Sv.interactive <- ggiraph::girafe(
        ggobj = ctd.Sv,
        dependencies = list(liberationsansHtmlDependency())
      )
...
Locations of CTD (blue points) and UCTD (green points) locations.  

```{r i-map-all-ctds, out.width='100%'}
# Create color palette for planned transects
txPal    <- colorFactor(wpt.colors, wpt.types)
...
i.map.all.ctds <- i.map %>%
  ...
  addLayersControl(
    baseGroups = c("Esri.NatGeoWorldMap", "Esri.OceanBasemap",
                   "Esri.WorldImagery","Esri.WorldStreetMap","OpenStreetMap.Mapnik"),
    overlayGroups = c("UCTD Casts-Unprocessed", "UCTD Casts",
                      "CTD Casts-Unprocessed", "CTD Casts",
                      "Transects","Vessel Track","Vessel Positions"),
    options = layersControlOptions(collapsed = FALSE))

# Add UCTD cast data
if (exists("uctd.missing.sf")) {
  i.map.all.ctds <- i.map.all.ctds %>% 
    addCircleMarkers(data = uctd.missing.sf,
                     radius = 5, color = "red", stroke = TRUE, opacity = 0.5,
                     fillOpacity = 0.8, fillColor =  "green", 
                     label = ~cast,
                     popup = ~cast,
                     group = "UCTD Casts-Unprocessed")
}
...
if (exists("uctd.sf")) {
  i.map.all.ctds <- i.map.all.ctds %>% 
    addCircleMarkers(data = uctd.sf,
                     radius = 5, color = "#000414", stroke = FALSE, opacity = 0.5,
                     fillOpacity = 0.8, fillColor =  "green", 
                     label = ~cast,
                     popup = ~cast,
                     group = "UCTD Casts")
...
if (exists("ctd.missing.sf")) {
  i.map.all.ctds <- i.map.all.ctds %>% 
    addCircleMarkers(data = ctd.missing.sf,
                     radius = 5, color = "red", stroke = TRUE, opacity = 0.5,
                     fillOpacity = 0.8, fillColor =  "blue", 
                     label = ~cast,
                     popup = ~cast,
                     group = "CTD Casts-Unprocessed")
}
