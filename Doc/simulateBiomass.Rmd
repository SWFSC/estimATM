---
title: "Simulate CPS biomass from Acoustic-Trawl Surveys"
...
pacman::p_load(tidyverse,swfscMisc,reshape2,tcltk,lubridate,sp,
               ...
               rworldmap,rworldxtra,data.table,stplanr,ggspatial,gt)
...
if (is.null(doc.name)) {doc.name <- "simulateBiomass.Rmd"}

# Create output directories for simulation
dir_create(here("Simulation"), c("Data", "Figs", "Output"))
```
...
Options in this section control the execution of various code chunks. All chunks should be run (i.e., controls  <- `T`) the first time to copy all necessary data files, import data from databases, generate maps and figures, estimate abundance and biomass, etc. Subsequent processing may be sped-up by setting some controls to `F` if source data or analyses have not changed (e.g., if only changes to a figure have been made).

```{r processing-controls,echo=TRUE}
# Simulation filtering settings
keep.even <- T # Keep even (T) or odd (F) transects 

# Set processing loop controls (T/F) -------------------------------------------------------
# Run only selected loops, usually T
...
# Sometimes F
assign.clusters   <- F # Assign NASC to nearest trawl cluster
...
To more easily configure processing for different surveys and distribute updated code, a separate file used to configure survey-specific settings is created for each survey. The `settings*.R` files are stored in the `estimATM/Doc/settings` sub-directory. The settings file is defined automatically based on the name of the directory in which the .Rproj file is stored (in this case, `r here()`). For this analysis, **`r prj.settings`** was used.
...
ggsave(here("Simulation/Figs/fig_survey_plan.png"), survey.plan,
...
include_graphics(here("Simulation/Figs/fig_survey_plan.png"))
```

# Process backscatter data
## Read and summarize CSV files from Echoview  
For each .CSV file produced during Echoview processing, data within each `r bin.length`-m long interval are vertically integrated every `r bin.depth` m (down to 100 m, then at 150 and 250 m) for each vessel. If the CTD.app has been used to define the stop integration depth, it is added, else NASC down to 50 m depth (NASC.50) is used. Transect names are extracted from .CSV file names, short transects are excluded, and transect numbers are assigned.

# Process backscatter data
## Read and summarize CSV files from Echoview  
For each .CSV file produced during Echoview processing, data within each `r bin.length`-m long interval are vertically integrated every `r bin.depth` m (down to 100 m, then at 150 and 250 m) for each vessel. If the CTD.app has been used to define the stop integration depth, it is added, else NASC down to 70 m depth (NASC.70) is used. Transect names are extracted from .CSV file names, short transects are excluded, and transect numbers are assigned. Nearshore and offshore transects are processed but excluded from the primary analysis, but are available for further analysis if desired (e.g., `estimate.os = TRUE`, `estimate.ns = TRUE`, etc.).
...
# Down-sample backscatter data if simulating results
# Filter planned transects
transects.sim <- transects.sf %>% 
  mutate(transect.name = sprintf("RL %03d", Transect)) %>% 
  filter(Type %in% c("Compulsory","Adaptive")) %>% 
  filter(Transect < 109) 

nasc <- nasc %>% 
  filter(transect.name %in% transects.sim$transect.name)
...
# Save NASC summary
save(nasc.summ, file = here("Simulation/Output/nasc_summ_tx.Rdata"))
```
...
write.csv(nasc.plot, file = here("Simulation/Output/cps_nasc_plot.csv"), 
          row.names = F, quote = F)
...
if (save.figs) {
  ...
  ggsave(nasc.map.tx.name, 
         filename = here("Simulation/Figs/fig_nasc_summary_map_tx_name.png"), 
         width = map.width, height = map.height)
  
  ggsave(nasc.map.tx.num,  
         filename = here("Simulation/Figs/fig_nasc_summary_map_tx_num.png"),  
         width = map.width, height = map.height)
  ...
  ggsave(nasc.map.tx, filename = here("Simulation/Figs/fig_nasc_summary_map_tx.png"), 
...
# load(here("Simulation/Output/nasc_plot_cps.Rdata"))

include_graphics(here("Simulation/Figs/fig_nasc_summary_map_tx.png"))
```  
...
```{r import-trawl-data, include=F}
# Load trawl data
load(here("Data/Trawl/trawl_data.Rdata"))
```  

## Process trawl haul data  
Haul data are filtered, foramatted, and assigned to clusters, where a trawl cluster contains only trawls conducted on the same evening (i.e., the difference in equilibrium time between trawls in the same cluster is < 12 h).
...
# Filter trawl data used for simulation -----------------------------------------
# Get clusters to retain
if (keep.even) {
  keep.clusters <- seq(2, max(haul$cluster), by = 2)
} else {
  keep.clusters <- seq(1, max(haul$cluster), by = 2)
}

# Filter unwanted clusters
haul <- filter(haul, cluster %in% keep.clusters)
...
# Save haul data
save(haul, file = here("Simulation/Output/haul_info.Rdata"))
```  
...
haul %>% 
  ...
  select(-notes, -stopLatDecimal, -stopLongDecimal, -haulBackTime) %>% 
...
if (save.figs) {
  ...
  trawl.cluster.plot <- base.map +
    ...
    geom_shadowtext(
      data = cluster.mid, aes(X, Y, label = cluster),
      colour = "black", bg.colour = "white", size = 2) +
    # Plot panel label
  ...
  ggsave(trawl.cluster.plot,
         filename = here("Simulation/Figs/fig_trawl_cluster_map.png"),
         width = map.width, height = map.height)
}

include_graphics(here("Simulation/Figs/fig_trawl_cluster_map.png"))
```  
...
# Remove catch data with no corresponding hauls when hauls are down-sampled
catch <- filter(catch, !is.na(cluster))

save(catch, file = here("Simulation/Output/catch_final.Rdata"))
...
save(haul.summ.wt, cluster.summ.wt, haul.pie, cluster.pie, haul.zero, cluster.zero,
     file = here("Simulation/Output/catch_info.Rdata"))
```
...
if (assign.clusters) {
  ...
  cluster.distance <- data.frame(
    datetime = rep(NA_POSIXct_, nrow(nasc)),
    cluster = rep(NA, nrow(nasc)), 
    cluster.distance = rep(NA, nrow(nasc)))
  
  # Configure progress bar
  pb <- tkProgressBar("R Progress Bar", "Cluster Assignment", 0, 100, 0)
  
  # Assign trawl clusters
  for (i in 1:nrow(nasc)) {
    # Calculate distance between each NASC interval and all trawl clusters
    temp.distance <- distance(nasc$lat[i], nasc$long[i], 
                              super.clusters$lat, super.clusters$long, 
                              units = "nm")
    
    # Assign cluster with minimum distance to NASC interval
    cluster.distance$datetime[i]         <- nasc$datetime[i]
    cluster.distance$cluster[i]          <- super.clusters$cluster[which.min(temp.distance)]
    cluster.distance$cluster.distance[i] <- temp.distance[which.min(temp.distance)]
    
    # Update progress bar
    pb.prog <- round(i/nrow(nasc)*100)
  ...
  save(cluster.distance, file = here("Simulation/Output/nasc_cluster_distance.Rdata"))
  
} else {
  load(here("Simulation/Output/nasc_cluster_distance.Rdata"))  
  
}

# Add cluster distances to nasc
nasc <- left_join(nasc, cluster.distance)
```

## Map trawl cluster polygons around acoustic backscatter  
...
nasc.super.clusters <- plyr::ddply(nasc, "cluster", find_hull) %>% 
...
if (save.figs) {
  ...
  nasc.cluster.plot <- base.map +
    ...
    # geom_point(data = nasc, aes(X, Y, colour = factor(cluster)),
    #            show.legend = FALSE) +
    # # Plot convex hull around NASC clusters
    # geom_sf(data = nasc.super.clusters, aes(fill = factor(cluster)),
  ...
  ggsave(nasc.cluster.plot,
         filename = here("Simulation/Figs/fig_nasc_cluster_map.png"),
         width = map.width, height = map.height)
}

include_graphics(here("Simulation/Figs/fig_nasc_cluster_map.png"))
```  
...
if (save.figs) {
  ...
  ggsave(nasc.trawl.cluster.wt,
         filename = here("Simulation/Figs/fig_nasc_trawl_cluster_wt.png"),
         width = map.width*2, height = map.height)
  
  save(trawl.pie.cluster.wt, trawl.pie.cluster.wt, nasc.trawl.cluster.wt,
       file = here("Simulation/Output/trawl_species_proportion_map.Rdata"))
  
} else {
  load(here("Simulation/Output/trawl_species_proportion_map.Rdata"))
}

include_graphics(here("Simulation/Figs/fig_nasc_trawl_cluster_wt.png"))
```  
...
# Get hauls to retain
keep.hauls <-  unique(haul$haul)

# Retain only lengths for kept hauls
lengths.all <- filter(lengths.all, haul %in% keep.hauls)
...
save(lengths.all, file = here("Simulation/Output/lengths_all.Rdata"))
...
save(lengths.all, lengths, lw.df,
     file = here("Simulation/Output/lengths_final.Rdata"))
...
ggsave(length.outlier.plot, 
       filename = here("Simulation/Figs/fig_length_outliers.png"),
       height = 5, width = 7)

# Write outliers to file
write_csv(length.outliers, here("Simulation/Output/specimen_outliers.csv"))

# Save final length table
write_csv(lengths, here("Simulation/Output/lengths_final.csv"))
...
if (nrow(lengths.binned) > 0) {
  ...
  ggsave(binned.length.plot,
         file = here("Simulation/Figs/fig_binned_length_histogram.png"),
         height = 6, width = 6)
}

# Save results
save(length.summ, file = here("Simulation/Output/length_summary_all.Rdata"))
```  
...
# Write to CSV
write.csv(catch.comp, file = here("Simulation/Output/catch_comparison.csv"), 
          quote = F, row.names = F)
...
if (nrow(catch.comp.errors) > 0) {
  write.csv(catch.comp.errors,file = here("Simulation/Output/catch_comparison_errors.csv"),quote = F,row.names = F)
}
```  
...
# Save output to CSV
if (nrow(catch.comp.errors) > 0) {
  write.csv(catch.comp.errors, file = here("Simulation/Output/catch_comparison_errors.csv"),
...
lw.plot <- ggplot() + 
  ...
  geom_point(data = filter(lengths.all, season == tolower(survey.season)), 
...
ggsave(lw.plot, filename = here("Simulation/Figs/fig_LW_plots.png"),
       width = 10, height = 7) 
...
# save length/weight plot
ggsave(lw.plot.leg, filename = here("Simulation/Figs/fig_LW_plots_leg.png"),
       width = 10, height = 7) 

# Add L/W plot
include_graphics(here("Simulation/Figs/fig_LW_plots.png"))
```

# Process CUFES data

## Import CUFES data  
During surveys, or for surveys for which CUFES data have not been verified and uploaded to SQL Server, data in SQLite format may be obtained by contacting [Dave Griffith](Dave.Griffith@NOAA.gov). To import from the SQL Server, a machine ODBC connection must be configured to the CUFES databases on the Estrella server (IP address: 161.55.235.187). To request access to the CUFES database, contact [Ed Weber](Ed.Weber@NOAA.gov). 
...
save(cufes.bad, file = here("Simulation/Output/cufes_bad.Rdata"))

# Write CUFES data from current survey to CSV
write.csv(cufes, file = here("Simulation/Output/cufes_data.csv"), 
...
if (save.figs) {
  ...
  ggsave(nasc.cufes.trawl.plot, 
         filename = here("Simulation/Figs/fig_nasc_cufes_haul_wt.png"),
         width = map.width*3, height = map.height)
  
  ggsave(nasc.cufes.trawl.plot.long, 
         filename = here("Simulation/Figs/fig_nasc_cufes_haul_wt_long.png"),
         width = map.width, height = map.height*3)
}

include_graphics(here("Simulation/Figs/fig_nasc_cufes_haul_wt_long.png"))
```

# Calculate target strengths (TS) from lengths 
## Calculate TS from TL  

View the parameters used to estimate target strength in the estimate_ts function in the 'Code' subdirectory.
...
# Save results
write.csv(ts.df,   file = here("Simulation/Output/ts_estimates.csv"), quote = F, row.names = F)
write.csv(lengths, file = here("Simulation/Output/ts_lengths.csv"),   quote = F, row.names = F)
```  
...
ggsave(ts.theory.plot, 
       filename = here("Simulation/Figs/fig_ts_theoretical_scatter.png"), 
...
ggsave(ts.histogram, 
       filename = here("Simulation/Figs/fig_ts_histograms.png"), 
       width = ts.width, height = 2)
...
ggsave(ts.combo.plot, 
       filename = here("Simulation/Figs/fig_ts_combo_plot.png"), 
       height = 4, width = ts.width)

include_graphics(here("Simulation/Figs/fig_ts_combo_plot.png"))
```  
...
# save TS plot
ggsave(sigma.plot,
       filename = here("Simulation/Figs/fig_sigma_plots.png"), 
       width = 10, height = 7)   

include_graphics(here("Simulation/Figs/fig_sigma_plots.png"))
```
...
write.csv(n.summ.haul, file = here("Simulation/Output/n_summ_haul.csv"), 
          quote = F, row.names = F)
write.csv(l.summ.haul, file = here("Simulation/Output/l_summ_haul.csv"), 
          quote = F, row.names = F)
write.csv(l.summ.cluster, file = here("Simulation/Output/l_summ_cluster.csv"), 
          quote = F, row.names = F)
write.csv(trawl.ts, file = here("Simulation/Output/trawl_ts_EstimateCPS.csv"), 
...
if (save.figs) {
  ...
  # Comtine results of haul and cluster summaries and save
  sigma.plot.all <- plot_grid(sigma.plot.haul, sigma.plot.cluster, 
                              nrow = 2, align = 'h', labels = c("a)","b)"))
  ggsave(sigma.plot.all, 
         filename = here("Simulation/Figs/fig_sigma_comparisons_all.png"), 
         height = 10, width = 10)  
}

include_graphics(here("Simulation/Figs/fig_sigma_comparisons_all.png"))
```  
...
Calculate and combine sigmas by weight (`sigmawg`) and number (`sigmaindiv`) for each species and summarise total CPS catch by weight (`CPS.wg`) and number (`CPS.num`).
...
# Save to file
write.csv(clf, file = here("Simulation/Output/clf_cluster_summary.csv"), 
          quote = F, row.names = F)
```  
...
ts.proportions[atm:::isNaNdf(ts.proportions)] <- NA

# Save to file
write.csv(ts.proportions, file = here("Simulation/Output/ts_proportions_raw.csv"), 
          row.names = F, quote = F)
...
# Save to file
write.csv(clf, file = here("Simulation/Output/clf_ts_proportions.csv"),
          row.names = F, quote = F)

save(clf, file = here("Simulation/Output/clf_ts_proportions.Rdata"))
```
...
if (save.figs) {
  ...
  acoustic.prop.cluster.wg <- base.map +
    ...
    geom_scatterpie(data = acoustic.prop.wg, 
                    aes(X, Y, group = cluster, r = pie.radius),
                    cols = c("prop.anch.wg","prop.jack.wg","prop.her.wg",
                             "prop.mack.wg","prop.sar.wg"),
                    color = 'black', alpha = 0.8) +
    # Plot empty trawl locations
    geom_point(data = cluster.zero, aes(X, Y),
  ...
  acoustic.prop.comparison <- plot_grid(acoustic.prop.cluster.indiv, acoustic.prop.cluster.wg,
                                        nrow = 1, labels = c("a)", "b)"))
  
  # Save trawl species proportions plots
  ggsave(acoustic.prop.comparison,
         filename = here("Simulation/Figs/fig_acoustic_proportion_comparison.png"),
         width = map.width*2, height = map.height)
  
} 

include_graphics(here("Simulation/Figs/fig_acoustic_proportion_comparison.png"))
```
...
acoustic.prop.cluster.final <- base.map +
  ...
  scale_fill_manual(name = 'Species',
                    ...
                    values = c(anchovy.color, pac.herring.color, jack.mack.color,  
                               pac.mack.color, sardine.color)) +
  # Plot panel label
  # ggtitle("Acoustic Proportions by Cluster") +
  coord_sf(crs = crs.proj, 
           xlim = unname(c(map.bounds["xmin"], map.bounds["xmax"])), 
           ylim = unname(c(map.bounds["ymin"], map.bounds["ymax"])))
...
if (save.figs) {
  ...
  ggsave(nasc.cufes.prop.plot, 
         filename = here("Simulation/Figs/fig_nasc_cufes_acoustic_cluster.png"),
         width = map.width*3, height = map.height)
  
  ggsave(nasc.cufes.prop.plot.long, 
         filename = here("Simulation/Figs/fig_nasc_cufes_acoustic_cluster_long.png"),
         width = map.width, height = map.height*3)  
  
  ggsave(nasc.trawl.acoustic.prop, 
         filename = here("Simulation/Figs/fig_nasc_acoustic_cluster.png"),
         width = map.width*2, height = map.height)

}

include_graphics(here("Simulation/Figs/fig_nasc_cufes_acoustic_cluster_long.png"))
```
...
for (i in unique(lengths$scientificName)) {
  ...
  lf.table <- pivot_wider(lf.df, id_cols = cluster, names_from = lf.labels, values_from = counts, values_fn = sum) %>% 
    rowwise(cluster) %>%
  ...
  # Rename columns
  names(lf.table)[2:ncol(lf.table)] <- paste("L",names(lf.table)[2:ncol(lf.table)], sep = "")
  ...
  write.csv(clf.final,
            file = paste(here("Output/cluster_length_frequency_"), 
  ...
  # Save plot
  ggsave(lf.plot, 
         filename = paste(here("Simulation/Figs/fig_cluster_length_frequency_"), 
                          i, ".png", sep = ""),
         height = 7, width = 10) 
}

# Save results
save(cluster.final, file = here("Simulation/Output/cluster_length_frequency_all.Rdata"))
save(lf.final,      file = here("Simulation/Output/cluster_length_frequency_tables.Rdata"))
```
...
save(nasc, clf, file = here("Simulation/Output/nasc_clf.Rdata"))
...
# Save results
save(nasc, file = here("Simulation/Output/cps_nasc_prop.Rdata"))

# Write results to CSV
write.csv(nasc, file = here("Simulation/Output/cps_nasc_prop.csv"), 
          quote = F, row.names = F)
```  
...
(ref:apportion-backscatter) Proportion of backscatter from each species (red points) relative to the total backscatter from all species (blac points).
...
nasc.prop.all <- nasc %>%
  mutate(`Engraulis mordax`      = cps.nasc*prop.anch,
         ...
         `Clupea pallasii`       = cps.nasc*prop.her) # %>% project_df(to = crs.proj)
...
if (save.figs) {
  ...
  # Save map
  ggsave(here("Simulation/Figs/fig_nasc_acoustic_proportions.png"), map.prop.all,
         width = map.width*3, height = map.height*2)

} 

include_graphics(here("Simulation/Figs/fig_nasc_acoustic_proportions.png"))
```  
...
# Calculate biomass density for each species
nasc <- nasc %>% 
  mutate( 
...
save(nasc.density, nasc.density.summ, 
     file = here("Simulation/Output/nasc_biomass_density.Rdata"))

write_csv(nasc.density, here("Simulation/Output/nasc_density.csv"))
write_csv(nasc.density.summ, here("Simulation/Output/nasc_density_summary.csv"))
...
save(nasc.clust.hist, file = here("Simulation/Output/cluster_distance_data.Rdata"))
...
ggsave(clust.dist.all, 
       file = here("Simulation/Figs/fig_cluster_interval_distance_histogram.png"), 
       height = 5, width = 10)

include_graphics(here("Simulation/Figs/fig_cluster_interval_distance_histogram.png"))
```  
...
nasc.clust.biom <- nasc.clust.hist %>% 
  ...
  mutate(distance = dist.bin * ceiling(max(cluster.distance$cluster.distance)/30))
...
clust.biom.hist <- ggplot(nasc.clust.biom, aes(distance, total.density)) + 
...
ggsave(clust.biom.all, 
       file = here("Simulation/Figs/fig_cluster_distance_cumulative_biomass.png"), 
       height = 5, width = 10)

include_graphics(here("Simulation/Figs/fig_cluster_distance_cumulative_biomass.png"))
```
...
save(tx.nn, file = here("Simulation/Output/transect_spacing.Rdata"))
...
Sampling strata are defined automatically according to survey vessel, transct spacing, coastline features, and 
...
If the strata are unable to be defined automatically using the rules above, the analyst may define them manually in the appropriate survey info file (e.g., `Code/survey_info_XXXXYY.R`). 
...
# Filter stata.final using strata.filter
strata.final <- strata.final %>% 
...
save(strata.final, strata.summ, strata.summ.rm,
     file = here("Simulation/Output/strata_info.Rdata"))

# Write results to .csv file
write.csv(strata.final, file  = here("Simulation/Output/strata_final.csv"),  
          quote = F,row.names = F)

write.csv(strata.summ, file   = here("Simulation/Output/strata_summary.csv"),
          quote = F,row.names = F)
...
strata.summ %>% 
  ...
  mutate(nTx = cell_spec(nTx, knitr.format, color = "black",
         ...
         nClust = cell_spec(nClust, knitr.format, color = "black",
                            background = ifelse(nClust >= nClusters.min,"green","red")),
         nIndiv = cell_spec(nIndiv, knitr.format, color = "black",
                            background = ifelse(nIndiv >= nIndiv.min,"green","red"))) %>% 
  kable(format = knitr.format, booktabs = TRUE, escape = F,
...
View results of strata definition algorithm compated to transect spacing, biomass density, etc.
...
landmarks <- data.frame(
  name = c("Pt. Conception", "Cape Mendocino", "Cape Flattery"),
  lat.start = c(34.448266, 40.430520, 48.490),
  y = rep(0,3))
...
if (nrow(strata.summ.rm) > 0) {
  ...
  biomass.dens.plot.lat <- ggplot(strata.summ) + 
    ...
    geom_vline(xintercept = c(34.448266, 40.430520, 48.490), linetype = "dashed") +
    ...
    geom_text_repel(data = filter(nasc.density.summ, log(density + 1) > 0.01),
  ...
  ggsave(biomass.dens.plot.lat, 
         filename = here("Simulation/Figs/fig_biomass_density_transect_lat.png"),
         height = 10, width = 8.5*n_distinct(strata.summ$vessel.name))
  ...
  ggsave(biomass.dens.plot.tx, 
         filename = here("Simulation/Figs/fig_biomass_density_transect_tx.png"),
         height = 13, width = 13)
  
  # Original plot, based on transect name on the x-axis, with non log-transformed densities
  biomass.dens.plot.tx.name <- ggplot(strata.summ) +
    # geom_rect(aes(xmin = start - 0.4, xmax = end + 0.4, 
    #               ymin = 0, ymax = max.tx.dens, 
    #               group = stratum, colour = factor(stratum)),
    #           fill = 'gray70', alpha = 0.5) +
    # geom_rect(data = strata.summ.rm, aes(xmin = start - 0.4, xmax = end + 0.4, 
    #                                      ymin = 0, ymax = max.tx.dens, 
    ...
    geom_hline(yintercept = 0, colour = 'gray20') +
    geom_path(data = nasc.density.summ, 
              aes(transect.name, density, group = scientificName)) +
    geom_point(data = nasc.density.summ, 
               aes(transect.name, density, group = scientificName, fill = dist.bin), 
               shape = 21, size = 3) +
    # Label non-zero biomass density to facilitate picking strata
    geom_text_repel(data = filter(nasc.density.summ, density > 0.01),
                    aes(transect.name, density, label = transect.name, group = scientificName),
                    colour = 'blue', size = 3, fontface = 'italic', nudge_y = 1,
                    angle = 90) +
    scale_colour_discrete("Stratum") +
    scale_fill_discrete("Spacing (nmi)") +
    ...
    xlab("Transect") + 
    scale_y_continuous(expression(Biomass~density~(t~nmi^-2))) +
    # scale_y_continuous(expression(Biomass~density~(t~nmi^-2)), 
    #                    limits = c(-0.5, max.tx.dens), expand = c(0,0)) +
    theme_bw() +
    theme(strip.background.x = element_blank(),
          strip.text.x       = element_text(face = "italic"),
          axis.text.x        = element_text(angle = 90))
  
  ggsave(biomass.dens.plot.tx.name, 
         filename = here("Simulation/Figs/fig_biomass_density_transect_tx_name.png"),
         height = 10, width = 20)
  ...
  ggsave(biomass.dens.plot.lat, 
         filename = here("Simulation/Figs/fig_biomass_density_transect_lat.png"),
         height = 12, width = 10*n_distinct(strata.summ$vessel.name))
...
include_graphics(here("Simulation/Figs/fig_biomass_density_transect_lat.png"))
```  

## Define stratum polygons and compute area  

For each species, polygons are drawn around backscatter within each strata and the area of each polygon is calculated. The results are stored in a named list (by scientific name) and made availble to the point and bootstrap estimate functions below.  
...
for (i in unique(strata.final$scientificName)) {
  ...
  for (j in sort(unique(strata.sub$stratum))) {
    ...
    # Find nearest point on the 5 m isobath
    ...
    # Get the latitude of the nearest points on the 5 m isobath, +/- 50 points
...
save(strata.primary, 
     file = here("Simulation/Output/strata_primary_raw.Rdata"))

save(strata.nse.temp, 
     file = here("Simulation/Output/strata_nse_raw.Rdata"))
...
save(strata.primary, 
     file = here("Simulation/Output/strata_primary_final.Rdata"))
...
strata.primary.sub %>% 
  select(-area) %>% 
  sf::st_write(here::here("Simulation/Output/strata_primary.shp"),
               delete_layer = TRUE)
...
save(strata.nse, 
     file = here("Simulation/Output/strata_nse_final.Rdata"))
...
save(strata.primary.points,  
     file = here("Simulation/Output/strata_points_primary.Rdata"))

save(strata.nse.points, 
     file = here("Simulation/Output/strata_points_nearshore.Rdata"))

# Write primary stata points to CSV
write.csv(strata.primary.points,  
          file = here("Simulation/Output/strata_points_primary.csv"),
          quote = F, row.names = F)
...
write.csv(strata.summ.primary,  
          file = here("Simulation/Output/strata_summary_primary.csv"),
          quote = F, row.names = F)

write.csv(strata.summ.nse,  
          file = here("Simulation/Output/strata_summary_nse.csv"),
          quote = F, row.names = F)
```  
...
if (save.figs) {
  ...
  # Save figure
  ggsave(here("Simulation/Figs/fig_nasc_acoustic_proportions_strata.png"), map.stratum.all,
         width = map.width*2.6, height = map.height*1.5)
}

include_graphics(here("Simulation/Figs/fig_nasc_acoustic_proportions_strata.png"))
```  
...
if (save.figs) {
  ...
  for (i in unique(strata.primary$scientificName)) {
    for (j in unique(filter(strata.primary, scientificName == i)$stock)) {
      ...
      pos.cluster.txt <- pos.clusters %>% 
        ...
        ungroup() # %>% project_df(to = crs.proj)
      ...
      # Save figures
      ggsave(biomass.dens, 
             filename = paste(here("Simulation/Figs/fig_biomass_dens_"), i, "-", j, ".png",sep = ""),
             width  = map.width,height = map.height)
  ...
  # Save map objects
  save(biomass.dens.figs, file = here("Simulation/Output/biomass_dens_maps_all.Rdata")) 
  save(nasc.density.plot.all, file = here("Simulation/Output/nasc_density_plot_all.Rdata"))
  
} else {
  load(here("Simulation/Output/biomass_dens_maps_all.Rdata"))
  load(here("Simulation/Output/nasc_density_plot_all.Rdata"))
}
...
for (i in cps.spp) {
  if (is.null(biomass.dens.figs[[i]])) {
    ...
    ggsave(biomass.dens.temp, 
           filename = paste(here("Simulation/Figs/fig_biomass_dens_"), i, ".png", sep = ""))
  }
}
```
...
# Add cluster species composition labels and pop-ups ---------------------------
...
```{r interactive-data-explorer, fig.cap='(ref:interactive-data-explorer)',echo=FALSE}
# Leaflet options
...
# Create color pallette for trawls casts (presently unused)
...
if (do.imap) {
  ...
  i.map <- leaflet() %>% 
    ...
    # Add biomass density legend
    leaflet::addLegend("bottomleft", colors = dens.colors,
                       values = sort(unique(nasc.density.sf$bin.level)),
                       labels = dens.labels,
                       title = "Biomass Density <br/> (t nmi<sup>-2</sup>)",
    ...
    leaflet::addLegend("bottomleft", colors = dens.colors,
                       ...
                       labels = dens.labels,
                       title = "Biomass Density <br/> (t nmi<sup>-2</sup>)",
                       opacity = 1, group = "Biomass (J. mackerel)") %>%
    leaflet::addLegend("bottomleft", colors = dens.colors,
                       values = sort(unique(nasc.density.sf$bin.level)),
                       labels = dens.labels,
                       title = "Biomass Density <br/> (t nmi<sup>-2</sup>)",
    ...
    hideGroup(c(
      ...
      "Planned Transects (Ancillary)", "Trawl Cluster Polygons",
      "Biomass (Anchovy)", "Biomass (Sardine)",
      "Biomass (J. mackerel)", "Biomass (P. mackerel)", 
      "Biomass (Herring)")) %>% 
  ...
  if (save.imap) {
    mapshot(i.map, url = here("Simulation/Output/estimateBiomass_explorer.HTML"))
  }
  
  # Save imap
  dir_create(here("Simulation/Output/imap"))
  save(i.map, file = here("Simulation/Output/imap/estimateBiomass_explorer.Rdata"))
...
# Calculate abundance and biomass estimates  
...
save(nasc, file = here("Simulation/Output/nasc_final.Rdata"))
...
save(point.estimates, 
     file = here("Simulation/Output/biomass_point_estimates.Rdata")) 

save(point.estimates.nse, 
     file = here("Simulation/Output/biomass_point_estimates_nse.Rdata"))

# Save strata nasc summaries to CSV
write_csv(nasc.summ.strata, here("Simulation/Output/nasc_strata_summary.csv"))
...
# Save point estimates
save(pe, file = here("Simulation/Output/biomass_point_estimates_final.Rdata"))
write_csv(pe, here("Simulation/Output/biomass_point_estimates_final.csv"))
...
save(nasc.nse.final, file = here("Simulation/Output/nasc_nse_final.Rdata"))
```
...
# Save point estimates
save(pe.nse, file = here("Simulation/Output/biomass_point_estimates_nse_final.Rdata"))
write_csv(pe.nse, here("Simulation/Output/biomass_point_estimates_nse_final.csv"))
...
if (do.bootstrap) {
  ...
  # Create data frame for comparing bootstrap esimates
  ...
  save(bootstrap.estimates, abundance.estimates, survey.summary, 
       ...
       file = (here("Simulation/Output/biomass_bootstrap_est.Rdata")))
  
  # Write bootstrap comparison to CSV
  write_csv(bootstrap.comp, here("Simulation/Output/bootstrap_comparison.csv"))
} else{
  # Save bootstrap results
  load(here("Simulation/Output/biomass_bootstrap_est.Rdata"))
}
...
# Save results
save(be.stratum, 
     file = here("Simulation/Output/biomass_bootstrap_estimates_stratum.Rdata"))
...
# Save results
write.csv(be.survey, 
          file  = here("Simulation/Output/biomass_bootstrap_estimates_survey.csv"),
          quote = F,row.names = F)

save(be.survey, file = here("Simulation/Output/biomass_bootstrap_estimates_survey.Rdata"))
```  
...
# Save results
write.csv(be,
          file = here("Simulation/Output/biomass_bootstrap_estimates_final.csv"),
          quote = F, row.names = F)
save(be,  file = here("Simulation/Output/biomass_bootstrap_estimates_final.Rdata"))
...
ggsave(biomass.histogram.stratum, 
       filename = here("Simulation/Figs/fig_biomass_histogram_stratum.png"),
       height = 8, width = 14)

include_graphics(here("Simulation/Figs/fig_biomass_histogram_stratum.png"))
```  
...
ggsave(biomass.histogram.survey,
       filename = here("Simulation/Figs/fig_biomass_histogram_survey.png"),
       height = 8, width = 14)

include_graphics(here("Simulation/Figs/fig_biomass_histogram_survey.png"))
```  
...
```{r biomass-bootstrap-estimates-nearshore, include=FALSE}
# Generate multiple bootstrap biomass estimates
if (do.bootstrap) {
  # Create data frame for biomass estimates
  ...
  # abundance.estimates.nse[atm:::isNaNdf(abundance.estimates.nse)] <- 0
  ...
  save(bootstrap.estimates.nse, abundance.estimates.nse, survey.summary.nse, 
       ...
       file = (here("Simulation/Output/biomass_bootstrap_est_nearshore.Rdata")))
} else{
  # Save bootstrap results
  load(here("Simulation/Output/biomass_bootstrap_est_nearshore.Rdata"))
}
...
# Save results
save(be.stratum.nse, 
     file = here("Simulation/Output/biomass_bootstrap_estimates_stratum_nearshore.Rdata"))
...
write.csv(be.survey.nse, 
          file  = here("Simulation/Output/biomass_bootstrap_estimates_survey_nearshore.csv"),
          quote = F,row.names = F)

save(be.survey.nse, file = here("Simulation/Output/biomass_bootstrap_estimates_survey_nearshore.Rdata"))
```
...
be.nse <- be.stratum.nse %>% 
  ...
  arrange(Species, Stock, Stratum) %>% 
  replace_na(list(Stratum = "All", pct.tot.B = 100)) %>% # Replace NAs with "All"
...
be.nse[atm:::isNaNdf(be.nse)] <- NA

# Save results
write.csv(be.nse,
          file = here("Simulation/Output/biomass_bootstrap_estimates_final_nse.csv"),
          quote = F, row.names = F)

save(be.nse, 
     file = here("Simulation/Output/biomass_bootstrap_estimates_final_nse.Rdata"))
...
# Save plot
ggsave(biomass.histogram.stratum.nse, filename = here("Simulation/Figs/fig_biomass_histogram_stratum_nse.png"),
       height = 8, width = 14)

include_graphics(here("Simulation/Figs/fig_biomass_histogram_stratum_nse.png"))
```  
...
ggsave(biomass.histogram.survey.nse,
       filename = here("Simulation/Figs/fig_biomass_histogram_survey_nse.png"),
       height = 8, width = 14)

include_graphics(here("Simulation/Figs/fig_biomass_histogram_survey_nse.png"))
```

## Calculate biomass - Other estimates
### Offshore strata  

If desired, estimate biomass in the offshore strata. 
...
### Nearshore strata
If desired, estimate biomass in the nearshore strata.
...
if (combine.regions) {
  ...
  # Save combined nasc
  save(nasc.combo, file = here("Simulation/Output/nasc_combo.Rdata"))
...
# Import comparisons
bootstrap.comp <- read_csv(here("Simulation/Output/bootstrap_comparison.csv")) %>% 
...
# Save plot
ggsave(bootstrap.comp.plot, filename = here("Simulation/Figs/fig_bootrap_function_comparison.png"),
       height = 4, width = 8)

# Include plot
include_graphics(here("Simulation/Figs/fig_bootrap_function_comparison.png"))
```
...
save(lf.summ, file = here("Simulation/Output/length_frequency_summary.Rdata"))
...
for (i in unique(strata.final$scientificName)) {
  ...
  pct.summ.cluster <- nasc.temp %>% 
    ...
    mutate(pct.abund.stratum = num.cluster/num.stratum*100,
           pct.txt      = paste("(", stratum, ") ", round(pct.abund.stratum, 1),"%", sep = ''),
  ...
  a.ratio <- ifelse(n.facets > 1, round(n.facets)/lf.ncols, 1)
  
  # Save plot
  ggsave(lf.summ.plot, 
         filename = paste(here("Simulation/Figs/fig_cluster_relative_length_frequency_"), 
                          i, ".png", sep = ""),
...
# Save plots
save(lf.summ.plots, file = here("Simulation/Output/relative_length_frequencies_all.Rdata"))
...
for (i in unique(abund.summ$Species)) {
  for (j in unique(abund.summ$Stock[abund.summ$Species == i])) {
    ...
    # Save plot
    ggsave(L.disagg.all, 
           filename = paste(here("Simulation/Figs/fig_L_disagg_"), i, "-", j, ".png", sep = ""), 
           height = 6, width = 6)
...
for (i in cps.spp) {
  if (is.null(L.disagg.plots[[i]])) {
    ...
    ggsave(L.disagg.temp, 
           filename = paste(here("Simulation/Figs/fig_L_disagg_"),i,".png", sep = ""))
  }
}  
```  
...
# Save results
save(L.abund.table, abund.summ, file = here("Simulation/Output/abundance_table_all.Rdata"))
```
...
if (combine.regions) {
  if (file.exists(here("Simulation/Figs/fig_biomass_dens_combo_Engraulis mordax-Northern.png"))) {
    include_graphics(here("Simulation/Figs/fig_biomass_dens_combo_Engraulis mordax-Northern.png"))  
  } else {
    print("No data for this stock.")
  }
} else {
  if (file.exists(here("Simulation/Figs/fig_biomass_dens_Engraulis mordax-Northern.png"))) {
    include_graphics(here("Simulation/Figs/fig_biomass_dens_Engraulis mordax-Northern.png"))  
  } else {
    print("No data for this stock.")
  }
}
...
if (combine.regions) {
  if (file.exists(here("Simulation/Figs/fig_L_disagg_combo_Engraulis mordax-Northern.png"))) {
    include_graphics(here("Simulation/Figs/fig_L_disagg_combo_Engraulis mordax-Northern.png"))  
  } else {
    print("No data for this stock.")
  } 
} else {
  if (file.exists(here("Simulation/Figs/fig_L_disagg_Engraulis mordax-Northern.png"))) {
    include_graphics(here("Simulation/Figs/fig_L_disagg_Engraulis mordax-Northern.png"))  
  } else {
    print("No data for this stock.")
  }
}
...
if ("Engraulis mordax" %in% unique(abund.summ$Species)) {
  ...
  if (nrow(abund.summ.table) != 0) {
    if (doc.type == "docx") {
      ...
      kable(abund.summ.table, booktabs = F,escape = F,longtable = T,
            digits = c(0),
            format.args = list(big.mark = ","),
...
if (combine.regions) {
  if (file.exists(here("Simulation/Figs/fig_biomass_dens_combo_Engraulis mordax-Central.png"))) {
    include_graphics(here("Simulation/Figs/fig_biomass_dens_combo_Engraulis mordax-Central.png"))  
  } else {
    print("No data for this stock.")
  }
} else {
  if (file.exists(here("Simulation/Figs/fig_biomass_dens_Engraulis mordax-Central.png"))) {
    include_graphics(here("Simulation/Figs/fig_biomass_dens_Engraulis mordax-Central.png"))  
  } else {
    print("No data for this stock.")
  }
}
...
# Insert disaggregated abundance plots for anchovy
if (combine.regions) {
  if (file.exists(here("Simulation/Figs/fig_L_disagg_combo_Engraulis mordax-Central.png"))) {
    include_graphics(here("Simulation/Figs/fig_L_disagg_combo_Engraulis mordax-Central.png"))  
  } else {
    print("No data for this stock.")
  } 
} else {
  if (file.exists(here("Simulation/Figs/fig_L_disagg_Engraulis mordax-Central.png"))) {
    include_graphics(here("Simulation/Figs/fig_L_disagg_Engraulis mordax-Central.png"))  
  } else {
    print("No data for this stock.")
  }
}
...
if (file.exists(here("Simulation/Figs/fig_cluster_relative_length_frequency_Engraulis mordax.png"))) {
  include_graphics(here("Simulation/Figs/fig_cluster_relative_length_frequency_Engraulis mordax.png"))  
} else {
  print("No data for this stock.")
}
```  
...
if (combine.regions) {
  if (file.exists(here("Simulation/Figs/fig_biomass_dens_combo_Sardinops sagax-Northern.png"))) {
    include_graphics(here("Simulation/Figs/fig_biomass_dens_combo_Sardinops sagax-Northern.png"))  
  } else {
    print("No data for this stock.")
  }
} else {
  if (file.exists(here("Simulation/Figs/fig_biomass_dens_Sardinops sagax-Northern.png"))) {
    include_graphics(here("Simulation/Figs/fig_biomass_dens_Sardinops sagax-Northern.png"))  
  } else {
    print("No data for this stock.")
  }
}
...
if (combine.regions) {
  if (file.exists(here("Simulation/Figs/fig_L_disagg_combo_Sardinops sagax-Northern.png"))) {
    include_graphics(here("Simulation/Figs/fig_L_disagg_combo_Sardinops sagax-Northern.png"))  
  } else {
    print("No data for this stock.")
  } 
} else {
  if (file.exists(here("Simulation/Figs/fig_L_disagg_Sardinops sagax-Northern.png"))) {
    include_graphics(here("Simulation/Figs/fig_L_disagg_Sardinops sagax-Northern.png"))  
  } else {
    print("No data for this stock.")
  }
}
...
if (combine.regions) {
  if (file.exists(here("Simulation/Figs/fig_biomass_dens_combo_Sardinops sagax-Southern.png"))) {
    include_graphics(here("Simulation/Figs/fig_biomass_dens_combo_Sardinops sagax-Southern.png"))  
  } else {
    print("No data for this stock.")
  }
} else {
  if (file.exists(here("Simulation/Figs/fig_biomass_dens_Sardinops sagax-Southern.png"))) {
    include_graphics(here("Simulation/Figs/fig_biomass_dens_Sardinops sagax-Southern.png"))  
  } else {
    print("No data for this stock.")
  }
}
...
# Insert disaggregated abundance plots for sardine
if (combine.regions) {
  if (file.exists(here("Simulation/Figs/fig_L_disagg_combo_Sardinops sagax-Southern.png"))) {
    include_graphics(here("Simulation/Figs/fig_L_disagg_combo_Sardinops sagax-Southern.png"))  
  } else {
    print("No data for this stock.")
  } 
} else {
  if (file.exists(here("Simulation/Figs/fig_L_disagg_Sardinops sagax-Southern.png"))) {
    include_graphics(here("Simulation/Figs/fig_L_disagg_Sardinops sagax-Southern.png"))  
  } else {
    print("No data for this stock.")
  }
}
...
if (file.exists(here("Simulation/Figs/fig_cluster_relative_length_frequency_Sardinops sagax.png"))) {
  include_graphics(here("Simulation/Figs/fig_cluster_relative_length_frequency_Sardinops sagax.png"))  
} else {
  print("No data for this stock.")
}
``` 
...
if (combine.regions) {
  if (file.exists(here("Simulation/Figs/fig_biomass_dens_combo_Scomber japonicus-All.png"))) {
    include_graphics(here("Simulation/Figs/fig_biomass_dens_combo_Scomber japonicus-All.png"))  
  } else {
    print("No data for this stock.")
  }
} else {
  if (file.exists(here("Simulation/Figs/fig_biomass_dens_Scomber japonicus-All.png"))) {
    include_graphics(here("Simulation/Figs/fig_biomass_dens_Scomber japonicus-All.png"))  
  } else {
    print("No data for this stock.")
  }
}
...
if (combine.regions) {
  if (file.exists(here("Simulation/Figs/fig_L_disagg_combo_Scomber japonicus-All.png"))) {
    include_graphics(here("Simulation/Figs/fig_L_disagg_combo_Scomber japonicus-All.png"))  
  } else {
    print("No data for this stock.")
  } 
} else {
  if (file.exists(here("Simulation/Figs/fig_L_disagg_Scomber japonicus-All.png"))) {
    include_graphics(here("Simulation/Figs/fig_L_disagg_Scomber japonicus-All.png"))  
  } else {
    print("No data for this stock.")
  }
}
...
# Insert relative length frequency plot for anchovy
if (file.exists(here("Simulation/Figs/fig_cluster_relative_length_frequency_Scomber japonicus.png"))) {
  include_graphics(here("Simulation/Figs/fig_cluster_relative_length_frequency_Scomber japonicus.png"))  
} else {
  print("No data for this stock.")
}
``` 
...
if (combine.regions) {
  if (file.exists(here("Simulation/Figs/fig_biomass_dens_combo_Trachurus symmetricus-All.png"))) {
    include_graphics(here("Simulation/Figs/fig_biomass_dens_combo_Trachurus symmetricus-All.png"))  
  } else {
    print("No data for this stock.")
  }
} else {
  if (file.exists(here("Simulation/Figs/fig_biomass_dens_Trachurus symmetricus-All.png"))) {
    include_graphics(here("Simulation/Figs/fig_biomass_dens_Trachurus symmetricus-All.png"))  
  } else {
    print("No data for this stock.")
  }
}
...
if (combine.regions) {
  if (file.exists(here("Simulation/Figs/fig_L_disagg_combo_Trachurus symmetricus-All.png"))) {
    include_graphics(here("Simulation/Figs/fig_L_disagg_combo_Trachurus symmetricus-All.png"))  
  } else {
    print("No data for this stock.")
  } 
} else {
  if (file.exists(here("Simulation/Figs/fig_L_disagg_Trachurus symmetricus-All.png"))) {
    include_graphics(here("Simulation/Figs/fig_L_disagg_Trachurus symmetricus-All.png"))  
  } else {
    print("No data for this stock.")
  }
}
...
if (file.exists(here("Simulation/Figs/fig_cluster_relative_length_frequency_Trachurus symmetricus.png"))) {
  include_graphics(here("Simulation/Figs/fig_cluster_relative_length_frequency_Trachurus symmetricus.png"))  
} else {
  print("No data for this stock.")
}
``` 
...
if (combine.regions) {
  if (file.exists(here("Simulation/Figs/fig_biomass_dens_combo_Clupea pallasii-All.png"))) {
    include_graphics(here("Simulation/Figs/fig_biomass_dens_combo_Clupea pallasii-All.png"))  
  } else {
    print("No data for this stock.")
  }
} else {
  if (file.exists(here("Simulation/Figs/fig_biomass_dens_Clupea pallasii-All.png"))) {
    include_graphics(here("Simulation/Figs/fig_biomass_dens_Clupea pallasii-All.png"))  
  } else {
    print("No data for this stock.")
  }
}
...
if (combine.regions) {
  if (file.exists(here("Simulation/Figs/fig_L_disagg_combo_Clupea pallasii-All.png"))) {
    include_graphics(here("Simulation/Figs/fig_L_disagg_combo_Clupea pallasii-All.png"))  
  } else {
    print("No data for this stock.")
  } 
} else {
  if (file.exists(here("Simulation/Figs/fig_L_disagg_Clupea pallasii-All.png"))) {
    include_graphics(here("Simulation/Figs/fig_L_disagg_Clupea pallasii-All.png"))  
  } else {
    print("No data for this stock.")
  }
}
...
if (file.exists(here("Simulation/Figs/fig_cluster_relative_length_frequency_Clupea pallasii.png"))) {
  include_graphics(here("Simulation/Figs/fig_cluster_relative_length_frequency_Clupea pallasii.png"))  
} else {
  print("No data for this stock.")
}
```

# Compare real and simlulated estimates

Biomass estimates from the survey are graphically compared with those from the simulation. 

```{r compare-estimates-fig}
# Create copies of simulated biomass data
be.sim <- be %>% 
  # select(Species, Stock, everything()) %>% 
  filter(!Stratum == "All") %>% 
...
be.comp <- be.actual %>% 
  left_join(select(be.sim, Species, Stock, Stratum, Biomass, lower.ci.B, upper.ci.B, biomass.cv),
            by = c("Species", "Stock", "Stratum")) %>% 
  mutate(
    # Species = paste(Species, Stock, sep = "-"),
    B.lower.x = abs(Biomass.x - lower.ci.B.x),
    B.lower.y = abs(Biomass.y - lower.ci.B.y),
    B.upper.x = abs(Biomass.x - upper.ci.B.x),
    B.upper.y = abs(Biomass.y - upper.ci.B.y))

b.comp.plot <- ggplot(be.comp) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  geom_point(aes(Biomass.x, Biomass.y, fill = Stock), size = 4, shape = 21) +
  geom_errorbar(aes(x = Biomass.x, ymin = lower.ci.B.y, ymax = upper.ci.B.y)) + 
  geom_errorbarh(aes(y = Biomass.y, xmin = lower.ci.B.x, xmax = upper.ci.B.x))  +
  facet_wrap(~Species, scales = "free") +
  theme_bw() +
  labs(x = "Actual biomass",
       y = "Simulated biomass",
       title = "Comparison of actual and simulated biomass (+/- CIs)")

ggsave(b.comp.plot, filename = here("Simulation/Figs/fig_simulated_biomass_comparison.png"),
       height = 8, width = 12)

include_graphics(here("Simulation/Figs/fig_simulated_biomass_comparison.png"))
```
