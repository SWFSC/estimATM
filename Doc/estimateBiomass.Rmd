...
pacman::p_load(tidyverse,swfscMisc,reshape2,tcltk,lubridate,sp,readxl,
               ...
               leaflet.extras,mapview,photobiology,fs,plotly,ggnewscale,
...
# Define global knitr NA display value
...
Deep backscatter depth: **`r nasc.depth.deep`**  
Nearshore catch source: **`r glue::glue_collapse(catch.source.ns, sep = ", ", last = " and ")`**  
Nearshore trawl source (if included): **`r unname(cluster.source["NS"])`**  
Number of bootstrap iterations: **`r boot.num`**  
...
Options in this section of the settings file control the execution of the various code chunks. All chunks should usually run (i.e., controls  <- `T`) the first time to copy all necessary data files, import data from databases, generate maps and figures, estimate abundance and biomass, etc. Subsequent processing may be sped-up by setting some controls to `F` if source data or analysis code has not changed.

```{r user-controls, echo = TRUE}
# Processing controls (T/F) ---------------------------------------------------
### Usually TRUE
get.db            <- ifelse(Sys.info()['nodename'] %in% c("SWC-KSTIERHOFF-"), F, T) 
...
do.ggiraph        <- F # Use ggiraph interactive figures instead of ggplot
get.bathy         <- T # Extract ETOPO1 bathymetry data in the survey footprint
...
extrapolate.biomass <- F # Extrapolate biomass into the nearshore region
```

### Survey-specific settings  

To more easily configure processing for different surveys and distribute updated code, the settings file is also used to configure survey-specific settings for each survey (e.g., **`r prj.settings`**). There are **MANY** settings, all of which can have a bearing on analysis results and should be reviewed at the start of each survey analysis.
...
if (copy.files) {
  ...
  # Copy backscatter files
  source(here("Code/copy_files_nasc.R"))
  
  # Copy CUFES files
  if(file.exists(file.path(survey.dir[survey.vessel.primary], "DATA/BIOLOGICAL/CUFES"))) {
    # Copy CUFES files
    source(here("Code/copy_files_cufes.R")) 
  }
  
  if (dir.exists(uctd.dir)) {
    # Copy UCTD files
    source(here("Code/copy_files_uctd.R"))
  }
  
  if (dir.exists(ctd.dir)) {
    # Copy CTD files
    source(here("Code/copy_files_ctd.R"))
  }
}
```  
...
# Source code to get nav data for Saildrone from ERDDAP
if (get.nav.sd) {
...
save(map.bounds, file = here("Output/basemap_bounds.Rdata"))
...
# Configure spatial layers for geoprocessing

```{r configure-spatial-files, include=FALSE}
# Load and modify masking polygons
# Read North America land mask shapefile and buffer by the distance of nearshore transects from shore
...
ns_clip <- st_read(here("Data/GIS/bathy_5m_wc_poly.shp")) %>% 
  st_cast("POLYGON") %>% 
  st_transform(crs.proj)
...
```{r process-backscatter, include=FALSE}
if (process.csv) {
  if (!process.csv.all) {
        ...
        if (rm.nearshore[i]) {
          nasc.nearshore <- nasc.vessel %>% 
            filter(str_detect(transect.orig, nasc.pattern.nearshore[i]) |
                     ...
                     vessel.name %in% nasc.vessels.nearshore) 
          ...
          if (nrow(nasc.nearshore) > 0) {
            ...
            # Remove all nearshore NASC from nasc.vessel
            nasc.vessel <- nasc.vessel %>% 
              filter(!str_detect(transect.orig, nasc.pattern.nearshore[i])) %>% 
              filter(!str_detect(tolower(filename), "nearshore")) %>% 
...
## Compare CPS backscatter (cps.nasc) to `r nasc.depth.cps`
...
```{r process-backscatter-krill,eval=FALSE}
...
big.nasc <- nasc %>%
  ...
  select(rank, cps.nasc, label, popup, datetime, dist_m, sounder, 
         lat, long, transect.name, vessel.name, vessel.orig, type)

# Write outliers to CSV
write_csv(big.nasc, 
          here("Output/nasc_outliers.csv"))

# Create outlier plot
nasc.outlier.plot <- ggplot(big.nasc, aes(rank, cps.nasc, ids = label, fill = type)) +
  geom_point_interactive(shape = 21, size = 2, alpha = 0.7, aes(tooltip = label)) +
  ...
  nasc.outlier.plot.interactive <- ggiraph::girafe(
    ggobj = nasc.outlier.plot,
...
if (save.figs) {
  ...
  if (exists("nasc.outlier.plot.interactive")) {
    # Plot NASC outliers
    nasc.outlier.plot.interactive
  } else {
...
## If estimating nearshore biomass, retain core area intervals that fall outside the nearshore mask ----------
if (estimate.ns) {
  nasc.sf.sub <- nasc.sf.sub <- nasc %>% 
    st_as_sf(coords = c("long","lat"), crs = crs.geog) %>% 
    st_transform(crs = crs.proj) %>% 
    st_difference(nearshore_mask)
  
  # Keep core area acoustic intervals that fall outside the nearshore mask
  nasc <- filter(nasc, id %in% nasc.sf.sub$id)
  
  # mapview(nasc.sf.sub) + mapview(nearshore_mask)  
}
...
Trawl haul, catch, and length (both individual and binned lengths of CPS) data are imported from either an Access (at sea, generally) or SQL Server database (after the survey). To import from the SQL Server, a machine ODBC connection must be configured to the TRAWL and CUFES databases on the Estrella server (IP address: 161.55.235.187). For help configuring an ODBC connection, submit a [SWFSC help desk request](https://apps-st.fisheries.noaa.gov/jira/servicedesk/customer/portal/2/user/login?destination=portal%2F2). To request access to the Trawl database, contact [Yuhong Gu](Yuhong.Gu@NOAA.gov).  
...
```{r handle-japanese-sardine-data}
if (!estimate.j.sardine) {
  catch.all <- catch.all %>% 
    # Replace J. sardine species code with P. sardine
    mutate(species = ifelse(species == 551222, 161729, species)) %>% 
    group_by(cruise, ship, haul, collection, species, netSampleType,
             isWtEstimated, hasLF, notes, flaggedData, presenceOnly) %>%
    summarise_at(vars(subSampleCount:remainingSubSampleWtkg), sum) %>% 
    ungroup() %>%
...
if (survey.name %in% c("2107RL")) {
  ...
  # Combine hauls from FSVs and JCF
...
if (survey.name %in% c("2207RL")) {
  ...
  #   filter(startLatDecimal >= 40.42) %>%
  #   mutate(haul = haul + max.haul.rl,
  #          cluster = cumsum(c(0, diff(equilibriumTime, units = "hours")) > 12) + 1 + max.cluster.rl)
...
if (save.figs) {
  ...
  trawl.cluster.plot <- base.map +
    ...
    # Plot trawl paths
    geom_sf(data = filter(haul.paths, period == "Fishing")) +
    # Plot trawl midpoint
...
haul.ts.spp <- sort(unique(haul.ts$scientificName))
haul.ts.labels <- unname(pie.labs[names(pie.labs) %in% haul.ts.spp]
                         [order(names(pie.labs[names(pie.labs) %in% haul.ts.spp]))])
haul.ts.colors <- unname(pie.colors[names(pie.colors) %in% haul.ts.spp]
                         [order(names(pie.colors[names(pie.colors) %in% haul.ts.spp]))])
...
if (save.figs) {
  ...
  haul.ts.map <- base.map + 
    ...
    scale_colour_manual(name = 'Species',
                        labels = haul.ts.labels,
                        values = haul.ts.colors) +
    coord_sf(crs = crs.proj, 
             xlim = unname(c(map.bounds["xmin"], map.bounds["xmax"])), 
             ylim = unname(c(map.bounds["ymin"], map.bounds["ymax"])))
  ...
  mutate(
    totalLength_mm = case_when(
      scientificName == "Allosmerus elongatus" ~ 
        convert_length("Allosmerus elongatus", .$forkLength_mm, "FL", "TL"),
      scientificName == "Clupea pallasii" ~ 
        convert_length("Clupea pallasii", .$forkLength_mm, "FL", "TL"),
      scientificName == "Engraulis mordax" ~ 
        convert_length("Engraulis mordax", .$standardLength_mm, "SL", "TL"),
...
# ggplot(filter(lengths.all, scientificName == "Allosmerus elongatus")) + 
#   geom_point(aes(totalLength_mm, weightg), colour = "red") + geom_point(aes(forkLength_mm, weightg))
...
# ggplot(lw.df, aes(L, W, colour = scientificName)) + 
#   geom_line() + facet_wrap(~scientificName, scales = "free")
...
lw.plot <- ggplot() + 
  ...
  geom_point(data = lengths, 
             ...
             shape = 21, alpha = 1) + 
  scale_fill_manual(name = "Sex", values = c("pink","lightblue","white")) +
...
if (process.cufes) {
  ...
  print("No CUFES data to process.")
}
```  
...
if (save.figs) {
  ...
  if (exists("nasc.map.cps") & exists("cufes.density.all") & exists("trawl.pie.haul.wt")) {
    ...
    ggsave(nasc.cufes.haul.wt.long, 
           filename = here("Figs/fig_nasc_cufes_haul_wt_long.png"),
           width = map.width, height = map.height*3)
    
    save(nasc.cufes.haul.wt, nasc.cufes.haul.wt.long, 
         file = here("Output/nasc_cufes_trawl_map.Rdata"))
    
    include_graphics(here("Figs/fig_nasc_cufes_haul_wt_long.png")) 
  }
  
  # Combine backscatter and trawl
  if (exists("nasc.map.cps") & exists("trawl.pie.haul.wt")) {
    nasc.haul.wt <- plot_grid(nasc.map.cps, trawl.pie.haul.wt, 
                              nrow = 1, labels = c("a)", "b)")) 
    
    nasc.haul.wt.long <- plot_grid(nasc.map.cps, trawl.pie.haul.wt, 
                                   ncol = 1, labels = c("a)", "b)")) 
    
    # Save composite plot
    ggsave(nasc.haul.wt,
           filename = here("Figs/fig_nasc_haul_wt.png"),
           width = map.width*2, height = map.height) 
    
    # Save composite plot
    ggsave(nasc.haul.wt.long,
           filename = here("Figs/fig_nasc_haul_wt_long.png"),
           width = map.width*2, height = map.height*3) 
    
    include_graphics(here("Figs/fig_nasc_haul_wt_long.png")) 
  }  
}
```
...
# Create TS histograms by species
...
trawl.ts.names <- data.frame(
  scientificName = c("Allosmerus elongatus","Clupea pallasii","Engraulis mordax",
                     "Etrumeus acuminatus","Sardinops sagax","Scomber japonicus",
                     "Trachurus symmetricus"),
  shortName      = c("smelt","her","anch","rher",
                     "sar","mack","jack"))
...
# Subset smelt results
ts.sub.smelt <- filter(ts.summ, scientificName == "Allosmerus elongatus") %>% 
  select(-scientificName)
names(ts.sub.smelt) <- paste(names(ts.sub.smelt), "smelt", sep = ".") 
...
clf <- cluster.mid %>% 
  ...
  # Add smelt TS estimates to clf
  left_join(ts.sub.smelt,by  = c("cluster" = "cluster.smelt")) %>% 
...
# Calculate total CPS number in each cluster
  ...
  summarise(CPS.num = sum(num.sar, num.anch, num.mack, num.jack, 
                          num.her, num.rher, num.smelt, num.other, 
                          na.rm = TRUE))
...
ts.proportions <- clf %>% 
  ...
  summarise(
    ...
    weighted.wg  = (meanwg.sar  * num.sar  * sigmawg.sar +
                      ...
                      meanwg.smelt * num.smelt * sigmawg.smelt +
    ...
    prop.smelt    = (num.smelt    * sigmaindiv.smelt)  / weighted.num,
    prop.smelt.wg = (meanwg.smelt * sigmawg.smelt  * num.smelt) / weighted.wg,
...
clf$sigmaindiv.smelt[clf$prop.smelt == 0] <- 1
...
clf$sigmawg.smelt[clf$prop.smelt  == 0]   <- 1
...
acoustic.prop.indiv <- clf %>%
  ...
  select(cluster, lat, long, prop.anch, prop.jack, prop.her,
         prop.mack, prop.sar, prop.rher, prop.smelt, prop.other, sample.type) %>% 
  replace(. == 0, 0.0000001) %>% 
  project_df(to = crs.proj) %>% 
  mutate(pie.radius = pie.radius)

acoustic.prop.wg <- clf %>%
  filter(!is.na(CPS.wg), CPS.wg > 0) %>% 
  select(cluster, lat, long, prop.anch = prop.anch.wg, prop.jack = prop.jack.wg, 
         prop.her = prop.her.wg, prop.mack = prop.mack.wg, 
         prop.sar = prop.sar.wg, prop.rher = prop.rher.wg, 
         prop.smelt = prop.smelt.wg, prop.other = prop.other.wg, sample.type) %>% 
  replace(. == 0, 0.0000001) %>% 
  project_df(to = crs.proj) %>% 
  mutate(pie.radius = pie.radius)
...
if (save.figs) {
  ...
  acoustic.prop.cluster.indiv <- base.map +
    ...
    geom_scatterpie(data = acoustic.prop.indiv, 
                    ...
                    # r = pie.radius,
                    cols = pie.cols.prop[names(pie.cols.prop) %in% pie.spp],
...
if (save.figs) {
  # Plot backscatter, CUFES, and acoustic proportion map
  if (exists("nasc.map.cps") & exists("cufes.density.all") & exists("trawl.pie.haul.wt")) {
    ...
    # Plot backscatter, CUFES, and acoustic proportion map
    nasc.prop.plot <- plot_grid(nasc.map.cps, acoustic.prop.cluster.final,
                                nrow = 1, labels = c("a)", "b)"))
    
    ggsave(nasc.prop.plot, 
           filename = here("Figs/fig_nasc_acoustic_proportion.png"),
           width = map.width*2, height = map.height)
    
    save(nasc.prop.plot,
         file = here("Output/nasc_acoustic_proportion.Rdata"))
  ...
  #   nasc.cluster.prop.plot <- plot_grid(nasc.cluster.plot, acoustic.prop.cluster.final,
  #                                         nrow = 1, labels = c("a)", "b)"))
  #   
  #   ggsave(nasc.cluster.prop.plot, 
  #          filename = here("Figs/fig_nasc_cluster_acoustic_proportion.png"),
  #          width = map.width*2, height = map.height)
  #   
  #   save(nasc.cluster.prop.plot,
  #        file = here("Output/nasc_cluster_acoustic_proportion.Rdata"))
}
```
...
for (i in unique(lengths$scientificName)) {
  ...
  lf.table <- pivot_wider(lf.df, id_cols = cluster, names_from = lf.labels, 
                          values_from = counts, values_fn = sum) %>% 
    rowwise(cluster) %>%
    mutate('(all)' = sum(c_across(all_of(lf.labels)))) %>% 
    arrange(cluster)
  
  lf.table.h <- pivot_wider(lf.df, id_cols = haul, names_from = lf.labels, 
                            values_from = counts, values_fn = sum) %>% 
    rowwise(haul) %>%
    mutate('(all)' = sum(c_across(all_of(lf.labels)))) %>% 
    arrange(haul)
...
nasc.prop.all <- nasc %>%
  mutate(`Engraulis mordax`      = cps.nasc*prop.anch,
         ...
         `Allosmerus elongatus`  = cps.nasc*prop.smelt) # %>% project_df(to = crs.proj)
...
nasc <- nasc %>% 
  mutate( 
    ...
    smelt.dens = cps.nasc*prop.smelt / (4*pi*sigmawg.smelt) / 1000)
...
nasc.density <- nasc %>%
  select(transect, transect.orig, int, lat, long, 
         anch.dens, her.dens, jack.dens, mack.dens, 
         sar.dens, rher.dens, smelt.dens) %>% 
  ...
  summarise(
    ...
    `Allosmerus elongatus`  = mean(smelt.dens)) %>% 
...
# dens.levels.all <- sort(unique(c(unique(nasc.density$bin.level),
#                                  unique(nasc.density.ns$bin.level))))
...
nasc.clust.hist <- nasc %>% 
  select(cluster.distance, anch.dens, her.dens, 
         jack.dens, mack.dens, sar.dens, rher.dens, smelt.dens) %>% 
  rename(     
    ...
    `Allosmerus elongatus`  = smelt.dens) %>% 
...
# save.image(file = here("Output/stratum-polygon-testing.Rdata"))
# load(here("Output/stratum-polygon-testing.Rdata"))
...
tx.mid <- nasc %>% 
  ...
  st_as_sf(coords = c("long","lat"), crs = crs.geog) %>% 
  mutate(transect.orig = as.numeric(str_replace(transect.name, 
                                                paste0(survey.vessel.primary, " "), 
                                                "")))

# Create a data frame for results
if (exists("tx.nn")) rm(tx.nn)

# Compute distance to nearest transect
## Must be improved for variably spaced transects
...
# Remove geometry from tx.nn
tx.nn <- st_set_geometry(tx.nn, NULL)

# Add transect number and computed spacing 
# Get easternmost nasc interval
stratum.tx.i <- nasc %>% 
  group_by(transect) %>% 
  summarise(
...
# Combine waypoints and create sampled transects
stratum.transects <- stratum.tx.i %>% 
  bind_rows(stratum.tx.o) %>% 
  group_by(transect) %>% 
  summarise(do_union = FALSE) %>% 
  st_cast("LINESTRING") %>% 
  ungroup() %>% 
  mutate(distance = round(as.numeric(st_length(.))/1852,1)) %>% 
  mutate(Type = "Compulsory")

# mapview(stratum.transects, zcol = "transect") + mapview(tx.mid)

# Create sampled transects sf object
tx.sampled <- stratum.transects %>% 
  # filter(Type == "Compulsory", Transect %in% tx.mid$transect.orig) %>%
  left_join(select(tx.nn, transect, min.dist)) 

if (exists("tx.spacing.manual")) {
  tx.sampled <- tx.sampled %>% 
    # Use manually defined transect spacing
    bind_cols(spacing.nm = tx.spacing.manual$spacing.nm) %>% 
...
for (i in unique(tx.sampled$Type)) {
  for (j in unique(tx.sampled$transect)){
    # Subset tx.sampled for each transect
    tx.sub <- filter(tx.sampled, Type == i, transect == j) %>% 
      st_transform(crs.proj)
    ...
    if (nrow(tx.sub) > 0) {
      ...
      ext.dist <- as.numeric(st_length(tx.sub))*tx.ext.pct
      ...
      if (exists("tx.sampled.ext")) {
        tx.sampled.ext <- bind_rows(tx.sampled.ext, extended.line)
      } else {
        tx.sampled.ext <- extended.line
      }
    }
  }
}
...
# Check result
# mapview(tx.sampled.ext) + mapview(tx.sampled, color = "black")

# Buffer transects
rm("tx.sampled.buff")

for (iii in unique(tx.sampled.ext$transect)) {
  tx.sampled.ext.sub <- filter(tx.sampled.ext, transect == iii) 
  ...
  if (exists("tx.sampled.buff")) {
    tx.sampled.buff <- bind_rows(tx.sampled.buff, tx.buff.tmp)
  } else {
    tx.sampled.buff <- tx.buff.tmp
  }
}

# # View results of extending and buffering
# mapview(tx.sampled.buff, zcol = "transect") + mapview(tx.sampled.ext, color = "black") + mapview(tx.sampled)
...
landmarks <- data.frame(
  ...
  lat.start = c(32.529, 34.46, 40.44, 48.490),
  y = rep(0, 4)) %>% 
...
if (nrow(strata.summ.rm) > 0) {
  ...
  # Biomass density by species and transect, showing stratum breaks (log scale)
  biomass.dens.plot.lat.log <- ggplot(strata.summ) + 
    ...
    geom_text(data = filter(stock.breaks, scientificName == "Engraulis mordax"),
    ...
    geom_text(data = filter(stock.breaks, scientificName == "Sardinops sagax"),
              aes(lat.start, max.tx.dens.log, label = scientificName, group = scientificName),
              inherit.aes = FALSE, nudge_x = -0.6, hjust = 1, vjust = -1,
              fontface = "italic", alpha = 0.5, colour = "red") +
  ...
  # Biomass density by species and transect, showing stratum breaks (linear scale)
  biomass.dens.plot.lat <- ggplot(strata.summ) + 
    geom_rect(aes(xmin = lat.start, xmax = lat.end, ymin = 0, ymax = max.tx.dens, 
                  group = stratum, colour = factor(stratum)), 
              fill = 'gray70', alpha = 0.5) +
    geom_rect(data = strata.summ.rm, aes(xmin = lat.start, xmax = lat.end, ymin = 0, ymax = max.tx.dens,
                                         group = stratum.orig),
              fill = 'gray50', alpha = 0.5, linetype = "dotted") +
    geom_hline(yintercept = 0, colour = 'gray20') +
    geom_vline(xintercept = landmarks$lat.start, linetype = "dashed") +
    geom_vline(xintercept = stock.breaks$lat.start, linetype = "dashed", colour = "red") +
    geom_text(data = landmarks, 
              aes(lat.start, max.tx.dens, label = name), 
              inherit.aes = FALSE, nudge_x = 0.1, hjust = 1, vjust = 0, 
              fontface = "italic", alpha = 0.5) +
    geom_text(data = filter(stock.breaks, scientificName == "Engraulis mordax"),
              aes(lat.start, max.tx.dens.log, label = scientificName, group = scientificName),
    ...
    geom_text(data = filter(stock.breaks, scientificName == "Sardinops sagax"),
              ...
              fontface = "italic", alpha = 0.5, colour = "red") +
    geom_path(data = nasc.density.summ,  
              aes(start.lat, density, group = scientificName)) +
    geom_point(data = nasc.density.summ, 
               aes(start.lat, density, group = scientificName,
                   fill = dist.bin), shape = 21, size = 3) +
    # Label non-zero biomass density to facilitate picking strata
    geom_text_repel(data = filter(nasc.density.summ, density > 0),
                    aes(start.lat, density, label = transect, group = scientificName),
                    colour = 'blue', size = 3, fontface = 'italic', nudge_y = 1) +
    scale_colour_discrete("Stratum") +
    scale_fill_discrete("Spacing (nmi)") +
    facet_wrap(scientificName ~ vessel.name, nrow = 1) + 
    scale_x_continuous("Start latitude", 
                       breaks = seq(floor(min(strata.summ$lat.start)), 
                                    ceiling(max(strata.summ$lat.end)),1)) + 
    scale_y_continuous(expression(Mean~biomass~density~(t~nmi^-2))) +
    theme_bw() + 
    theme(strip.text.x       = element_text(face = "bold.italic"),
          strip.background.x = element_rect(fill = NA)) +
    coord_flip()
  ...
  biomass.dens.plot.lat.log <- ggplot(strata.summ) + 
    ...
    scale_x_continuous("Start latitude", 
                       breaks = seq(floor(min(strata.summ$lat.start)), 
                                    ceiling(max(strata.summ$lat.end)),1)) + 
    scale_y_continuous(expression(Mean~log~biomass~density~(log(t+1)~nmi^-2))) +
    theme_bw() + 
    theme(strip.text.x       = element_text(face = "bold.italic"),
          strip.background.x = element_rect(fill = NA)) +
    coord_flip()
  ...
  biomass.dens.plot.lat <- ggplot(strata.summ) + 
    ...
    geom_text(data = landmarks, 
              ...
              fontface = "italic", alpha = 0.5) +
    geom_text(data = stock.breaks,
              aes(lat.start, max.tx.dens, label = scientificName, group = scientificName),
              inherit.aes = FALSE, nudge_x = -0.1, hjust = 1, vjust = -1,
              fontface = "italic", alpha = 0.5, colour = "red") +
    geom_path(data = nasc.density.summ,  
              aes(start.lat, density, group = scientificName)) +
...
if (diff(range(nasc.density.summ$start.lat)) > 6) {
  # If latitude range greater than 6 degrees
  ggsave(biomass.dens.plot.lat.log, 
  ...
  ggsave(biomass.dens.plot.lat, 
         filename = here("Figs/fig_biomass_density_transect_lat_linear.png"),
         height = 12, width = 12*n_distinct(strata.summ$vessel.name))
...
# Add plot by latitude, in log scale
include_graphics(here("Figs/fig_biomass_density_transect_lat.png"))
```  

(ref:stratification-biomass-density-linear) Biomass density (t nmi^-2^) versus latitude (easternmost portion of each transect) and strata used to estimate biomass and abundance (shaded regions; outline indicates stratum number) for each species and survey vessel (labels above plots; e.g., SH = _Bell M. Shimada_, RL = _Reuben Lasker_, LM = _Lisa Marie_). Blue number labels correspond to the transect numbers with positive biomass (log(t + 1) > 0.01) to facilitate discrimination of stratum breaks. Point fills indicate transect spacing (i.e., the shortest distance between the midpoint of each transect and midpoint of all other transects, in nmi), which is used to define different sampling strata, for example when adaptive sampling occurs. Solid horizontal lines indicate prominent biogeographic landmarks that often delineate stock boundaries (e.g., Cape Mendocino for the central and northern stocks of Northern anchovy and Pt. Conception for the northern and southern stocks of Pacific sardine). Dashed red lines indicate the actual boundaries used for assigning biomass to various stocks, if different than above.

```{r stratification-biomass-density-linear,fig.cap='(ref:stratification-biomass-density-linear)'}
# Add plot by latitude, in linear scale
include_graphics(here("Figs/fig_biomass_density_transect_lat_linear.png"))
```
...
```{r create-pseudo-transects,eval=FALSE}
# This is the original way of drawing stratum polygons, which has been replaced by the method in the following chunk.
...
```{r create-final-strata-polygons, include=FALSE}
# # Load and modify masking polygons
...
for (i in unique(strata.final$scientificName)) {
  ...
  primary.poly.i <- tx.sampled.buff %>% 
    ...
    ungroup() %>%
    st_difference(na_buffer) %>% 
    mutate(area = st_area(.))
  
  # Create the final nse polygon for each species, sf-style
  nearshore.poly.i <- tx.sampled.buff %>% 
    left_join(strata.sub) %>% 
    left_join(nasc.stock.temp) %>%
    filter(!is.na(stratum)) %>% 
    group_by(scientificName, stock, stratum) %>% 
    summarise(geometry = st_union(x)) %>%
    ungroup() %>%
    st_intersection(na_buffer) %>% 
    st_difference(nearshore_mask) %>% 
    mutate(area = st_area(.))
  
  # mapview(primary.poly.i, zcol = "stock") + mapview(nearshore.poly.i, zcol = "stock")
  
  # mapview(tx.sampled.buff, color = "black", fill = NA) +
  #   mapview(primary.poly.i, zcol = "stratum") +
  #   mapview(tx.sampled.ext) + 
  #   mapview(tx.sampled, color = "black") +
  #   mapview(na_buffer, color = "black", fill = NA) +
  #   mapview(ns_mask, color = "red") +
  #   mapview(filter(transects.sf, Type == "Nearshore"), color = "green")
  
  # Combine with other polygons ----------------------------------------
  if (exists("strata.primary")) {
    strata.primary <- bind_rows(strata.primary, primary.poly.i)
  } else {
    strata.primary <- primary.poly.i
  }
  
  if (exists("strata.nse")) {
    strata.nse <- bind_rows(strata.nse, nearshore.poly.i)
  } else {
    strata.nse <- nearshore.poly.i
  }
}

# # Check results
# ggplot() +
#   geom_sf(data = strata.primary, aes(fill = stock)) +
#   geom_sf(data = strata.nse, aes(fill = stock), colour = "black") +
...
# Create polygons
strata.super.polygons <- tx.sampled.buff %>%
  group_by(Type) %>%
  summarise(geometry = st_union(x)) %>%
  ungroup() %>%
  st_difference(na_buffer) %>% 
...
# Save final primary polygons
save(strata.primary, 
     file = here("Output/strata_primary_final_newMethod.Rdata"))

# Save final nearshore polygons
save(strata.nse, 
     file = here("Output/strata_nse_final_newMethod.Rdata"))

## Write polygons to shapefile
strata.primary %>% 
  select(-area) %>% 
  sf::st_write(here::here("Output/strata_primary_newMethod.shp"),
               delete_layer = TRUE)

## Write polygons to shapefile
strata.nse %>% 
  select(-area) %>% 
  sf::st_write(here::here("Output/strata_nse_newMethod.shp"),
               delete_layer = TRUE)
...
if (prj.name %in% c("1807RL")) {
  ...
  # Recombine
  strata.nse <- bind_rows(strata.nse, strata.nse.sd)
}
...
# Plot polygon points - primary
# ggplot(strata.primary.points, aes(long, lat, group = stratum, fill = stock)) +
#   geom_polygon() +
#   facet_wrap(~scientificName) +
#   coord_map()
...
# Plot polygon points - nse
# ggplot(strata.nse.points, aes(long, lat, group = stratum, fill = stock)) +
#   geom_polygon() +
#   facet_wrap(~scientificName) +
#   coord_map()
...
# Write primary strata points to CSV
write_csv(strata.primary.points,  
...
write_csv(strata.summ.primary,  
          file = here("Output/strata_summary_primary.csv"))

write_csv(strata.summ.nse,  
...
if (process.ctd) {
  ...
  if (length(uctd.hdr) == 0) {
    ...
    if (process.ctd) {
      ...
      uctd.proc <- dir_ls(here("Data/UCTD"), regexp = uctd.cast.pattern, recurse = TRUE) 
...
if (exists("ctd.info.all")) {
  if (process.ctd) {
    ...
    #   # Create variable for nearest cluster
    ...
    #     transect.distance <- swfscMisc::distance(ctd.info.all$lat[i], ctd.info.all$long[i], 
...
if (exists("ctd.info.all")) {
  ...
  include_graphics(here("Figs/fig_ctd_uctd_map.png"))
  # include_graphics(here("Figs/fig_ctd_uctd_map_all.png"))
} else {
  print("No CTD casts present.")
}
```  
...
```{r map-ctd-casts-interactive,fig.cap='(ref:all-casts-mapview)'}
...
# Create data frames for point estimate results
nasc.summ.strata    <- data.frame()
nasc.nse.final      <- data.frame()
point.estimates     <- data.frame()
point.estimates.nse <- data.frame()
...
if (extrapolate.biomass) {
  ...
  print("No nearshore extrapolation estimates computed.")
}
...
be %>% 
  mutate(pe.range = between(Biomass, lower.ci.B, upper.ci.B)) %>% 
  mutate(pe.range = cell_spec(
    pe.range, knitr.format, color = "black",
    background = ifelse(pe.range,"green","red"))) %>% 
  ...
  row_spec(0, align = c("c")) %>%
  collapse_rows(columns = c(1:2)) %>%
  add_header_above(c(" " = 2, "Stratum" = 4, "Trawl" = 2, "Biomass" = 6))
```
...
if (extrapolate.biomass) {
  ...
  print("Biomass not extrapolated")
}
```
...
```{r estimate-nearshore-biomass, include=FALSE}
...
#### Data QA/QC
##### Nearshore specimen length/weight plot

```{r lw-plots-nearshore}
if (estimate.ns) {
  if (exists("lw.plot.ns")) {
    # Create interactive plot of nearshore specimen L/W
    if (do.plotly) {
      ggplotly(lw.plot.ns)
    } else {
      lw.plot.ns
    ...
    print("No purse seine length/weight plot.")
  }  
} else {
  print("No nearshore sampling done.")
}
```

##### Nearshore purse seine set locations

```{r set-location-map}
if (estimate.ns) {
  if (file.exists(here("Figs/fig_seine_sets.png"))) {
    include_graphics(here("Figs/fig_seine_sets.png"))
  } else {
    print("No purse seine set location map.")
  }
} else {
  print("No nearshore sampling done.")
}
...
if (combine.regions) {
  ...
  nasc.plot.combo <- nasc.combo %>%
    select(filename, vessel.name, transect.name, transect, 
           int, dist_m, datetime, lat, long, cps.nasc) %>% 
  ...
  if (save.figs) {
    ...
    source(here("Code/combineNascClusters.R"))
...
for (i in unique(strata.final$scientificName)) {
  ...
  if (i == "Clupea pallasii") {
    ...
    nasc.temp <- nasc.temp %>% 
      mutate(num.abund = (cps.nasc*prop.smelt)/(4*pi*sigmaindiv.smelt))
  }
...
(ref:l-disagg-anch-n) Abundance versus standard length (upper panel) and biomass (kg) versus standard length (lower panel) for the northern stock of Northern Anchovy (_Engraulis mordax_) in the survey area.
...
(ref:l-disagg-anch-c) Abundance versus standard length (upper panel) and biomass (kg) versus standard length (lower panel) for the central stock of Northern Anchovy (_Engraulis mordax_) in the survey area.
...
(ref:l-disagg-sar-n) Abundance versus standard length (upper panel) and biomass (kg) versus standard length (lower panel) for the northern stock of Pacific Sardine (_Sardinops sagax_) in the survey area.
...
(ref:l-disagg-sar-s) Abundance versus standard length (upper panel) and biomass (kg) versus standard length (lower panel) for the southern stock of Pacific Sardine (_Sardinops sagax_) in the survey area.
...
(ref:l-disagg-mack) Abundance versus standard length (upper panel) and biomass (kg) versus standard length (lower panel) for Pacific mackerel (_Scomber japonicus_) in the survey area.
...
(ref:l-disagg-jack) Abundance versus standard length (upper panel) and biomass (kg) versus standard length (lower panel) for Jack Mackerel (_Trachurus symmetricus_) in the survey area.
...
(ref:l-disagg-her) Abundance versus standard length (upper panel) and biomass (kg) versus standard length (lower panel) for Pacific herring (_Clupea pallasii_) in the survey area.
...
(ref:l-disagg-rher) Abundance versus standard length (upper panel) and biomass (kg) versus standard length (lower panel) for round herring (_Etrumeus acuminatus_) in the survey area.
...
## Results for whitebait smelt (_Allosmerus elongatus_)
#### Biomass estimates

(ref:biomass-smelt) Biomass estimates (metric tons, t) and their precisions (upper and lower 95% confidence intervals, CI~95%~; and coefficients of variation, CVs) for Whitebait Smelt (_Allosmerus elongatus_) in the core and nearshore survey regions. Stratum areas are nmi^2^.

```{r biomass-smelt}
be.table.sub.all <- be.table.summ %>% 
  filter(Name == "Allosmerus elongatus") %>% 
  mutate(Region = "All")
...
be.table.sub %>% 
  rename("point_estimate" = "$\\hat{B}$",
         "ci_lower" = "CI$_{L,95\\%}$",
         "ci_upper" =	"CI$_{U,95\\%}$") %>%
  write_csv(file = here("Output/biomass_final_Allosmerus elongatus_All.csv"),
            na = "")
...
(ref:biom-dens-smelt) Biomass densities of whitebait smelt (_Allosmerus elongatus_), per strata, throughout the survey region. The blue numbers represent the locations of trawl clusters with at least one herring. The gray line represents the vessel track.

```{r biom-dens-smelt,fig.cap='(ref:biom-dens-smelt)',out.height='100%',fig.pos='H'}
if (combine.regions) {
  if (file.exists(here("Figs/fig_biomass_dens_combo_Allosmerus elongatus-All.png"))) {
    include_graphics(here("Figs/fig_biomass_dens_combo_Allosmerus elongatus-All.png"))  
  } else {
    print("No data for this stock.")
  }
} else {
  if (file.exists(here("Figs/fig_biomass_dens_Allosmerus elongatus-All.png"))) {
    include_graphics(here("Figs/fig_biomass_dens_Allosmerus elongatus-All.png"))  
  } else {
    print("No data for this stock.")
  }
}
...
(ref:l-disagg-smelt) Abundance versus standard length (upper panel) and biomass (kg) versus standard length (lower panel) for whitebait smelt (_Allosmerus elongatus_) in the survey area.

```{r l-disagg-smelt,fig.cap='(ref:l-disagg-smelt)',out.height='100%',fig.pos='H'}
# Insert disaggregated abundance plots for smelt
if (combine.regions) {
  if (file.exists(here("Figs/fig_L_disagg_combo_Allosmerus elongatus-All.png"))) {
    include_graphics(here("Figs/fig_L_disagg_combo_Allosmerus elongatus-All.png"))  
  } else {
    print("No data for this stock.")
  } 
} else {
  if (file.exists(here("Figs/fig_L_disagg_Allosmerus elongatus-All.png"))) {
    include_graphics(here("Figs/fig_L_disagg_Allosmerus elongatus-All.png"))  
  } else {
    print("No data for this stock.")
  }
}
...
#### Abundance-by-length table

(ref:l-freq-summ-smelt) Abundance versus standard length (cm) for for whitebait smelt (_Allosmerus elongatus_).

```{r l-freq-summ-smelt}
# Print table for round herring
if ("Allosmerus elongatus" %in% unique(abund.summ$Species)) {
  abund.summ.table <- abund.summ %>% 
    filter(Species == "Allosmerus elongatus", Stock == "All") %>% 
    select(Stock, SL, abundance) %>% 
    rename(Abundance = abundance,
           '$L_S$' = SL) %>%
    ungroup()
  ...
  print("No whitebait smelt present.")
}
```
...
(ref:rlf-smelt) Standard length frequency distributions of whitebait smelt (_Allosmerus elongatus_) per nighttime trawl cluster, annotated with the number of individuals caught and their percentage contributions to the abundance in each stratum and across all strata (i.e., for the entire survey).

```{r rlf-smelt,out.height='100%',fig.pos='H'}
# Insert relative length frequency plot for round herring
if (file.exists(here("Figs/fig_cluster_relative_length_frequency_Allosmerus elongatus.png"))) {
  include_graphics(here("Figs/fig_cluster_relative_length_frequency_Allosmerus elongatus.png"))  
} else {
  print("No data for this stock.")
}
```
...
# Interactively explore survey data and biomass results  
...
```{r interactive-data-explorer,out.height="12in",out.width="100%"}
# Leaflet options
...
if (process.cufes) {
  # Create color palette for CUFES
...
# Select plot levels for backscatter data
if (exists("nasc.plot.ns")) {
  nasc.levels.all <- sort(unique(c(unique(nasc.plot$bin.level),
                                   unique(nasc.plot.ns$bin.level))))  
} else {
  nasc.levels.all <- sort(unique(nasc.plot$bin.level))
}
...
if (exists("strata.nearshore")){
  strata.nearshore.sub <- strata.nearshore %>% 
    st_collection_extract(type = "POLYGON") %>%
    # filter(!st_geometry_type(.) == "GEOMETRYCOLLECTION") %>% 
    st_transform(crs.geog)
}
...
if (do.imap) {
  i.map <- leaflet() %>% 
    ...
    addLayersControl(
      overlayGroups = c("MPAs", "State Waters", "Exclusive Economic Zones", 
                        ...
                        "Biomass (P. herring)", "Biomass (R. herring)", 
                        "Biomass (Smelt)"),
  ...
  if(nrow(filter(strata.primary.sub, scientificName == "Clupea pallasii")) > 0)
    i.map <- i.map %>%
      addPolygons(data = filter(strata.primary.sub, scientificName == "Clupea pallasii"), 
                  weight = 2, color = "#000414", fillColor = ~cpsPal(scientificName),
                  # weight = 2, color =  ~cpsPal(scientificName), fillColor = ~cpsPal(scientificName),
                  fillOpacity = 0.25,
                  group = "Biomass (P. herring)") %>% 
      ...
      addLegend("bottomleft", colors = dens.colors,
                ...
                opacity = 1, group = "Biomass (P. herring)") 
  ...
  if(nrow(filter(strata.primary.sub, scientificName == "Allosmerus elongatus")) > 0)
    i.map <- i.map %>%
      addPolygons(data = filter(strata.primary.sub, scientificName == "Allosmerus elongatus"),
                  weight = 2, color = "#000414", fillColor = ~cpsPal(scientificName),
                  # weight = 2, color =  ~cpsPal(scientificName), fillColor = ~cpsPal(scientificName),
                  fillOpacity = 0.25,
                  group = "Biomass (Smelt)") %>%
      addCircleMarkers(data = filter(nasc.density.sf, scientificName == "Allosmerus elongatus"),
                       radius = ~bin.level*2, color = "#000414", stroke = TRUE, weight = 1,
                       label = ~label, popup = ~popup,
                       fillOpacity = 0.75, fillColor =  ~densPal(bin.level),
                       group = "Biomass (Smelt)") %>%
  ...
  # Add nearshore biomass polygons
  if (exists("strata.nearshore")){
  if(nrow(filter(strata.nearshore.sub, scientificName == "Clupea pallasii")) > 0)
    i.map <- i.map %>% 
      addPolygons(data = filter(strata.nearshore.sub, scientificName == "Clupea pallasii"), 
                  weight = 2, color = "#000414", fillColor = ~cpsPal(scientificName),
                  # weight = 2, color = ~cpsPal(scientificName), fillColor = ~cpsPal(scientificName),
                  fillOpacity = 0.5, group = "Biomass (P. herring)") %>% 
      addCircleMarkers(data = filter(nasc.density.ns.sf, scientificName == "Clupea pallasii"),
                       radius = ~bin.level*2, color = "white", stroke = TRUE, weight = 1,
                       fillOpacity = 0.75, fillColor =  ~densPal(bin.level),
                       label = ~label, popup = ~popup, 
                       group = "Biomass (P. herring)") %>% 
      addLegend("bottomleft", colors = dens.colors,
                values = sort(unique(nasc.density.ns.sf$bin.level)),
                labels = dens.labels,
                title = "Biomass Density (Herring) <br/> (t nmi<sup>-2</sup>)",
                opacity = 1, group = "Biomass (P. herring)")
  
  if(nrow(filter(strata.nearshore.sub, scientificName == "Engraulis mordax")) > 0)
    i.map <- i.map %>% 
      addPolygons(data = filter(strata.nearshore.sub, scientificName == "Engraulis mordax"), 
                  weight = 2, color = "#000414", fillColor = ~cpsPal(scientificName),
                  # weight = 2, color = ~cpsPal(scientificName), fillColor = ~cpsPal(scientificName),
                  fillOpacity = 0.5, group = "Biomass (Anchovy)",
                  label = ~paste(scientificName, stock, stratum))  %>% 
      addCircleMarkers(data = filter(nasc.density.ns.sf, scientificName == "Engraulis mordax"),
                       radius = ~bin.level*2, color = "white", stroke = TRUE, weight = 1,
                       fillOpacity = 0.75, fillColor =  ~densPal(bin.level),
                       label = ~label, popup = ~popup, 
                       group = "Biomass (Anchovy)") %>% 
  ...
  if(nrow(filter(strata.nearshore.sub, scientificName == "Sardinops sagax")) > 0)
    i.map <- i.map %>% 
      addPolygons(data = filter(strata.nearshore.sub, scientificName == "Sardinops sagax"), 
                  weight = 2, color = "#000414", fillColor = ~cpsPal(scientificName),
                  # weight = 2, color = ~cpsPal(scientificName), fillColor = ~cpsPal(scientificName),
                  fillOpacity = 0.5, group = "Biomass (Sardine)")  %>% 
      addCircleMarkers(data = filter(nasc.density.ns.sf, scientificName == "Sardinops sagax"),
                       radius = ~bin.level*2, color = "white", stroke = TRUE, weight = 1,
                       fillOpacity = 0.75, fillColor =  ~densPal(bin.level),
                       label = ~label, popup = ~popup, 
                       group = "Biomass (Sardine)") %>% 
  ...
  if(nrow(filter(strata.nearshore.sub, scientificName == "Scomber japonicus")) > 0)
    i.map <- i.map %>% 
      addPolygons(data = filter(strata.nearshore.sub, scientificName == "Scomber japonicus"), 
                  weight = 2, color = "#000414", fillColor = ~cpsPal(scientificName),
                  # weight = 2, color = ~cpsPal(scientificName), fillColor = ~cpsPal(scientificName),
                  fillOpacity = 0.5, group = "Biomass (P. mackerel)")  %>% 
      addCircleMarkers(data = filter(nasc.density.ns.sf, scientificName == "Scomber japonicus"),
                       radius = ~bin.level*2, color = "white", stroke = TRUE, weight = 1,
                       fillOpacity = 0.75, fillColor =  ~densPal(bin.level),
                       label = ~label, popup = ~popup, 
                       group = "Biomass (P. mackerel)") %>% 
  ...
  if(nrow(filter(strata.nearshore.sub, scientificName == "Trachurus symmetricus")) > 0)
    i.map <- i.map %>% 
      addPolygons(data = filter(strata.nearshore.sub, scientificName == "Trachurus symmetricus"), 
                  weight = 2, color = "#000414", fillColor = ~cpsPal(scientificName),
                  # weight = 2, color = ~cpsPal(scientificName), fillColor = ~cpsPal(scientificName),
                  fillOpacity = 0.5, group = "Biomass (J. mackerel)")  %>% 
      addCircleMarkers(data = filter(nasc.density.ns.sf, scientificName == "Trachurus symmetricus"),
                       radius = ~bin.level*2, color = "white", stroke = TRUE, weight = 1,
                       fillOpacity = 0.75, fillColor =  ~densPal(bin.level),
                       label = ~label, popup = ~popup, 
                       group = "Biomass (J. mackerel)") %>% 
      addLegend("bottomleft", colors = dens.colors,
                values = sort(unique(nasc.density.ns.sf$bin.level)),
                labels = dens.labels,
                title = "Biomass Density (J. mackerel) <br/> (t nmi<sup>-2</sup>)",
                opacity = 1, group = "Biomass (J. mackerel)")
  
  if(nrow(filter(strata.nearshore.sub, scientificName == "Etrumeus acuminatus")) > 0)
    i.map <- i.map %>% 
      addPolygons(data = filter(strata.nearshore.sub, scientificName == "Etrumeus acuminatus"), 
                  weight = 2, color = "#000414", fillColor = ~cpsPal(scientificName),
                  # weight = 2, color = ~cpsPal(scientificName), fillColor = ~cpsPal(scientificName),
                  fillOpacity = 0.5, group = "Biomass (R. herring)")  %>% 
      addCircleMarkers(data = filter(nasc.density.ns.sf, scientificName == "Etrumeus acuminatus"),
                       radius = ~bin.level*2, color = "white", stroke = TRUE, weight = 1,
                       fillOpacity = 0.75, fillColor =  ~densPal(bin.level),
                       label = ~label, popup = ~popup, 
                       group = "Biomass (R. herring)") %>% 
      addLegend("bottomleft", colors = dens.colors,
                values = sort(unique(nasc.density.ns.sf$bin.level)),
                labels = dens.labels,
                title = "Biomass Density (R. herring) <br/> (t nmi<sup>-2</sup>)",
                opacity = 1, group = "Biomass (R. herring)")
  
  if(nrow(filter(strata.nearshore.sub, scientificName == "Allosmerus elongatus")) > 0)
    i.map <- i.map %>% 
      addPolygons(data = filter(strata.nearshore.sub, scientificName == "Allosmerus elongatus"), 
                  weight = 2, color = "#000414", fillColor = ~cpsPal(scientificName),
                  # weight = 2, color = ~cpsPal(scientificName), fillColor = ~cpsPal(scientificName),
                  fillOpacity = 0.5, group = "Biomass (Smelt)")  %>% 
      addCircleMarkers(data = filter(nasc.density.ns.sf, scientificName == "Allosmerus elongatus"),
                       radius = ~bin.level*2, color = "white", stroke = TRUE, weight = 1,
                       fillOpacity = 0.75, fillColor =  ~densPal(bin.level),
                       label = ~label, popup = ~popup, 
                       group = "Biomass (Smelt)") %>% 
      addLegend("bottomleft", colors = dens.colors,
                values = sort(unique(nasc.density.ns.sf$bin.level)),
                labels = dens.labels,
                title = "Biomass Density (Smelt) <br/> (t nmi<sup>-2</sup>)",
                opacity = 1, group = "Biomass (Smelt)")
  }
  ...
  ## Nearshore
  if (exists("nasc.plot.ns.sf")){
    if (nrow(filter(nasc.plot.ns.sf, NASC < 200)) > 0)
      i.map <- i.map %>%  
        addCircleMarkers(data = filter(nasc.plot.ns.sf, NASC < 200), 
                         radius = ~bin.level*2, color = "white", stroke = TRUE, weight = 1,
                         fillOpacity = 0.75,  fillColor = ~nascPal(bin.level), 
                         label = ~label, popup = ~popup, 
                         group = "Backscatter-CPS (Small)")
  ...
  i.map <- i.map %>% 
    hideGroup(c(
      "State Waters","MPAs","Exclusive Economic Zones","Graticule",
...
## Per species (free axes)

(ref:biomass-ts-line-facet) Estimated biomasses of the `r num2words(n_distinct(be.table$Stock))` most abundant CPS populations or subpopulations of `r num2words(n_distinct(be.table$Name))` species in the CCE during summer since 2008. Surveys typically span the area between Cape Flattery and San Diego, but in some years also include Vancouver Island, Canada (2015-2019, 2024) and portions of Baja CA (2021-2022, 2024). Error bars are 95% confidence intervals. **Note: different y-axis limits per species.**

```{r biomass-ts-line-facet,fig.cap='(ref:biomass-ts-line-facet)',out.height='100%',fig.pos='H'}
if (file.exists(here("Figs/fig_biomass_ts_line_facet.png"))) {
  include_graphics(here("Figs/fig_biomass_ts_line_facet.png"))  
} else {
...
## Entire community
(ref:biomass-ts-combo) a) Estimated and b) cumulative estimated biomasses of the `r num2words(n_distinct(be.table$Stock))` most abundant CPS populations or subpopulations of `r num2words(n_distinct(be.table$Name))` species in the CCE during summer since 2008. Surveys typically span the area between Cape Flattery and San Diego, but in some years also include Vancouver Island, Canada (2015-2019, 2024) and portions of Baja CA (2021-2022, 2024).

```{r biomass-ts-combo,fig.cap='(ref:biomass-ts-combo)',out.height='100%',fig.pos='H'}
...
if (sum(!is.na(cal.vessels) > 0)) {
  if (process.cal) {
    ...
    # Extract calibration results from past surveys
    ...
    # Load CW calibration results from past surveys
...
if (exists(paste0("all.output.", survey.vessel.primary))) {
  # Assign data frame using the name of the primary survey vessel
  assign("cal.results", eval(parse(text = paste0("all.output.", survey.vessel.primary))))
