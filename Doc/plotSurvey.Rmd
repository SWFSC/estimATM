...
if (copy.files) {
  ...
  if (sampling.trawl) {
    ...
    haul.db <- dir_ls(file.path(survey.dir[survey.vessel.primary], trawl.dir),
...
if (sampling.trawl) {
  ...
  # Match hauls headers to nav data
  ...
  for (i in seq_along(haul$equilibriumTime)) {
    min.diff.eq       <- which.min(abs(difftime(haul$equilibriumTime[i], nav.match.position$time)))
    nav.match.haul.eq <- bind_rows(nav.match.haul.eq, nav.match.position[min.diff.eq, ])
  }
  
  for (i in seq_along(haul$haulBackTime)) {
    min.diff.hb       <- which.min(abs(difftime(haul$haulBackTime[i], nav.match.position$time)))
