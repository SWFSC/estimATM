...
pacman::p_load(tidyverse,swfscMisc,lubridate,cowplot,here,marmap,fs,sf, odbc,
               mapview,mapdata,photobiology,ggmap,knitr,bookdown,tcltk,RSQLite,
               rnaturalearth,ggspatial,kableExtra,geosphere,janitor,DT,xts,xml2)
...
nav.source    <- "RosePoint" # Navigation data source: ERDDAP or SCS
...
extract.bathy <- T # Extract bathymetry for each nav point
...
# Limit nav data if containing data outside of the survey period
if (exists("nav.limits")) {
  nav <- nav %>% 
    filter(between(date(time), min(nav.limits), max(nav.limits)))
}
...
if (extract.bathy) {
  ...
  nav.depth <- get.depth(noaa.bathy, nav$long, nav$lat, locator = F, distance = T) %>%
    rename(long = lon) %>%
    bind_cols(select(nav, time, dist, SOG)) %>%
  ...
  write_csv(nav.depth, here("Output/nav_bathy_Lasker.csv"))
...
nav.summ <- nav.depth %>% 
  ...
  summarise(
    dist_km  = round(sum(dist, na.rm = TRUE)),
    pings_ek = round(sum(pings, na.rm = TRUE))) %>% 
  mutate(
    dist_nmi = round(dist_km * 0.539957)) %>% 
  select(depth_bin:dist_km, dist_nmi, everything())

nav.sog.summ <- nav.depth %>% 
  filter(!is.nan(dist), SOG > 5) %>% 
  mutate(date = date(time)) %>% 
  group_by(date, day_night) %>% 
  summarise(
    SOG  = mean(SOG)) 

nav.sog.summ.all <- nav.depth %>% 
  filter(!is.nan(dist), SOG > 5) %>% 
  mutate(date = date(time)) %>% 
  group_by(day_night) %>% 
  summarise(
...
write_csv(nav.sog.summ, here("Output", paste0(survey.name, "_SOG_Summary_", survey.vessel, ".csv")))
```
