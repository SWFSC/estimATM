...
haul <- filter(haul.all, cruise %in% cruise.name) %>% 
  ...
  mutate(duration = round(difftime(haulBackTime, equilibriumTime, units = "mins"), 1), 
...
if (exists("set.catch")) {
  ...
  set.catch.table <- set.catch.ca %>% 
    ...
    select(key.set, commonName, blank1, scientificName, blank2, county, MPA, 
           ...
           method, disposition, facility) %>% 
    mutate(datetime = format(ymd_hms(datetime), "%m/%d/%Y"))
...
if(exists("set.lengths")) {
  ...
  set.specimen.summ <- set.specimens.ca %>% 
    ...
    select(vessel.name, key.set, commonName, scientificName, weightg) %>% 
...
if (nrow(haul.mpa) > 0) {
  leaflet() %>% 
    ...
    addPolylines(data = haul.paths, color = ~trawlPathPal(period), weight = 5, opacity = 0.8, 
...
if (exists("set.catch.table")) {
  if (nrow(set.catch.table) > 0) {
    set.catch.table %>% 
    ...
    print("No seine catches in CA waters.")
  }
} else {
  print("No seine catches to date.")
}
```
...
```{r specimen-summary}
if (nrow(specimen.summ) > 0) {
  specimen.summ %>% 
    adorn_totals() %>% 
    datatable(rownames = FALSE)  
} else {
  print("No specimens to date.")
}
```
