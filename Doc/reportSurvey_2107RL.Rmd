...
output: 
  bookdown::pdf_document2:
    ...
    toc: no
    includes: 
      in_header: yaml/header_2107RL.tex
...
# with variables that might also be used by the reportBiomass
...
# Specify number of significant digits for beam model results
...
if ("LBC" %in% cal.vessels) {
  ...
  # Specify number of significant digits for beam model results
  bm.res.lbc$bm_sa_corr <- formatC(bm.res.lbc$bm_sa_corr,format="f",digits=2)
  bm.res.lbc$dev_bm_rms <- formatC(bm.res.lbc$dev_bm_rms,format="f",digits=2)
  ...
  names(cal.params.lbc) <- c("Frequency","Model","Serial Number","Transmit Power ($p_\\mathrm{et}$)",
                             ...
                             "3-dB Beamwidth Athw. ($\\beta_\\mathrm{-3dB}$)",
                             "Angle Offset Along. ($\\alpha_{0}$)","Angle Offset Athw. ($\\beta_{0}$)",
                             "Theoretical TS ($TS_\\mathrm{theory}$)")#,"Ambient Noise")
...
if ("LM" %in% cal.vessels) {
  ...
  # Specify number of significant digits for beam model results
  bm.res.lm$bm_sa_corr <- formatC(bm.res.lm$bm_sa_corr,format="f",digits=2)
  bm.res.lm$dev_bm_rms <- formatC(bm.res.lm$dev_bm_rms,format="f",digits=2)
...
if ("JCF" %in% cal.vessels) {
  ...
  # Specify number of significant digits for beam model results
  bm.res.jcf$bm_sa_corr <- formatC(bm.res.jcf$bm_sa_corr,format="f",digits=2)
  bm.res.jcf$dev_bm_rms <- formatC(bm.res.jcf$dev_bm_rms,format="f",digits=2)
...
```{r plot-cal-pings-jcf}
if ("JCF" %in% cal.vessels) {
  # Get ping data from current survey and format for plotting
  cal.pings.jcf <- cal.pings.jcf %>% 
    mutate(cal_date = date(date_time)) %>% 
    # filter(between(cal_date,
    #                ymd(cal.plot.date) - days(cal.window),
    #                ymd(cal.plot.date) + days(cal.window))) %>% 
  ...
  # Set axis limits based on range of ping angles
  cal.lim.tmp <- round(max(max(cal.pings.jcf$along), max(cal.pings.jcf$athw))) 
  ...
  # subset only outlier points
  outliers <- filter(cal.pings.jcf, outlier == 1)
  
  cal.pings.jcf <- cal.pings.jcf %>% 
    left_join(select(cal.res.jcf,txdr_freq,txdr_type,target_ts,
                     txdr_gain,bm_txdr_gain,
                     bm_alon_ba,bm_athw_ba,
                     bm_alon_oa,bm_athw_oa)) %>% 
    mutate(
  ...
  if (cal.scales == "fixed") {
    # Plot beam-uncompensated target strength data #####
    tsu.scatter.jcf <- ggplot(filter(cal.pings.jcf, outlier == 0), aes(athw, along)) +
      geom_point(aes(colour = TS_u)) + 
      geom_point(data = filter(cal.pings.jcf, outlier == 1), aes(athw, along), 
                 shape = "+", size = 1, alpha = 0.7) +
      facet_wrap(~txdr_type, scales = cal.scales) +
      scale_colour_viridis(name = expression(paste(italic(TS)[u]," (dB)",sep = "")),
                           option = "magma") +
    ...
    # Plot beam-compensated target strength data #####
    tsc.scatter.jcf <- ggplot(filter(cal.pings.jcf, outlier == 0), aes(athw, along)) +
      geom_point(aes(fill = relTS_c_scaled), shape = 21) + 
      geom_point(data = filter(cal.pings.jcf, outlier == 1), aes(athw, along),
                 shape = "+", size = 4) +
      facet_wrap(~txdr_type, scales = cal.scales) + 
  ...
  # Save TS_u plot 
  ggsave(tsu.scatter.jcf, filename = here("Figs/fig_cal_TSu_scatter_JCF.png"),  
         width = 9, height = 8)
  
  # Save TS_c plot 
  ggsave(tsc.scatter.jcf, filename = here("Figs/fig_cal_TSrel_scatter_JCF.png"), 
         width = 9, height = 8)
}
```
...
**Report on the `r survey.name.long` (`r survey.name`), `r survey.start` to `r survey.end` `r survey.year`, conducted aboard NOAA ship _`r survey.vessel.long`_, Mexican research vessel _Dr. Jorge Carranza Fraser_, fishing vessels _Lisa Marie_ and _Long Beach Carnage_, and uncrewed surface vehicles**  

Josiah S. Renfree^1^, Noelle M. Bowlin^1^, Brad E. Erisman^1^, Ramón Isaac Rojas-González^2^, Gabriel E. Johnson^1,3^, Scott A. Mau^1^, David W. Murfin^1^, Brittany D. Schwartzkopf^1^, Thomas S. Sessions^1^, Kevin L. Stierhoff^1^, Juan Roberto F. Vallarta-Zárate^2^, Lanora Vasquez^1^, William Watson^1^, Juan P. Zwolinski^1,4^, and David A. Demer^1^
...
^2^Instituto Nacional de Pesca y Acuacultura (INAPESCA)   
Mexico Avenue 190   
Del Carmen, Coyoacán    
...
(ref:vessel-pic) NOAA ship _`r survey.vessel`_ (top left), RV _Dr. Jorge Carranza Fraser_ (bottom left), F/V _Long Beach Carnage_ (top right), F/V _Lisa Marie_ (middle right), and an uncrewed surface vehicle (Saildrone USV, bottom right).

```{r vessel-pic, fig.cap='(ref:vessel-pic)', out.height='5.0in', fig.pos='H'}
include_graphics(here("Images/img_vessels_2107RL_V2.png"))
```

\newpage  

## Scientific Personnel {#introduction-personnel}
The collection and analysis of the survey data were conducted by members of 1-NOAA, 2-INAPESCA, 3-Maz Sardina, and 4-volunteers. Asterisks denote Chief Scientists.  
...
* _Lasker_
  + Leg I:   J. Renfree^1\*^ and G. Johnson^1^
  + Leg II:  S. Mau^1^, E. Pérez-Flores^2^, and S. Dolan^4^
  + Leg III: J. Zwolinski^1\*^, D. Murfin^1^, and L. Altamirano-López^2^
  + Leg IV:  K. Stierhoff^1\*^, S. Sessions^1^, and R. Vallarta-Zárate^2^

* _Carranza_
  + M. Vásquez-Ortiz^2^ and L. Altamirano-López^2^
  
**Trawl Sampling:**

* _Lasker_
  + Leg I:   M. Craig^1^, L. Heberer^1^, P. Kuriyama^1^, and S. Hensman^4^
  + Leg II:  B. Schwartzkopf^1^, L. Martin^1^, B. Bellerud^1^, J. Barnes^4^, and D. Hernández-Cruz^2^
  + Leg III: O. Snodgrass^1^, E. Adams^1^, C. Fahy^1^, Z. Skelton^1^, and J. Walker^1^
  + Leg IV:  K. James^1^, K. Koch^1^, K. Lane^1^, D. Lowry^1^, and J. Osuna-Soto^3^

* _Carranza_
  + D. Hernández-Cruz^2^, L. Huidobro-Campos^2^, A. Lizárraga-Rodríguez^2^, and S. Padilla-Galindo^2^

**CUFES Sampling:**

* _Lasker_
  + Leg I:   E. Gardner^1^ and A. Freire de Carvalho^1^
  + Leg II:  W. Watson^1\*^ and M. Human^1^
  + Leg III: E. Gardner^1^ and S. Morales-Gutiérrez^2^
  + Leg IV:  L. Vasquez^1^ and V. Martínez-Magaña^2^
