...
process.tv80 <- F # Process Simrad TV80 data
...
```{r summarise-tdr-metrics,eval=FALSE}
...
# tdr.summ.haul %>% group_by(gear.type, duration.bin) %>% tally()
...
tdr.summ.all.gear <- tdr.summ.all %>% 
  group_by(gear.type) %>% 
  summarise(kite.depth = mean(kite.depth),
            footrope.depth = mean(footrope.depth),
            height = mean(mean.height))

tdr.summ.all.gear <- tdr.fishing %>% 
  group_by(gear.type) %>% 
  summarise(kite.depth = mean(Kite),
            footrope.depth = mean(Footrope),
            height = mean(Kite - Footrope), 
            area = mean(A)) %>% 
  arrange(gear.type)
...
```{r plot-fishing-data,eval=FALSE}
...
```{r plot-tdr-metrics-cruise,eval=FALSE}
