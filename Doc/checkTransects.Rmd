...
pacman::p_load(tidyverse,sf,mapview,lubridate,here,knitr,DT,bookdown,
               ...
               shadowtext,stplanr,tmaptools,patchwork)
...
load.data          <- T # Load CPS density data from {atmData}
...
## Write transect summary to CSV
transects.sf %>% 
  st_set_geometry(NULL) %>% 
  left_join(daylength.df) %>% 
  # select(Type:brg) %>% 
  write_csv(here("Output/transect_summary.csv"))
...
if (leaflet.checkTransects.simple) {
  ...
  i.map <- leaflet() %>% 
    ...
    hideGroup(c("CTD Stations","UCTD Stations", "State Waters")) %>%
...
Orange points represent UCTD stations. Blue points represent CTD stations. Green stations represent surface eDNA stations.
...
dens.plot.sar <- base.map +
  geom_point(data = filter(nasc.dens.all, sar.dens > 0), 
             aes(X, Y),
             # aes(X, Y, size = sar.dens),
             colour = sardine.color, alpha = 0.2, size = 0.5,
             show.legend = FALSE) +
...
dens.plot.mack <- base.map +
  geom_point(data = filter(nasc.dens.all, mack.dens > 0), 
             aes(X, Y),
             # aes(X, Y, size = mack.dens),
             colour = pac.mack.color, alpha = 0.2, size = 0.5,
             show.legend = FALSE) +
  scale_colour_viridis_c() +
  guides(colour = guide_legend(), 
         size = guide_legend()) +
...
dens.plot.jack <- base.map +
  geom_point(data = filter(nasc.dens.all, jack.dens > 0), 
             aes(X, Y),
             # aes(X, Y, size = jack.dens),
             colour = jack.mack.color, alpha = 0.2, size = 0.5,
             show.legend = FALSE) +
  scale_colour_viridis_c() +
  guides(colour = guide_legend(), 
         size = guide_legend()) +
...
dens.plot.anch <- base.map +
  geom_point(data = filter(nasc.dens.all, anch.dens > 0), 
             aes(X, Y),
             # aes(X, Y, size = anch.dens),
             colour = anchovy.color, alpha = 0.2, size = 0.5,
             show.legend = FALSE) +
  scale_colour_viridis_c() +
  guides(colour = guide_legend(), 
         size = guide_legend()) +
...
# Display map
if (here("Figs/fig_survey_plan_cps_distribution.png")) {
