...
get.db             <- F # Import trawl database
...
process.csv.all    <- T # Process all CSV files (F = new files only)
...
scale.pies         <- F # Scale catch pies to by total weight
...
if (exists("uctd.summ")|exists("ctd.summ")) {
  if (save.figs) {
    ...
    if (exists("haul.mid")) {
      haul.plot <- project_df(ungroup(haul.mid), to = crs.proj)
