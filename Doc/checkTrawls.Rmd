...
# lengths.all$totalLength_mm <- totalLength_mm + 5 
...
salmon.hauls <- filter(haul.all, haul %in% unique(salmon.catch$haul))
```
...
plot.evolution <- ggplot(data = haul) + 
  ...
  geom_point(aes(haul, as.numeric(evolutionTime), fill = trawlPerformance), 
...
imap.bounds <- map_bounds(haul$startLatDecimal, haul$startLongDecimal, 0.1) 
...
if (exists("nav.paths.sf")) {
  i.map <- leaflet() %>% 
    ...
    addCircleMarkers(data = cluster.catch.sf, radius = 5, color = "#000000", stroke = TRUE, weight = 2,
                     ...
                     popup = ~popup, label = ~label,
                     group =  "Trawls") %>%
    # Add trawls with procted salmon catch
    addCircleMarkers(data = filter(haul.locs.sf, haul %in% salmon.hauls$haul), 
    ...
    hideGroup(c("Exclusive Economic Zones","Trawls-Salmon",
...
haul.summary %>% 
  ...
  kable_styling(bootstrap_options = c("striped","hover","condensed"),
                full_width = FALSE) %>%
  row_spec(0, align = c("c")) %>% 
  add_header_above(c(" " = 1, "Trawls/night" = 3, " " = 2))
