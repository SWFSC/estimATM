...
pacman::p_load(tidyverse,cowplot,knitr,shadowtext,sf,fs,vroom,leaflet,ggiraph,
               DBI,stringr,gdata,forecast,lubridate,here,rnaturalearth,viridis,
               DT,mapview,bookdown,ggspatial,leafem,leaflet.extras,xts,plotly)
...
# Plotting options
do.plotly       <- F # Set to F if having issues rendering Leaflet maps
```
...
```{r i-map-uctd, out.width='100%',eval=TRUE}
...
```{r uctd-depth-time-plot,out.width="100%"}
# ,out.height="8in"
if (exists("all.uctd.casts")) {
  if (length(all.uctd.casts) > 0) {
    if (save.figs) {
...
```{r uctd-temp-depth-plot,out.width="100%"}
if (exists("all.uctd.casts")) {
  if (length(all.uctd.casts) > 0) {
    if (save.figs) {
...
```{r uctd-sal-depth-plot,out.width="100%"}
if (exists("all.uctd.casts")) {
  if (length(all.uctd.casts) > 0) {
    if (save.figs) {
...
```{r uctd-sv-depth-plot,out.width="100%"}
if (exists("all.uctd.casts")) {
  if (length(all.uctd.casts) > 0) {
    if (save.figs) {
...
```{r uctd-chl-depth-plot,out.width="100%"}
if (exists("all.uctd.casts")) {
  if (length(all.uctd.casts) > 0) {
...
```{r ctd-temp-depth-plot,out.width="100%"}
if (exists("all.ctd.casts")) {
  if (length(all.ctd.casts) > 0) {
    if (save.figs) {
      # plot temperature v depth
...
```{r ctd-sal-depth-plot,out.width="100%",out.height="8in"}
if (exists("all.ctd.casts")) {
  if (length(all.ctd.casts) > 0) {
    if (save.figs) {
      # plot salinity v depth
...
```{r ctd-sv-depth-plot,out.width="100%"}
if (exists("all.ctd.casts")) {
  if (length(all.ctd.casts) > 0) {
    if (save.figs) {
      # plot sound speed v depth
...
```{r i-map-all-ctds, out.width='100%',eval=TRUE}
# Create color palette for planned transects
txPal    <- colorFactor(wpt.colors, wpt.types)
