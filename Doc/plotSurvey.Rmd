...
if (sampling.trawl) {
  ...
  for (h in unique(haul$haul)) {
    ...
    nav.fishing <- filter(nav, between(time, haul.tmp$equilibriumTime, haul.tmp$haulBackTime)) %>% mutate(period = "Fishing")
    nav.deploy  <- filter(nav, between(time, haul.tmp$netInWaterTime, haul.tmp$equilibriumTime)) %>% mutate(period = "Deployment")
    nav.recover <- filter(nav, between(time, haul.tmp$haulBackTime, haul.tmp$netOnDeckTime)) %>% mutate(period = "Recovery")
...
trawlPathPal <- colorFactor(c("green","black","red"), c("Deployment","Fishing","Recovery"))
...
A map showing the vessel track (gray lines), acoustic transects (black lines), and the locations of nighttime surface trawls (red), underway CTD (UCTD) casts (blue), and CTD casts (green, if present).  
