...
pacman::p_load(tidyverse,lubridate,mapview,kableExtra,leaflet,gdtools,
...
haul.bad %>% 
  ...
  select(haul, trawlDate, trawlPerformance) %>% #orderOcc
  datatable(rownames = FALSE)
```
...
haul.summary %>% 
  ...
  kable_styling(bootstrap_options = c("striped","hover","condensed"),
                full_width = FALSE) %>%
  row_spec(0, align = c("c")) %>% 
  add_header_above(c(" " = 1, "Trawls/night" = n_distinct(haul.summary$nHauls), " " = 2))
