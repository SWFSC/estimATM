...
if (!is.null(cal.vessels)) {
  ...
  for (i in cal.vessels) {
    ...
    # debugonce(atm::extract_cal_ecs)
    cal.ECS <- list.files(cal.dir[i], 
                          pattern = paste0(str_extract(survey.name, "^\\d{4}"), i, "_Template.ecs"), 
                          full.names = TRUE) %>%
      atm::extract_cal_ecs() %>% 
      arrange(Frequency)
    ...
    cal.params <- select(cal.ECS,
                         ...
                         "Transmitted Power ($p_\\mathrm{et}$)" = TransmittedPower,
