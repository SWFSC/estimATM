...
get.db.ts        <- T # Get biomass time series data from database
...
biomass.pct.rher <- biomass.spp.summ %>% 
  ...
  arrange(year)
# Summarize percent total biomass for W. smelt
biomass.pct.smelt <- biomass.spp.summ %>% 
  filter(species == "Allosmerus elongatus") %>% 
  arrange(year)
```

\pagenumbering{gobble}

**DISTRIBUTION, BIOMASS, AND DEMOGRAPHICS OF COASTAL PELAGIC FISHES IN THE CALIFORNIA CURRENT ECOSYSTEM DURING SUMMER 2026 BASED ON ACOUSTIC-TRAWL SAMPLING**
...
Presented here are:  1) a detailed description of the ATM used to survey CPS in the California Current Ecosystem (CCE) off the west coast of the U.S.; and 2) estimates of the abundances, biomasses, spatial distributions, and demographics of CPS, specifically the northern and southern subpopulations of Pacific Sardine, the central and northern subpopulations of Northern Anchovy, Pacific Mackerel, Jack Mackerel, and Pacific Herring for the core and nearshore survey regions in which they were sampled. Japanese Sardine (_Sardinops melanosticta_) were **[likely?]** present in the survey area during the survey period [Longo2026]. However, due to their phenotypic resemblance and lack of genetic identification at the time biomasses were estimated, all sardine were considered Pacific Sardine (_S. sagax_). Round Herring (_Etrumeus acuminatus_) and smelt (particularly Whitebait Smelt, _Allosmerus elongatus_) are sometimes present in the survey; while their biomasses are estimated, they are not presented here.  
...
The planned core region transects were perpendicular to the coast, extending from the shallowest navigable depth (~20 m) to either a distance of 35 nmi or to the 1,000 ftm isobath, whichever is farther offshore (**Fig. \@ref(fig:survey-plan)**). Compulsory transects were spaced 20 nmi apart in the Southern CA Bight (SCB) and 15 nmi apart between Pt. Conception and Cape Flattery. When CPS were observed within the westernmost 3 nmi of a transect, that transect and the next one to the north were extended in 5-nmi increments until no CPS were observed in the last 3 nmi of the extension, to a maximum extension of 50 nmi.
...
if (exists(paste("all.output", survey.vessel.primary, sep = "."))) {
  # Copy calibration results
  cal.results <- get(paste("all.output", survey.vessel.primary, sep = "."))
...
if (exists("all.output.LBC")) {
  # Copy calibration results
  cal.results <- get("all.output.LBC")
...
**[Review this given possible changes to the EV template in 2026.]**
...
if (file.exists(here("Figs/fig_nasc_cluster_map_combo.png"))) {
  include_graphics(here("Figs/fig_nasc_cluster_map_combo.png"))
} else if (file.exists(here("Figs/fig_nasc_cluster_map.png"))) {
  include_graphics(here("Figs/fig_nasc_cluster_map.png"))
} else {
  print("No figure present")
}
```  
...
if (file.exists(here("Images/img_sardine_habitat_composite_2606RL_newModel_AllVessels.png"))) {
  include_graphics(here("Images/img_sardine_habitat_composite_2606RL_newModel_AllVessels.png"))
...
The nearshore region spanned an area from 5-m depth to approximately 5 nmi from the continental coast, or 2.5 nmi from the Santa Cruz and Santa Catalina Islands, between San Diego and Cape Flattery. _Long Beach Carnage_ surveyed from approximately San Diego to Half Moon Bay, CA and around the Santa Cruz and Santa Catalina Islands, with `nrow(nasc.summ.lbc.ns)` east-west transects totaling `sprintf("%.0f", sum(nasc.summ.lbc.ns$distance))` nmi and `set.summ$n[set.summ$vessel.name == "LBC"]` purse-seine sets during `das.lbc` DAS (**Fig. \@ref(fig:nasc-seine-lbc)**). _Lisa Marie_ surveyed from Half Moon Bay to Cape Flattery, WA with `nrow(nasc.summ.lm.ns)` east-west transects totaling `sprintf("%.0f", sum(nasc.summ.lm.ns$distance))` nmi and `set.summ$n[set.summ$vessel.name == "LM"]` purse-seine sets during `das.lm` DAS (**Fig. \@ref(fig:nasc-seine-lm)**). In the nearshore region, one to `num2words(max(as.numeric(be.ns$Stratum),na.rm = TRUE))` post-survey strata were defined by their transect spacing and the densities of biomass attributed to each species.
...
<!-- https://docs.google.com/document/d/1brlI4uXMfdw5mfewRfdAoZAE4WgbCdjrjqkhapuuV6U/edit?tab=t.add429ox13eh#heading=h.2rrrnzuvnbgo -->

**Leg I**

On 18 June, following a one day delay due to needed repairs on Generator #2, Lasker departed from 10th Avenue Marine Terminal in San Diego, CA at ~1700 (all times UTC). The first day of operations included a certification of the newly repaired bow thruster outside of San Diego Bay, after which the ABS inspectors were put ashore via small craft. For the remainder of 18 June, engineers from AML Oceanographic conducted testing and training of the newly installed Moving Vessel Profiler (MVP200). At 1230 on 19 June, the AML engineers disembarked via small craft outside Mission Bay. Lasker then transited to the first nearshore station, arriving at ~1500 to begin acoustic sampling along transect 001. Transect sampling then progressed northward throughout the Southern California Bight, utilizing regional sampling in order to ensure adequate trawl coverage in both the inshore and offshore regions. No surface trawls were conducted on the night of 30 June due to recent catches of marine mammals that approached the authorized take limit. On 1 July, the final acoustic and eDNA sampling of Leg 1 was conducted, completing all sampling through transect 014 off Pt. Conception. Lasker then transited back to San Diego, arriving at 10th Avenue Marine Terminal at ~2100 on 2 July to complete Leg 1. 

**Leg II**

On 6 July 1700 UTC, Lasker departed from 10th Avenue Marine Terminal to begin Leg 2. While transiting to Pt. Conception, a critical circuit breaker on the main electrical distribution bus failed but was repaired while underway. On 7 July, the acoustic lander deployed near Pt. Conception during the 2025 IWCPS was successfully recovered and a new one was deployed. At 2200, acoustic sampling resumed along transect 015. Electrical repairs to the electrical bus were not successful while underway so Lasker was forced to call port in Port Hueneme, CA for repairs. On 8 July at 1930, Lasker arrived at Naval Pier War 3 North, Port Hueneme. On 10 July at 1600, following successful repairs. Lasker departed Port Hueneme and resumed acoustic sampling along transect 016 at 0215 on 11 July. On 19 July, acoustic sampling was completed along transects 038 and 037. From 0130 on 20 July until 0330 on 21 July, acoustic sampling was conducted along the remaining offshore portions of transects 036 to 032 were sampled from north to south before transiting to San Francisco Bay. On 21 July at 1645, Lasker arrived at Coast Guard Island in Alameda, CA to conclude Leg 2.  

**Leg III**

On 26 July, Lasker departed from Alameda at 1830 to begin Leg 3. On 27 July, acoustic sampling resumed at 1300 along transect 039 near Pt. Arena. Acoustic and trawl operations proceeded smoothly for the majority of the survey leg. Inclement weather was encountered on 1 August, which limited nighttime trawling opportunities for two days. On 8 August, along transect 070, the winch control board in MVP system failed but was repaired while underway using spares delivered with the system. At 0145 on 10 August, acoustic sampling was completed after sampling transects 072 and 073 off Tillamook and the ship transited toward Newport. On 10 August, Lasker arrived at MOC-P in Newport at 1245 to end Leg 3.  

**Leg IV**

On 23 August, after a delay of 9 days to repair leaks in the main propulsion motor aft heat exchanger, Lasker departed from MOC-P in Newport at 1630 and began the transit to transect 074. To mitigate the loss of sea days, the remaining transects were reduced in length by approximately 35% and the spacing between transects was increased from 12.5 to 15 nmi. Acoustic and trawl sampling resumed on August 24 along transect 074 near Astoria, OR and continued without interruption for the remainder of Leg 4. August 24th the MVP failed to return to the surface and had to be manually brought back on deck.  The UCTD was utilized for the rest of leg 4.  On 27 August, acoustic and trawl sampling ceased after the completion of transect 084 off Cape Flattery. On 29 August, Lasker arrived at MOC-P in Newport at ~2100 to complete Leg 4 and the CPS portion of the survey.  
...
Purse-seine catches from _Long Beach Carnage_ in the nearshore region were composed mostly of Pacific Sardine and Northern Anchovy collected along the mainland coast (**Fig. \@ref(fig:nasc-seine-lbc)b**). Relatively few Pacific Mackerel, and Jack Mackerel were present in purse-seine samples collected by _Long Beach Carnage_ (**Fig. \@ref(fig:nasc-seine-lbc)b**). Overall, dip-net samples from `set.summ$n[set.summ$vessel.name == "LBC"]` seines totaled `prettyNum(sum(seine.summ.lbc$total.wt),big.mark=",",digits=3)` kg of CPS (`prettyNum(seine.summ.lbc$total.wt[seine.summ.lbc$scientificName == "Sardinops sagax"],big.mark=",",digits=3)` kg of Pacific Sardine, `prettyNum(seine.summ.lbc$total.wt[seine.summ.lbc$scientificName == "Scomber japonicus"],big.mark=",",digits=3)` kg of Pacific Mackerel, `prettyNum(seine.summ.lbc$total.wt[seine.summ.lbc$scientificName == "Engraulis mordax"],big.mark=",",digits=2)` kg of Northern Anchovy, and `prettyNum(seine.summ.lbc$total.wt[seine.summ.lbc$scientificName == "Trachurus symmetricus"],big.mark=",",digits=2)` kg of Jack Mackerel, **Appendix \@ref(appendix-seine-sampling-lbc)**).  

### _Lisa Marie_ {#results-seine-catch-lm}  

Purse-seine catches from _Lisa Marie_ in the nearshore were composed mostly of Pacific Herring, except Northern Anchovy collected between Half Moon Bay and Bodega Bay, several catches of Pacific Sardine between Pt. Arena and Eureka, CA, and a few catches of Jack Mackerel near Crescent City and north of Tillamook, OR (**Fig. \@ref(fig:nasc-seine-lm)b**). Purse-seine sampling between Cape Mendocino and Tillamook was sparse due to few CPS targets, the presence of marine mammals that did not permit the deployment of the purse-seine gear, or both. Many of the purse-seine sets north of Cape Mendocino contained no CPS (**Fig. \@ref(fig:nasc-seine-lm)b**). Overall, the dip-net samples from `set.summ$n[set.summ$vessel.name == "LM"]` purse-seine sets totaled `prettyNum(sum(seine.summ.lm$total.wt),big.mark=",",digits=3)` kg of CPS (`prettyNum(seine.summ.lm$total.wt[seine.summ.lm$scientificName == "Trachurus symmetricus"],big.mark=",",digits=3)` kg of Jack Mackerel, `prettyNum(seine.summ.lm$total.wt[seine.summ.lm$scientificName == "Sardinops sagax"],big.mark=",",digits=3)` kg of Pacific Sardine, `prettyNum(seine.summ.lm$total.wt[seine.summ.lm$scientificName == "Clupea pallasii"],big.mark=",",digits=3)` kg of Pacific Herring, and `prettyNum(seine.summ.lm$total.wt[seine.summ.lm$scientificName == "Engraulis mordax"],big.mark=",",digits=3)` kg of Northern Anchovy, **Appendix \@ref(appendix-seine-sampling-lm)**).  
...
**[Update DAS discussion as needed.]**
...
if (file.exists(here("Images/img_sardine_stock_structure_lengths_summer2025.jpg"))) {
include_graphics(here("Images/img_sardine_stock_structure_lengths_summer2025.jpg"))  
} else {
  print("No stock structure figure available.")
}
```
...
Relative beam-compensated target strength ($TS_{rel}$, dB re 1 m^2^) measurements of a WC38.1 sphere at `r echo.freqs[survey.vessel.primary]` kHz for echosounders aboard _`r survey.vessel`_. $TS_{rel}$ is calculated as the difference between the beam-compensated target strength ($TS_c$) and the theoretical target strength ($TS_{theory}$).

```{r tsc-plot-rl, out.width='7in',fig.align='center',fig.pos='H'}
if (file.exists(here("Figs/fig_cal_TSrel_scatter_RL.png"))) {
...
if (file.exists(here("Figs/fig_cal_FM_AllFreqs_RL.png"))) {
...
all.ctds.table <- uctd.table %>%  # bind_rows(ctd.table, uctd.table) %>% 
...
trawl.summ <- haul.summ.wt %>%
  ...
  select(Haul = haul, Date, Latitude, Longitude, "N. Anchovy" = Anchovy, "P. Sardine" = Sardine, 
         ...
         "R. Herring" = RndHerring, Smelt, "All CPS" = AllCPS) %>% 
  # Reformat the date
...
Date, time (UTC), location, and biomasses (kg) of CPS collected for each purse-seine set by _Long Beach Carnage_. A total of `max(sets$set[sets$ship == "LBC"])` sets were conducted but `num2words(max(sets$set[sets$ship == "LBC"]) - nrow(sets[sets$ship == "LBC",]))` was excluded and not shown here.  

```{r seine-catch-summary-lbc,results='asis',eval=FALSE}
# Summarize trawl haul data
...
```{r seine-catch-summary-lm,results='asis',eval=FALSE}
# Summarize catch by species, used to subset columns
...
* _`r survey.vessel.long`_
  + `r cal.personnel[survey.vessel.primary]`
* _Long Beach Carnage_
  + `r cal.personnel["LBC"]`
* _Lisa Marie_
  + `r cal.personnel["LM"]`
