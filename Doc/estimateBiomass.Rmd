...
for (i in unique(strata.final$scientificName)) {
  ...
  # Old method for drawing primary polygons ----------------------------------------
...
# Create sf used to remove nearshore portions of the core area transects ---------------------------
# # Convert nearshore transect ends to sf
# tx.ends.sf <- tx.ends %>% 
#   st_as_sf(coords = c("long.i","lat.i"), crs = 4326) %>% 
#   st_transform(crs = crs.proj)
# 
# # # Read North America landmask shapefile
# # na_landmask <- st_read(here("Data/GIS/na_landmask_final.shp")) %>% 
# #   st_union() %>% 
# #   st_transform(crs = crs.proj)
# 
# # Compute distances from each transect end to the mainland coastline (in meters)
# dist.to.shore <- mean(as.numeric(st_distance(tx.ends.sf, na_landmask)))
# 
# # Buffer land mask by the mean distance from shore 
# na_buffer <- st_buffer(na_landmask, dist = dist.to.shore) %>% 
#   st_transform(crs.geog)

# Preview results
# mapview(na_buffer) +
#   mapview(filter(strata.primary,
#                  scientificName == "Clupea pallasii"),
#           zcol = "stratum") +
#   mapview(tx.ends.sf)

# # Clip primary polygons using the buffered landmask polygon -------------------------
# strata.primary <- strata.primary %>% 
#   st_make_valid() %>% 
#   st_difference(na_buffer) %>% 
#   ungroup() %>% 
...
#   st_cast("POLYGON") %>%
#   st_make_valid() %>%
#   st_difference(st_union(st_transform(ns_mask, crs = crs.geog))) %>%
#   mutate(area = st_area(.))

# Write final strata to shapefile ---------------------------
# ## Remove polygons that are GEOMETRYCOLLECTION
...
if (combine.regions) {
  ...
  if (save.figs) {
    ...
    # # Save maps
    # ggsave(nasc.cufes.prop.plot, 
    #        filename = here("Figs/fig_nasc_cufes_acoustic_cluster_combo.png"),
    #        width = map.width*3, height = map.height)
    # 
    # ggsave(nasc.cufes.prop.plot.long, 
    #        filename = here("Figs/fig_nasc_cufes_acoustic_cluster_combo_long.png"),
    #        width = map.width, height = map.height*3) 
  }
}
```
