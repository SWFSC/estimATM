...
```{r ctd-wpts-table}
if (nrow(ctd) > 0) {
  if (class(ctd) == "data.frame") {
    ctd %>% 
      select(Station = name, Region, Latitude, Longitude, Depth) %>% 
      arrange(Station) %>%
      datatable(rownames = FALSE)  
  } else if (inherits(ctd.sf, "sf")) {
    ctd %>% 
      select(Station = name, Region, Depth) %>% 
      arrange(Station) %>%
      mutate(
...
```{r uctd-wpts-table}
if (nrow(uctd) > 0) {
  if (class(uctd) == "data.frame") {
    uctd %>% 
      select(Station = name, Region, Latitude, Longitude, Depth) %>% 
      arrange(Station) %>%
      datatable(rownames = FALSE)  
  } else if (inherits(uctd.sf, "sf")) {
    uctd %>% 
      select(Station = name, Region, Depth) %>% 
      arrange(Station) %>%
      mutate(
...
```{r eDNA-wpts-table}
if (nrow(eDNA) > 0) {
  if (class(eDNA) == "data.frame") {
    eDNA %>% 
      select(Station = name, Region, Latitude, Longitude, Depth) %>% 
      arrange(Station) %>%
      datatable(rownames = FALSE)  
  } else if (inherits(eDNA, "sf")) {
    eDNA %>% 
      select(Station = name, Region, Depth) %>% 
      arrange(Station) %>%
      mutate(
...
if (nrow(pairovets) > 0) {
  if (class(pairovets) == "data.frame") {
    pairovets %>% 
      select(Station = name, Region, Latitude, Longitude, Depth) %>% 
      arrange(Station) %>%
      datatable(rownames = FALSE)  
  } else if (inherits(pairovets, "sf")) {
    pairovets %>% 
      select(Station = name, Region, Depth) %>% 
      arrange(Station) %>%
      mutate(
...
# save survey plan map
ggsave(here("Figs/fig_survey_plan_tx_only.png"), survey.plan,
       height = map.height, width = map.width)
