---
title: "Simulate CPS biomass from Acoustic-Trawl Surveys (Even Clusters Only)"
...
# Calls child document that contains background information about the 
# Acoustic Trawl Method employed by the SWFSC to survey coastal pelagic fishes
```
...
downsample.data   <- T # Downsample for simulation purposes
...
Files are copied locally for processing to avoid slow results when working across the network (especially from remote locations). Controls in the User Settings section (above) determine whether files are copied locally (usually `TRUE` during the first run) and whether files are overwritten. If files are known to have changed, set `overwrite.files <- TRUE` to overwrite files already present, else files already present will %>% skipped and only new files copied to the local directory. If files have not changed, set `copy.files <- FALSE` to speed processing (`overwrite.files` will be ignored and may be left set to `TRUE`).
...
if (download.hab) {
  if (date(erddap.survey.end) > date(now())) {
    ...
    hab.date.end   <- format((date(now()) - days(2)), "%Y%m%d")
  } else {
...
if (process.csv) {
  ...
  for (i in nasc.vessels) {
    ...
    if (!process.csv.all) {
      ...
      nasc.files <- nasc.files[!nasc.files %in% processed.csv]
    ...
    if (length(nasc.files) > 0) {
      ...
      processed.csv.vessel <- sort(unique(nasc.vessel$filename))
      ...
      # Indentify acoustic transects to include in processing, based on length
...
if (downsample.data) {
  ...
  nasc <- nasc %>% 
    filter(transect.name %in% transects.sf$transect.name)
}
...
nasc.outlier.plotly <- ggplot(big.nasc, 
                              aes(rank, cps.nasc, ids = label, fill = vessel.name)) +
...
if (save.figs) {
  ...
  nasc.daynight.plot <- base.map + 
    ...
    scale_colour_manual(name = "Time of day", values = c("Day" = "orange", "Night" = "black")) +
    scale_size_manual(name = "Time of day", values = c("Day" = 1, "Night" = 2)) +
...
Trawl hauls where a single species makes up $\geq$ 80% of the catch (by weight) are identified and haul metadata exported for furher analysis in Echoview to estimate _in situ_ target strength of various CPS.  
...
# Export haul data for single-target TS esimation
...
# Explore st_vironoi for faster assignment of clusters
# nasc.env <- st_convex_hull(nav.sf) %>% st_transform(crs = 3310)
# # 
# clust <- st_transform(cluster.mid.sf, crs = 3310)
# 
# voronoi.clusters <- st_voronoi(clust, envelope = nasc.env)

if (process.csv) {
  # Create varialble for nearest cluster and minumum distance
  cluster.distance <- data.frame(cluster = rep(NA, nrow(nasc)), 
  ...
  save(cluster.distance, file = here("Output/nasc_cluster_distance.Rdata"))
  
} else {
  load(here("Output/nasc_cluster_distance.Rdata"))  
  
}

# Add cluster distances to nasc
nasc <- bind_cols(nasc, cluster.distance)
```
...
if (save.figs) {
  ...
  nasc.cluster.plot <- base.map +
    ...
    # Plot panel label
    # ggtitle("Integrated NASC Clusters") +
    coord_sf(crs = crs.proj, 
             xlim = c(map.bounds["xmin"], map.bounds["xmax"]), 
             ylim = c(map.bounds["ymin"], map.bounds["ymax"]))
...
# load(here("Output/lengths_all.Rdata"))
...
if (nrow(catch.comp.errors) > 0) {
  write.csv(catch.comp.errors,file = here("Output/catch_comparison_errors.csv"),quote = F,row.names = F)
}
```  
...
### Examine speciment L-W data  

The relationship between lenght and weight of all specimens is visually compared to those from historic surveys and also to L/W models derived from those specimens. If missing length or weight measurements are present, they are estimated from these models using the following equations and the coefficients in the table below:  

$$W = a*L^b\text{,  and}$$  

$$L = (W/a)^{(1/b)}\text{.}$$   

Coefficients used to estimate total length (TL, mm) or weight in the equations above. Model types are: 'lm' = linear model, 'nlm' = naive linear model, and 'glm' = general linear model (Palance et al., in prep). Season is presently either spring or summer, and is defined in the survey info file.    
...
# Save results
write.csv(ts.df,   file = here("Output/ts_estimates.csv"), quote = F, row.names = F)
write.csv(lengths, file = here("Output/ts_lengths.csv"),   quote = F, row.names = F)
```  
...
# Save to file
write.csv(clf, file = here("Output/clf_cluster_summary.csv"), 
          quote = F, row.names = F)
```  
...
# Save to file
write.csv(ts.proportions, file = here("Output/ts_proportions_raw.csv"), 
          row.names = F, quote = F)
...
# Save to file
write.csv(clf, file = here("Output/clf_ts_proportions.csv"),
          row.names = F, quote = F)
...
for (i in unique(lengths$scientificName)) {
  ...
  # Save plot
  ggsave(lf.plot, 
         filename = paste(here("Figs/fig_cluster_length_frequency_"), 
                          i, ".png", sep = ""),
         height = 7, width = 10) 
}
...
save(nasc, clf, file = here("Output/nasc_clf.Rdata"))
...
nasc.clust.biom <- nasc.clust.hist %>% 
  ...
  mutate(distance = dist.bin * ceiling(max(cluster.distance)/30))
...
if (nrow(strata.summ.rm) > 0) {
  ...
  ggsave(biomass.dens.plot.tx.name, 
         filename = here("Figs/fig_biomass_density_transect_tx_name.png"),
         height = 10, width = 20)
...
if (save.figs) {
  ...
  for (i in unique(strata.primary$scientificName)) {
    for (j in unique(filter(strata.primary, scientificName == i)$stock)) {
      ...
      # Save figures
      ggsave(biomass.dens, 
             filename = paste(here("Figs/fig_biomass_dens_"), i, "-", j, ".png",sep = ""),
             width  = map.width,height = map.height)
...
for (i in cps.spp) {
  if (is.null(biomass.dens.figs[[i]])) {
    ...
    ggsave(biomass.dens.temp, 
           filename = paste(here("Figs/fig_biomass_dens_"), i, ".png", sep = ""))
  }
}
```
...
```{r get-ctd-nav}
# Assemble lat/long data from nav and nasc
ctd.nav <- select(nav, datetime, long, lat) %>% 
  ...
  if (length(uctd.hdr) == 0) {
    ...
    uctd.summ <- all.uctd.casts %>% 
      ...
      summarise(
        ...
        type = "UCTD") %>%
      left_join(select(all.uctd.hdr, cast, cast.date, datetime, lat, long, SOG, leg, lag)) %>%
      left_join(select(uctd.class, cast, class)) %>% 
      arrange(cast.date) %>%
...
uctd.table <- uctd.summ %>% 
  select("Cast" = cast.num, "Filename" = cast, "Date" = cast.date, 
         "Lat" = lat, "Long" = long, "Dutation" = time, "Depth" = max.depth,
         "Class" = class, "Lag" = lag) %>% 
...
if (length(ctd.hdr) == 0) {
  ...
  all.ctd.hdr <- all.ctd.hdr %>% 
    ...
    mutate(
      cast.num = seq(1, n()),
      type = "CTD",
      lag = difftime(cast.date, datetime, units = "secs"))
...
```{r interactive-data-explorer, fig.cap='(ref:interactive-data-explorer)',echo=FALSE,eval=FALSE}
# Leaflet options
...
# Create color pallette for CTD casts
...
if (save.imap) {
  mapshot(i.map, url = here("Output/estimateBiomass_explorer.HTML"))
}

# Save imap
dir_create(here("Output/imap"))
save(i.map, file = here("Output/imap/estimateBiomass_explorer.Rdata"))
...
if (combine.regions) {
  ...
  # Save combined nasc
  save(nasc.combo, here("Output/nasc_combo.Rdata"))
...
# Insert disaggregated abundance plots for jack mackerel
