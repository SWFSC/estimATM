...
# Create base map
base.map <- get_basemap(filter(transects, loc == "central"), states, countries,
...
if (nrow(filter(transects, Type == "Nearshore", Transect %in% lbc.transects)) > 0) {
  ...
  # Save the base map
  ggsave(lbc.map.plain, file = here("Figs", paste0("fig_survey_plan_LBC_plain.png")),
         height = map.height.region["central"], width = map.width["central"])
  
  lbc.map.plain2 <- base.map +
    geom_sf(data = filter(transects, Type == "Nearshore", Transect %in% lbc.transects),
            colour = "magenta",
            show.legend = "line") +
    coord_sf(crs = crs.proj, # CA Albers Equal Area Projection
