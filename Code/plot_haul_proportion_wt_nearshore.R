...
if (exists("haul.pos.ns.deep")) {
  ...
  if (nrow(haul.pos.ns.deep) > 0) {
    ...
    set.pie.haul.wt.deep <- base.map +
      ...
      # Plot empty cluster locations
      geom_point(data = haul.zero.ns.deep, aes(X, Y),
                 size = 3, shape = 21, fill = 'black', colour = 'white') +
      # Plot panel label
  ...
  if (doc.name == "simulateBiomass.Rmd") {
    # Save nasc plot
    ggsave(set.pie.haul.wt.deep,
           filename = here("Simulation/Figs/fig_trawl_proportion_haul_wt_nearshore_deep.png"),
           width = map.width, height = map.height) 
  } else if (doc.name == "reportMexico.Rmd") {
    # Save nasc plot
    ggsave(set.pie.haul.wt.deep,
           filename = here("Figs/fig_trawl_proportion_haul_wt_mx_nearshore_deep.png"),
           width = map.width, height = map.height) 
  } else {
    # Save nasc plot
    ggsave(set.pie.haul.wt.deep,
           filename = here("Figs/fig_trawl_proportion_haul_wt_nearshore_deep.png"),
           width = map.width, height = map.height) 
