...
## Retain core area intervals that fall outside the nearshore mask ----------
nasc.sf.sub <- nasc %>% 
  st_as_sf(coords = c("long","lat"), crs = crs.geog) %>% 
  st_transform(crs = crs.proj) %>% 
...
# Keep core area acoustic intervals that intersect the nearshore mask
...
if (save.figs) {
  ...
  haul.ts.map <- base.map + 
    ...
    scale_colour_manual(name = 'Species',
                        labels = unname(pie.labs[names(pie.labs) %in% haul.ts.spp]),
                        values = unname(pie.colors[names(pie.colors) %in% haul.ts.spp])) +
    coord_sf(crs = crs.proj, 
             xlim = unname(c(map.bounds["xmin"], map.bounds["xmax"])), 
             ylim = unname(c(map.bounds["ymin"], map.bounds["ymax"])))
...
acoustic.prop.wg <- clf %>%
  ...
  select(cluster, lat, long, prop.anch = prop.anch.wg, prop.jack = prop.jack.wg, 
         ...
         prop.other = prop.other.wg, sample.type) %>% 
  replace(. == 0, 0.0000001) %>% 
  project_df(to = crs.proj) %>% 
  mutate(pie.radius = pie.radius)
...
for (i in unique(lengths$scientificName)) {
  ...
  lf.table.h <- pivot_wider(lf.df, id_cols = haul, names_from = lf.labels, values_from = counts, values_fn = sum) %>% 
    rowwise(haul) %>%
    mutate('(all)' = sum(c_across(all_of(lf.labels)))) %>% 
    arrange(haul)
...
tx.sampled <- transects.sf %>% 
  filter(Type == "Compulsory", Transect %in% tx.mid$transect.orig) %>%
  left_join(select(tx.nn, Transect = transect.orig, transect, min.dist)) 
  ...
  if(nrow(filter(strata.nearshore.sub, scientificName == "Clupea pallasii")) > 0)
    i.map <- i.map %>% 
      addPolygons(data = filter(strata.nearshore.sub, scientificName == "Clupea pallasii"), 
                  weight = 2, color = "#000414", fillColor = ~cpsPal(scientificName),
                  # weight = 2, color = ~cpsPal(scientificName), fillColor = ~cpsPal(scientificName),
                  fillOpacity = 0.5, group = "Biomass (Herring)") %>% 
