...
if (exists("cluster.pos.ns.deep")) {
  # Get species present in the trawl catch
  pie.spp.ns.deep <- set.catch %>% 
    filter(scientificName %in% c("Clupea pallasii", "Engraulis mordax")) %>% 
    pull(scientificName) %>% 
    unique() %>% 
    sort()
  ...
  if (nrow(cluster.pos.ns.deep) > 0) {
    # Create trawl haul figure
    set.pie.cluster.wt.deep <- base.map +
      # Plot transects data
      geom_sf(data = filter(transects.sf, Type == "Nearshore"), size = 0.5, colour = "gray70", 
              alpha = 0.75, linetype = "dashed") +
      # Plot trawl pies
      ...
      # Configure trawl scale
      scale_fill_manual(name = 'Species',
                        labels = unname(pie.labs[names(pie.labs) %in% pie.spp.ns.deep]),
                        values = unname(pie.colors[names(pie.colors) %in% pie.spp.ns.deep])) +
      # Plot empty cluster locations
      geom_point(data = cluster.zero.ns.deep, aes(X, Y),
                 size = 3, shape = 21, fill = 'black', colour = 'white') +
      # Plot panel label
      coord_sf(crs = crs.proj, # CA Albers Equal Area Projection
               xlim = unname(c(map.bounds["xmin"], map.bounds["xmax"])), 
  ...
  if (doc.name == "simulateBiomass.Rmd") {
    # Save nasc plot
    ggsave(set.pie.cluster.wt.deep,
           filename = here("Simulation/Figs/fig_trawl_proportion_cluster_wt_nearshore_deep.png"),
           width = map.width, height = map.height) 
  } else if (doc.name == "reportMexico.Rmd") {
    # Save nasc plot
    ggsave(set.pie.cluster.wt.deep,
           filename = here("Figs/fig_trawl_proportion_cluster_wt_mx_nearshore_deep.png"),
           width = map.width, height = map.height) 
  } else {
    # Save nasc plot
    ggsave(set.pie.cluster.wt.deep,
           filename = here("Figs/fig_trawl_proportion_cluster_wt_nearshore_deep.png"),
           width = map.width, height = map.height) 
